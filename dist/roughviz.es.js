function ps(i, t) {
  return i < t ? -1 : i > t ? 1 : i >= t ? 0 : NaN;
}
function ms(i) {
  let t = i, e = i;
  function s(n, r, a, h) {
    for (a == null && (a = 0), h == null && (h = n.length); a < h; ) {
      const o = a + h >>> 1;
      e(n[o], r) < 0 ? a = o + 1 : h = o;
    }
    return a;
  }
  return i.length === 1 && (t = (n, r) => i(n) - r, e = function(n) {
    return (r, a) => ps(n(r), a);
  }(i)), { left: s, center: function(n, r, a, h) {
    a == null && (a = 0), h == null && (h = n.length);
    const o = s(n, r, a, h - 1);
    return o > a && t(n[o - 1], r) > -t(n[o], r) ? o - 1 : o;
  }, right: function(n, r, a, h) {
    for (a == null && (a = 0), h == null && (h = n.length); a < h; ) {
      const o = a + h >>> 1;
      e(n[o], r) > 0 ? h = o : a = o + 1;
    }
    return a;
  } };
}
const rn = ms(ps).right;
ms(function(i) {
  return i === null ? NaN : +i;
}).center;
const As = rn;
function st(i, t) {
  let e, s;
  if (t === void 0)
    for (const n of i)
      n != null && (e === void 0 ? n >= n && (e = s = n) : (e > n && (e = n), s < n && (s = n)));
  else {
    let n = -1;
    for (let r of i)
      (r = t(r, ++n, i)) != null && (e === void 0 ? r >= r && (e = s = r) : (e > r && (e = r), s < r && (s = r)));
  }
  return [e, s];
}
var Vi = Math.sqrt(50), Yi = Math.sqrt(10), Hi = Math.sqrt(2);
function ve(i, t, e) {
  var s = (t - i) / Math.max(0, e), n = Math.floor(Math.log(s) / Math.LN10), r = s / Math.pow(10, n);
  return n >= 0 ? (r >= Vi ? 10 : r >= Yi ? 5 : r >= Hi ? 2 : 1) * Math.pow(10, n) : -Math.pow(10, -n) / (r >= Vi ? 10 : r >= Yi ? 5 : r >= Hi ? 2 : 1);
}
function Q(i, t) {
  let e;
  if (t === void 0)
    for (const s of i)
      s != null && (e < s || e === void 0 && s >= s) && (e = s);
  else {
    let s = -1;
    for (let n of i)
      (n = t(n, ++s, i)) != null && (e < n || e === void 0 && n >= n) && (e = n);
  }
  return e;
}
function ut(i, t) {
  let e;
  if (t === void 0)
    for (const s of i)
      s != null && (e > s || e === void 0 && s >= s) && (e = s);
  else {
    let s = -1;
    for (let n of i)
      (n = t(n, ++s, i)) != null && (e > n || e === void 0 && n >= n) && (e = n);
  }
  return e;
}
function ys(i, t, e) {
  i = +i, t = +t, e = (n = arguments.length) < 2 ? (t = i, i = 0, 1) : n < 3 ? 1 : +e;
  for (var s = -1, n = 0 | Math.max(0, Math.ceil((t - i) / e)), r = new Array(n); ++s < n; )
    r[s] = i + s * e;
  return r;
}
var wi = Array.prototype.slice;
function hn(i) {
  return i;
}
var be = 1e-6;
function an(i) {
  return "translate(" + (i + 0.5) + ",0)";
}
function on(i) {
  return "translate(0," + (i + 0.5) + ")";
}
function ln(i) {
  return function(t) {
    return +i(t);
  };
}
function un(i) {
  var t = Math.max(0, i.bandwidth() - 1) / 2;
  return i.round() && (t = Math.round(t)), function(e) {
    return +i(e) + t;
  };
}
function cn() {
  return !this.__axis;
}
function xs(i, t) {
  var e = [], s = null, n = null, r = 6, a = 6, h = 3, o = i === 1 || i === 4 ? -1 : 1, l = i === 4 || i === 2 ? "x" : "y", u = i === 1 || i === 3 ? an : on;
  function d(c) {
    var g = s ?? (t.ticks ? t.ticks.apply(t, e) : t.domain()), p = n ?? (t.tickFormat ? t.tickFormat.apply(t, e) : hn), f = Math.max(r, 0) + h, y = t.range(), m = +y[0] + 0.5, A = +y[y.length - 1] + 0.5, b = (t.bandwidth ? un : ln)(t.copy()), S = c.selection ? c.selection() : c, w = S.selectAll(".domain").data([null]), v = S.selectAll(".tick").data(g, t).order(), M = v.exit(), j = v.enter().append("g").attr("class", "tick"), C = v.select("line"), P = v.select("text");
    w = w.merge(w.enter().insert("path", ".tick").attr("class", "domain").attr("stroke", "currentColor")), v = v.merge(j), C = C.merge(j.append("line").attr("stroke", "currentColor").attr(l + "2", o * r)), P = P.merge(j.append("text").attr("fill", "currentColor").attr(l, o * f).attr("dy", i === 1 ? "0em" : i === 3 ? "0.71em" : "0.32em")), c !== S && (w = w.transition(c), v = v.transition(c), C = C.transition(c), P = P.transition(c), M = M.transition(c).attr("opacity", be).attr("transform", function(F) {
      return isFinite(F = b(F)) ? u(F) : this.getAttribute("transform");
    }), j.attr("opacity", be).attr("transform", function(F) {
      var k = this.parentNode.__axis;
      return u(k && isFinite(k = k(F)) ? k : b(F));
    })), M.remove(), w.attr("d", i === 4 || i == 2 ? a ? "M" + o * a + "," + m + "H0.5V" + A + "H" + o * a : "M0.5," + m + "V" + A : a ? "M" + m + "," + o * a + "V0.5H" + A + "V" + o * a : "M" + m + ",0.5H" + A), v.attr("opacity", 1).attr("transform", function(F) {
      return u(b(F));
    }), C.attr(l + "2", o * r), P.attr(l, o * f).text(p), S.filter(cn).attr("fill", "none").attr("font-size", 10).attr("font-family", "sans-serif").attr("text-anchor", i === 2 ? "start" : i === 4 ? "end" : "middle"), S.each(function() {
      this.__axis = b;
    });
  }
  return d.scale = function(c) {
    return arguments.length ? (t = c, d) : t;
  }, d.ticks = function() {
    return e = wi.call(arguments), d;
  }, d.tickArguments = function(c) {
    return arguments.length ? (e = c == null ? [] : wi.call(c), d) : e.slice();
  }, d.tickValues = function(c) {
    return arguments.length ? (s = c == null ? null : wi.call(c), d) : s && s.slice();
  }, d.tickFormat = function(c) {
    return arguments.length ? (n = c, d) : n;
  }, d.tickSize = function(c) {
    return arguments.length ? (r = a = +c, d) : r;
  }, d.tickSizeInner = function(c) {
    return arguments.length ? (r = +c, d) : r;
  }, d.tickSizeOuter = function(c) {
    return arguments.length ? (a = +c, d) : a;
  }, d.tickPadding = function(c) {
    return arguments.length ? (h = +c, d) : h;
  }, d;
}
function Ht(i) {
  return xs(3, i);
}
function Dt(i) {
  return xs(4, i);
}
var ke = {}, Mi = {};
function Se(i) {
  return new Function("d", "return {" + i.map(function(t, e) {
    return JSON.stringify(t) + ": d[" + e + '] || ""';
  }).join(",") + "}");
}
function we(i) {
  var t = /* @__PURE__ */ Object.create(null), e = [];
  return i.forEach(function(s) {
    for (var n in s)
      n in t || e.push(t[n] = n);
  }), e;
}
function U(i, t) {
  var e = i + "", s = e.length;
  return s < t ? new Array(t - s + 1).join(0) + e : e;
}
function dn(i) {
  var t, e = i.getUTCHours(), s = i.getUTCMinutes(), n = i.getUTCSeconds(), r = i.getUTCMilliseconds();
  return isNaN(i) ? "Invalid Date" : ((t = i.getUTCFullYear()) < 0 ? "-" + U(-t, 6) : t > 9999 ? "+" + U(t, 6) : U(t, 4)) + "-" + U(i.getUTCMonth() + 1, 2) + "-" + U(i.getUTCDate(), 2) + (r ? "T" + U(e, 2) + ":" + U(s, 2) + ":" + U(n, 2) + "." + U(r, 3) + "Z" : n ? "T" + U(e, 2) + ":" + U(s, 2) + ":" + U(n, 2) + "Z" : s || e ? "T" + U(e, 2) + ":" + U(s, 2) + "Z" : "");
}
function vs(i) {
  var t = new RegExp('["' + i + `
\r]`), e = i.charCodeAt(0);
  function s(h, o) {
    var l, u = [], d = h.length, c = 0, g = 0, p = d <= 0, f = !1;
    function y() {
      if (p)
        return Mi;
      if (f)
        return f = !1, ke;
      var A, b, S = c;
      if (h.charCodeAt(S) === 34) {
        for (; c++ < d && h.charCodeAt(c) !== 34 || h.charCodeAt(++c) === 34; )
          ;
        return (A = c) >= d ? p = !0 : (b = h.charCodeAt(c++)) === 10 ? f = !0 : b === 13 && (f = !0, h.charCodeAt(c) === 10 && ++c), h.slice(S + 1, A - 1).replace(/""/g, '"');
      }
      for (; c < d; ) {
        if ((b = h.charCodeAt(A = c++)) === 10)
          f = !0;
        else if (b === 13)
          f = !0, h.charCodeAt(c) === 10 && ++c;
        else if (b !== e)
          continue;
        return h.slice(S, A);
      }
      return p = !0, h.slice(S, d);
    }
    for (h.charCodeAt(d - 1) === 10 && --d, h.charCodeAt(d - 1) === 13 && --d; (l = y()) !== Mi; ) {
      for (var m = []; l !== ke && l !== Mi; )
        m.push(l), l = y();
      o && (m = o(m, g++)) == null || u.push(m);
    }
    return u;
  }
  function n(h, o) {
    return h.map(function(l) {
      return o.map(function(u) {
        return a(l[u]);
      }).join(i);
    });
  }
  function r(h) {
    return h.map(a).join(i);
  }
  function a(h) {
    return h == null ? "" : h instanceof Date ? dn(h) : t.test(h += "") ? '"' + h.replace(/"/g, '""') + '"' : h;
  }
  return { parse: function(h, o) {
    var l, u, d = s(h, function(c, g) {
      if (l)
        return l(c, g - 1);
      u = c, l = o ? function(p, f) {
        var y = Se(p);
        return function(m, A) {
          return f(y(m), A, p);
        };
      }(c, o) : Se(c);
    });
    return d.columns = u || [], d;
  }, parseRows: s, format: function(h, o) {
    return o == null && (o = we(h)), [o.map(a).join(i)].concat(n(h, o)).join(`
`);
  }, formatBody: function(h, o) {
    return o == null && (o = we(h)), n(h, o).join(`
`);
  }, formatRows: function(h) {
    return h.map(r).join(`
`);
  }, formatRow: r, formatValue: a };
}
var gn = vs(",").parse, fn = vs("	").parse;
function pn(i) {
  if (!i.ok)
    throw new Error(i.status + " " + i.statusText);
  return i.text();
}
function bs(i) {
  return function(t, e, s) {
    return arguments.length === 2 && typeof e == "function" && (s = e, e = void 0), function(n, r) {
      return fetch(n, r).then(pn);
    }(t, e).then(function(n) {
      return i(n, s);
    });
  };
}
var vt = bs(gn), bt = bs(fn);
function mn(i) {
  if (!i.ok)
    throw new Error(i.status + " " + i.statusText);
  if (i.status !== 204 && i.status !== 205)
    return i.json();
}
function ks(i, t) {
  return fetch(i, t).then(mn);
}
function ni(i, t) {
  if ((e = (i = t ? i.toExponential(t - 1) : i.toExponential()).indexOf("e")) < 0)
    return null;
  var e, s = i.slice(0, e);
  return [s.length > 1 ? s[0] + s.slice(2) : s, +i.slice(e + 1)];
}
var Ss, An = /^(?:(.)?([<>=^]))?([+\-( ])?([$#])?(0)?(\d+)?(,)?(\.\d+)?(~)?([a-z%])?$/i;
function Di(i) {
  if (!(t = An.exec(i)))
    throw new Error("invalid format: " + i);
  var t;
  return new Ti({ fill: t[1], align: t[2], sign: t[3], symbol: t[4], zero: t[5], width: t[6], comma: t[7], precision: t[8] && t[8].slice(1), trim: t[9], type: t[10] });
}
function Ti(i) {
  this.fill = i.fill === void 0 ? " " : i.fill + "", this.align = i.align === void 0 ? ">" : i.align + "", this.sign = i.sign === void 0 ? "-" : i.sign + "", this.symbol = i.symbol === void 0 ? "" : i.symbol + "", this.zero = !!i.zero, this.width = i.width === void 0 ? void 0 : +i.width, this.comma = !!i.comma, this.precision = i.precision === void 0 ? void 0 : +i.precision, this.trim = !!i.trim, this.type = i.type === void 0 ? "" : i.type + "";
}
function Me(i, t) {
  var e = ni(i, t);
  if (!e)
    return i + "";
  var s = e[0], n = e[1];
  return n < 0 ? "0." + new Array(-n).join("0") + s : s.length > n + 1 ? s.slice(0, n + 1) + "." + s.slice(n + 1) : s + new Array(n - s.length + 2).join("0");
}
Di.prototype = Ti.prototype, Ti.prototype.toString = function() {
  return this.fill + this.align + this.sign + this.symbol + (this.zero ? "0" : "") + (this.width === void 0 ? "" : Math.max(1, 0 | this.width)) + (this.comma ? "," : "") + (this.precision === void 0 ? "" : "." + Math.max(0, 0 | this.precision)) + (this.trim ? "~" : "") + this.type;
};
const Re = { "%": function(i, t) {
  return (100 * i).toFixed(t);
}, b: function(i) {
  return Math.round(i).toString(2);
}, c: function(i) {
  return i + "";
}, d: function(i) {
  return Math.abs(i = Math.round(i)) >= 1e21 ? i.toLocaleString("en").replace(/,/g, "") : i.toString(10);
}, e: function(i, t) {
  return i.toExponential(t);
}, f: function(i, t) {
  return i.toFixed(t);
}, g: function(i, t) {
  return i.toPrecision(t);
}, o: function(i) {
  return Math.round(i).toString(8);
}, p: function(i, t) {
  return Me(100 * i, t);
}, r: Me, s: function(i, t) {
  var e = ni(i, t);
  if (!e)
    return i + "";
  var s = e[0], n = e[1], r = n - (Ss = 3 * Math.max(-8, Math.min(8, Math.floor(n / 3)))) + 1, a = s.length;
  return r === a ? s : r > a ? s + new Array(r - a + 1).join("0") : r > 0 ? s.slice(0, r) + "." + s.slice(r) : "0." + new Array(1 - r).join("0") + ni(i, Math.max(0, t + r - 1))[0];
}, X: function(i) {
  return Math.round(i).toString(16).toUpperCase();
}, x: function(i) {
  return Math.round(i).toString(16);
} };
function Pe(i) {
  return i;
}
var Ri, ot, ze = Array.prototype.map, Fe = ["y", "z", "a", "f", "p", "n", "µ", "m", "", "k", "M", "G", "T", "P", "E", "Z", "Y"];
function yn(i) {
  var t, e, s = i.grouping === void 0 || i.thousands === void 0 ? Pe : (t = ze.call(i.grouping, Number), e = i.thousands + "", function(c, g) {
    for (var p = c.length, f = [], y = 0, m = t[0], A = 0; p > 0 && m > 0 && (A + m + 1 > g && (m = Math.max(1, g - A)), f.push(c.substring(p -= m, p + m)), !((A += m + 1) > g)); )
      m = t[y = (y + 1) % t.length];
    return f.reverse().join(e);
  }), n = i.currency === void 0 ? "" : i.currency[0] + "", r = i.currency === void 0 ? "" : i.currency[1] + "", a = i.decimal === void 0 ? "." : i.decimal + "", h = i.numerals === void 0 ? Pe : function(c) {
    return function(g) {
      return g.replace(/[0-9]/g, function(p) {
        return c[+p];
      });
    };
  }(ze.call(i.numerals, String)), o = i.percent === void 0 ? "%" : i.percent + "", l = i.minus === void 0 ? "-" : i.minus + "", u = i.nan === void 0 ? "NaN" : i.nan + "";
  function d(c) {
    var g = (c = Di(c)).fill, p = c.align, f = c.sign, y = c.symbol, m = c.zero, A = c.width, b = c.comma, S = c.precision, w = c.trim, v = c.type;
    v === "n" ? (b = !0, v = "g") : Re[v] || (S === void 0 && (S = 12), w = !0, v = "g"), (m || g === "0" && p === "=") && (m = !0, g = "0", p = "=");
    var M = y === "$" ? n : y === "#" && /[boxX]/.test(v) ? "0" + v.toLowerCase() : "", j = y === "$" ? r : /[%p]/.test(v) ? o : "", C = Re[v], P = /[defgprs%]/.test(v);
    function F(k) {
      var R, O, I, L = M, E = j;
      if (v === "c")
        E = C(k) + E, k = "";
      else {
        var T = (k = +k) < 0 || 1 / k < 0;
        if (k = isNaN(k) ? u : C(Math.abs(k), S), w && (k = function(H) {
          t:
            for (var et, gt = H.length, B = 1, V = -1; B < gt; ++B)
              switch (H[B]) {
                case ".":
                  V = et = B;
                  break;
                case "0":
                  V === 0 && (V = B), et = B;
                  break;
                default:
                  if (!+H[B])
                    break t;
                  V > 0 && (V = 0);
              }
          return V > 0 ? H.slice(0, V) + H.slice(et + 1) : H;
        }(k)), T && +k == 0 && f !== "+" && (T = !1), L = (T ? f === "(" ? f : l : f === "-" || f === "(" ? "" : f) + L, E = (v === "s" ? Fe[8 + Ss / 3] : "") + E + (T && f === "(" ? ")" : ""), P) {
          for (R = -1, O = k.length; ++R < O; )
            if (48 > (I = k.charCodeAt(R)) || I > 57) {
              E = (I === 46 ? a + k.slice(R + 1) : k.slice(R)) + E, k = k.slice(0, R);
              break;
            }
        }
      }
      b && !m && (k = s(k, 1 / 0));
      var q = L.length + k.length + E.length, G = q < A ? new Array(A - q + 1).join(g) : "";
      switch (b && m && (k = s(G + k, G.length ? A - E.length : 1 / 0), G = ""), p) {
        case "<":
          k = L + k + E + G;
          break;
        case "=":
          k = L + G + k + E;
          break;
        case "^":
          k = G.slice(0, q = G.length >> 1) + L + k + E + G.slice(q);
          break;
        default:
          k = G + L + k + E;
      }
      return h(k);
    }
    return S = S === void 0 ? 6 : /[gprs]/.test(v) ? Math.max(1, Math.min(21, S)) : Math.max(0, Math.min(20, S)), F.toString = function() {
      return c + "";
    }, F;
  }
  return { format: d, formatPrefix: function(c, g) {
    var p = d(((c = Di(c)).type = "f", c)), f = 3 * Math.max(-8, Math.min(8, Math.floor(function(A) {
      return (A = ni(Math.abs(A))) ? A[1] : NaN;
    }(g) / 3))), y = Math.pow(10, -f), m = Fe[8 + f / 3];
    return function(A) {
      return p(y * A) + m;
    };
  } };
}
function ne(i, t) {
  switch (arguments.length) {
    case 0:
      break;
    case 1:
      this.range(i);
      break;
    default:
      this.range(t).domain(i);
  }
  return this;
}
Ri = yn({ decimal: ".", thousands: ",", grouping: [3], currency: ["$", ""], minus: "-" }), ot = Ri.format, Ri.formatPrefix;
const We = Symbol("implicit");
function Ai() {
  var i = /* @__PURE__ */ new Map(), t = [], e = [], s = We;
  function n(r) {
    var a = r + "", h = i.get(a);
    if (!h) {
      if (s !== We)
        return s;
      i.set(a, h = t.push(r));
    }
    return e[(h - 1) % e.length];
  }
  return n.domain = function(r) {
    if (!arguments.length)
      return t.slice();
    t = [], i = /* @__PURE__ */ new Map();
    for (const a of r) {
      const h = a + "";
      i.has(h) || i.set(h, t.push(a));
    }
    return n;
  }, n.range = function(r) {
    return arguments.length ? (e = Array.from(r), n) : e.slice();
  }, n.unknown = function(r) {
    return arguments.length ? (s = r, n) : s;
  }, n.copy = function() {
    return Ai(t, e).unknown(s);
  }, ne.apply(n, arguments), n;
}
function Tt() {
  var i, t, e = Ai().unknown(void 0), s = e.domain, n = e.range, r = 0, a = 1, h = !1, o = 0, l = 0, u = 0.5;
  function d() {
    var c = s().length, g = a < r, p = g ? a : r, f = g ? r : a;
    i = (f - p) / Math.max(1, c - o + 2 * l), h && (i = Math.floor(i)), p += (f - p - i * (c - o)) * u, t = i * (1 - o), h && (p = Math.round(p), t = Math.round(t));
    var y = ys(c).map(function(m) {
      return p + i * m;
    });
    return n(g ? y.reverse() : y);
  }
  return delete e.unknown, e.domain = function(c) {
    return arguments.length ? (s(c), d()) : s();
  }, e.range = function(c) {
    return arguments.length ? ([r, a] = c, r = +r, a = +a, d()) : [r, a];
  }, e.rangeRound = function(c) {
    return [r, a] = c, r = +r, a = +a, h = !0, d();
  }, e.bandwidth = function() {
    return t;
  }, e.step = function() {
    return i;
  }, e.round = function(c) {
    return arguments.length ? (h = !!c, d()) : h;
  }, e.padding = function(c) {
    return arguments.length ? (o = Math.min(1, l = +c), d()) : o;
  }, e.paddingInner = function(c) {
    return arguments.length ? (o = Math.min(1, c), d()) : o;
  }, e.paddingOuter = function(c) {
    return arguments.length ? (l = +c, d()) : l;
  }, e.align = function(c) {
    return arguments.length ? (u = Math.max(0, Math.min(1, c)), d()) : u;
  }, e.copy = function() {
    return Tt(s(), [r, a]).round(h).paddingInner(o).paddingOuter(l).align(u);
  }, ne.apply(d(), arguments);
}
function ws(i) {
  var t = i.copy;
  return i.padding = i.paddingOuter, delete i.paddingInner, delete i.paddingOuter, i.copy = function() {
    return ws(t());
  }, i;
}
function je() {
  return ws(Tt.apply(null, arguments).paddingInner(1));
}
function Pi(i, t, e) {
  i.prototype = t.prototype = e, e.constructor = i;
}
function Ce(i, t) {
  var e = Object.create(i.prototype);
  for (var s in t)
    e[s] = t[s];
  return e;
}
function Gt() {
}
var Et = 0.7, Nt = 1 / Et, zt = "\\s*([+-]?\\d+)\\s*", Bt = "\\s*([+-]?\\d*\\.?\\d+(?:[eE][+-]?\\d+)?)\\s*", ht = "\\s*([+-]?\\d*\\.?\\d+(?:[eE][+-]?\\d+)?)%\\s*", xn = /^#([0-9a-f]{3,8})$/, vn = new RegExp("^rgb\\(" + [zt, zt, zt] + "\\)$"), bn = new RegExp("^rgb\\(" + [ht, ht, ht] + "\\)$"), kn = new RegExp("^rgba\\(" + [zt, zt, zt, Bt] + "\\)$"), Sn = new RegExp("^rgba\\(" + [ht, ht, ht, Bt] + "\\)$"), wn = new RegExp("^hsl\\(" + [Bt, ht, ht] + "\\)$"), Mn = new RegExp("^hsla\\(" + [Bt, ht, ht, Bt] + "\\)$"), Ee = { aliceblue: 15792383, antiquewhite: 16444375, aqua: 65535, aquamarine: 8388564, azure: 15794175, beige: 16119260, bisque: 16770244, black: 0, blanchedalmond: 16772045, blue: 255, blueviolet: 9055202, brown: 10824234, burlywood: 14596231, cadetblue: 6266528, chartreuse: 8388352, chocolate: 13789470, coral: 16744272, cornflowerblue: 6591981, cornsilk: 16775388, crimson: 14423100, cyan: 65535, darkblue: 139, darkcyan: 35723, darkgoldenrod: 12092939, darkgray: 11119017, darkgreen: 25600, darkgrey: 11119017, darkkhaki: 12433259, darkmagenta: 9109643, darkolivegreen: 5597999, darkorange: 16747520, darkorchid: 10040012, darkred: 9109504, darksalmon: 15308410, darkseagreen: 9419919, darkslateblue: 4734347, darkslategray: 3100495, darkslategrey: 3100495, darkturquoise: 52945, darkviolet: 9699539, deeppink: 16716947, deepskyblue: 49151, dimgray: 6908265, dimgrey: 6908265, dodgerblue: 2003199, firebrick: 11674146, floralwhite: 16775920, forestgreen: 2263842, fuchsia: 16711935, gainsboro: 14474460, ghostwhite: 16316671, gold: 16766720, goldenrod: 14329120, gray: 8421504, green: 32768, greenyellow: 11403055, grey: 8421504, honeydew: 15794160, hotpink: 16738740, indianred: 13458524, indigo: 4915330, ivory: 16777200, khaki: 15787660, lavender: 15132410, lavenderblush: 16773365, lawngreen: 8190976, lemonchiffon: 16775885, lightblue: 11393254, lightcoral: 15761536, lightcyan: 14745599, lightgoldenrodyellow: 16448210, lightgray: 13882323, lightgreen: 9498256, lightgrey: 13882323, lightpink: 16758465, lightsalmon: 16752762, lightseagreen: 2142890, lightskyblue: 8900346, lightslategray: 7833753, lightslategrey: 7833753, lightsteelblue: 11584734, lightyellow: 16777184, lime: 65280, limegreen: 3329330, linen: 16445670, magenta: 16711935, maroon: 8388608, mediumaquamarine: 6737322, mediumblue: 205, mediumorchid: 12211667, mediumpurple: 9662683, mediumseagreen: 3978097, mediumslateblue: 8087790, mediumspringgreen: 64154, mediumturquoise: 4772300, mediumvioletred: 13047173, midnightblue: 1644912, mintcream: 16121850, mistyrose: 16770273, moccasin: 16770229, navajowhite: 16768685, navy: 128, oldlace: 16643558, olive: 8421376, olivedrab: 7048739, orange: 16753920, orangered: 16729344, orchid: 14315734, palegoldenrod: 15657130, palegreen: 10025880, paleturquoise: 11529966, palevioletred: 14381203, papayawhip: 16773077, peachpuff: 16767673, peru: 13468991, pink: 16761035, plum: 14524637, powderblue: 11591910, purple: 8388736, rebeccapurple: 6697881, red: 16711680, rosybrown: 12357519, royalblue: 4286945, saddlebrown: 9127187, salmon: 16416882, sandybrown: 16032864, seagreen: 3050327, seashell: 16774638, sienna: 10506797, silver: 12632256, skyblue: 8900331, slateblue: 6970061, slategray: 7372944, slategrey: 7372944, snow: 16775930, springgreen: 65407, steelblue: 4620980, tan: 13808780, teal: 32896, thistle: 14204888, tomato: 16737095, turquoise: 4251856, violet: 15631086, wheat: 16113331, white: 16777215, whitesmoke: 16119285, yellow: 16776960, yellowgreen: 10145074 };
function Oe() {
  return this.rgb().formatHex();
}
function Le() {
  return this.rgb().formatRgb();
}
function Vt(i) {
  var t, e;
  return i = (i + "").trim().toLowerCase(), (t = xn.exec(i)) ? (e = t[1].length, t = parseInt(t[1], 16), e === 6 ? Ge(t) : e === 3 ? new it(t >> 8 & 15 | t >> 4 & 240, t >> 4 & 15 | 240 & t, (15 & t) << 4 | 15 & t, 1) : e === 8 ? Xt(t >> 24 & 255, t >> 16 & 255, t >> 8 & 255, (255 & t) / 255) : e === 4 ? Xt(t >> 12 & 15 | t >> 8 & 240, t >> 8 & 15 | t >> 4 & 240, t >> 4 & 15 | 240 & t, ((15 & t) << 4 | 15 & t) / 255) : null) : (t = vn.exec(i)) ? new it(t[1], t[2], t[3], 1) : (t = bn.exec(i)) ? new it(255 * t[1] / 100, 255 * t[2] / 100, 255 * t[3] / 100, 1) : (t = kn.exec(i)) ? Xt(t[1], t[2], t[3], t[4]) : (t = Sn.exec(i)) ? Xt(255 * t[1] / 100, 255 * t[2] / 100, 255 * t[3] / 100, t[4]) : (t = wn.exec(i)) ? Be(t[1], t[2] / 100, t[3] / 100, 1) : (t = Mn.exec(i)) ? Be(t[1], t[2] / 100, t[3] / 100, t[4]) : Ee.hasOwnProperty(i) ? Ge(Ee[i]) : i === "transparent" ? new it(NaN, NaN, NaN, 0) : null;
}
function Ge(i) {
  return new it(i >> 16 & 255, i >> 8 & 255, 255 & i, 1);
}
function Xt(i, t, e, s) {
  return s <= 0 && (i = t = e = NaN), new it(i, t, e, s);
}
function qi(i, t, e, s) {
  return arguments.length === 1 ? function(n) {
    return n instanceof Gt || (n = Vt(n)), n ? new it((n = n.rgb()).r, n.g, n.b, n.opacity) : new it();
  }(i) : new it(i, t, e, s ?? 1);
}
function it(i, t, e, s) {
  this.r = +i, this.g = +t, this.b = +e, this.opacity = +s;
}
function Ie() {
  return "#" + zi(this.r) + zi(this.g) + zi(this.b);
}
function Ze() {
  var i = this.opacity;
  return ((i = isNaN(i) ? 1 : Math.max(0, Math.min(1, i))) === 1 ? "rgb(" : "rgba(") + Math.max(0, Math.min(255, Math.round(this.r) || 0)) + ", " + Math.max(0, Math.min(255, Math.round(this.g) || 0)) + ", " + Math.max(0, Math.min(255, Math.round(this.b) || 0)) + (i === 1 ? ")" : ", " + i + ")");
}
function zi(i) {
  return ((i = Math.max(0, Math.min(255, Math.round(i) || 0))) < 16 ? "0" : "") + i.toString(16);
}
function Be(i, t, e, s) {
  return s <= 0 ? i = t = e = NaN : e <= 0 || e >= 1 ? i = t = NaN : t <= 0 && (i = NaN), new rt(i, t, e, s);
}
function Ve(i) {
  if (i instanceof rt)
    return new rt(i.h, i.s, i.l, i.opacity);
  if (i instanceof Gt || (i = Vt(i)), !i)
    return new rt();
  if (i instanceof rt)
    return i;
  var t = (i = i.rgb()).r / 255, e = i.g / 255, s = i.b / 255, n = Math.min(t, e, s), r = Math.max(t, e, s), a = NaN, h = r - n, o = (r + n) / 2;
  return h ? (a = t === r ? (e - s) / h + 6 * (e < s) : e === r ? (s - t) / h + 2 : (t - e) / h + 4, h /= o < 0.5 ? r + n : 2 - r - n, a *= 60) : h = o > 0 && o < 1 ? 0 : a, new rt(a, h, o, i.opacity);
}
function rt(i, t, e, s) {
  this.h = +i, this.s = +t, this.l = +e, this.opacity = +s;
}
function Fi(i, t, e) {
  return 255 * (i < 60 ? t + (e - t) * i / 60 : i < 180 ? e : i < 240 ? t + (e - t) * (240 - i) / 60 : t);
}
Pi(Gt, Vt, { copy: function(i) {
  return Object.assign(new this.constructor(), this, i);
}, displayable: function() {
  return this.rgb().displayable();
}, hex: Oe, formatHex: Oe, formatHsl: function() {
  return Ve(this).formatHsl();
}, formatRgb: Le, toString: Le }), Pi(it, qi, Ce(Gt, { brighter: function(i) {
  return i = i == null ? Nt : Math.pow(Nt, i), new it(this.r * i, this.g * i, this.b * i, this.opacity);
}, darker: function(i) {
  return i = i == null ? Et : Math.pow(Et, i), new it(this.r * i, this.g * i, this.b * i, this.opacity);
}, rgb: function() {
  return this;
}, displayable: function() {
  return -0.5 <= this.r && this.r < 255.5 && -0.5 <= this.g && this.g < 255.5 && -0.5 <= this.b && this.b < 255.5 && 0 <= this.opacity && this.opacity <= 1;
}, hex: Ie, formatHex: Ie, formatRgb: Ze, toString: Ze })), Pi(rt, function(i, t, e, s) {
  return arguments.length === 1 ? Ve(i) : new rt(i, t, e, s ?? 1);
}, Ce(Gt, { brighter: function(i) {
  return i = i == null ? Nt : Math.pow(Nt, i), new rt(this.h, this.s, this.l * i, this.opacity);
}, darker: function(i) {
  return i = i == null ? Et : Math.pow(Et, i), new rt(this.h, this.s, this.l * i, this.opacity);
}, rgb: function() {
  var i = this.h % 360 + 360 * (this.h < 0), t = isNaN(i) || isNaN(this.s) ? 0 : this.s, e = this.l, s = e + (e < 0.5 ? e : 1 - e) * t, n = 2 * e - s;
  return new it(Fi(i >= 240 ? i - 240 : i + 120, n, s), Fi(i, n, s), Fi(i < 120 ? i + 240 : i - 120, n, s), this.opacity);
}, displayable: function() {
  return (0 <= this.s && this.s <= 1 || isNaN(this.s)) && 0 <= this.l && this.l <= 1 && 0 <= this.opacity && this.opacity <= 1;
}, formatHsl: function() {
  var i = this.opacity;
  return ((i = isNaN(i) ? 1 : Math.max(0, Math.min(1, i))) === 1 ? "hsl(" : "hsla(") + (this.h || 0) + ", " + 100 * (this.s || 0) + "%, " + 100 * (this.l || 0) + "%" + (i === 1 ? ")" : ", " + i + ")");
} }));
const re = (i) => () => i;
function Rn(i) {
  return (i = +i) == 1 ? Ms : function(t, e) {
    return e - t ? function(s, n, r) {
      return s = Math.pow(s, r), n = Math.pow(n, r) - s, r = 1 / r, function(a) {
        return Math.pow(s + a * n, r);
      };
    }(t, e, i) : re(isNaN(t) ? e : t);
  };
}
function Ms(i, t) {
  var e = t - i;
  return e ? function(s, n) {
    return function(r) {
      return s + r * n;
    };
  }(i, e) : re(isNaN(i) ? t : i);
}
const Ye = function i(t) {
  var e = Rn(t);
  function s(n, r) {
    var a = e((n = qi(n)).r, (r = qi(r)).r), h = e(n.g, r.g), o = e(n.b, r.b), l = Ms(n.opacity, r.opacity);
    return function(u) {
      return n.r = a(u), n.g = h(u), n.b = o(u), n.opacity = l(u), n + "";
    };
  }
  return s.gamma = i, s;
}(1);
function Pn(i, t) {
  t || (t = []);
  var e, s = i ? Math.min(t.length, i.length) : 0, n = t.slice();
  return function(r) {
    for (e = 0; e < s; ++e)
      n[e] = i[e] * (1 - r) + t[e] * r;
    return n;
  };
}
function zn(i, t) {
  var e, s = t ? t.length : 0, n = i ? Math.min(s, i.length) : 0, r = new Array(n), a = new Array(s);
  for (e = 0; e < n; ++e)
    r[e] = he(i[e], t[e]);
  for (; e < s; ++e)
    a[e] = t[e];
  return function(h) {
    for (e = 0; e < n; ++e)
      a[e] = r[e](h);
    return a;
  };
}
function Fn(i, t) {
  var e = /* @__PURE__ */ new Date();
  return i = +i, t = +t, function(s) {
    return e.setTime(i * (1 - s) + t * s), e;
  };
}
function ri(i, t) {
  return i = +i, t = +t, function(e) {
    return i * (1 - e) + t * e;
  };
}
function Wn(i, t) {
  var e, s = {}, n = {};
  for (e in i !== null && typeof i == "object" || (i = {}), t !== null && typeof t == "object" || (t = {}), t)
    e in i ? s[e] = he(i[e], t[e]) : n[e] = t[e];
  return function(r) {
    for (e in s)
      n[e] = s[e](r);
    return n;
  };
}
var Ni = /[-+]?(?:\d+\.?\d*|\.?\d+)(?:[eE][-+]?\d+)?/g, Wi = new RegExp(Ni.source, "g");
function jn(i, t) {
  var e, s, n, r = Ni.lastIndex = Wi.lastIndex = 0, a = -1, h = [], o = [];
  for (i += "", t += ""; (e = Ni.exec(i)) && (s = Wi.exec(t)); )
    (n = s.index) > r && (n = t.slice(r, n), h[a] ? h[a] += n : h[++a] = n), (e = e[0]) === (s = s[0]) ? h[a] ? h[a] += s : h[++a] = s : (h[++a] = null, o.push({ i: a, x: ri(e, s) })), r = Wi.lastIndex;
  return r < t.length && (n = t.slice(r), h[a] ? h[a] += n : h[++a] = n), h.length < 2 ? o[0] ? function(l) {
    return function(u) {
      return l(u) + "";
    };
  }(o[0].x) : function(l) {
    return function() {
      return l;
    };
  }(t) : (t = o.length, function(l) {
    for (var u, d = 0; d < t; ++d)
      h[(u = o[d]).i] = u.x(l);
    return h.join("");
  });
}
function he(i, t) {
  var e, s = typeof t;
  return t == null || s === "boolean" ? re(t) : (s === "number" ? ri : s === "string" ? (e = Vt(t)) ? (t = e, Ye) : jn : t instanceof Vt ? Ye : t instanceof Date ? Fn : function(n) {
    return ArrayBuffer.isView(n) && !(n instanceof DataView);
  }(t) ? Pn : Array.isArray(t) ? zn : typeof t.valueOf != "function" && typeof t.toString != "function" || isNaN(t) ? Wn : ri)(i, t);
}
function Cn(i, t) {
  return i = +i, t = +t, function(e) {
    return Math.round(i * (1 - e) + t * e);
  };
}
function En(i) {
  return +i;
}
var He = [0, 1];
function Mt(i) {
  return i;
}
function Xi(i, t) {
  return (t -= i = +i) ? function(e) {
    return (e - i) / t;
  } : function(e) {
    return function() {
      return e;
    };
  }(isNaN(t) ? NaN : 0.5);
}
function On(i, t, e) {
  var s = i[0], n = i[1], r = t[0], a = t[1];
  return n < s ? (s = Xi(n, s), r = e(a, r)) : (s = Xi(s, n), r = e(r, a)), function(h) {
    return r(s(h));
  };
}
function Ln(i, t, e) {
  var s = Math.min(i.length, t.length) - 1, n = new Array(s), r = new Array(s), a = -1;
  for (i[s] < i[0] && (i = i.slice().reverse(), t = t.slice().reverse()); ++a < s; )
    n[a] = Xi(i[a], i[a + 1]), r[a] = e(t[a], t[a + 1]);
  return function(h) {
    var o = As(i, h, 1, s) - 1;
    return r[o](n[o](h));
  };
}
function Gn() {
  var i, t, e, s, n, r, a = He, h = He, o = he, l = Mt;
  function u() {
    var c = Math.min(a.length, h.length);
    return l !== Mt && (l = function(g, p) {
      var f;
      return g > p && (f = g, g = p, p = f), function(y) {
        return Math.max(g, Math.min(p, y));
      };
    }(a[0], a[c - 1])), s = c > 2 ? Ln : On, n = r = null, d;
  }
  function d(c) {
    return c == null || isNaN(c = +c) ? e : (n || (n = s(a.map(i), h, o)))(i(l(c)));
  }
  return d.invert = function(c) {
    return l(t((r || (r = s(h, a.map(i), ri)))(c)));
  }, d.domain = function(c) {
    return arguments.length ? (a = Array.from(c, En), u()) : a.slice();
  }, d.range = function(c) {
    return arguments.length ? (h = Array.from(c), u()) : h.slice();
  }, d.rangeRound = function(c) {
    return h = Array.from(c), o = Cn, u();
  }, d.clamp = function(c) {
    return arguments.length ? (l = !!c || Mt, u()) : l !== Mt;
  }, d.interpolate = function(c) {
    return arguments.length ? (o = c, u()) : o;
  }, d.unknown = function(c) {
    return arguments.length ? (e = c, d) : e;
  }, function(c, g) {
    return i = c, t = g, u();
  };
}
function hi(i, t) {
  if ((e = (i = t ? i.toExponential(t - 1) : i.toExponential()).indexOf("e")) < 0)
    return null;
  var e, s = i.slice(0, e);
  return [s.length > 1 ? s[0] + s.slice(2) : s, +i.slice(e + 1)];
}
function wt(i) {
  return (i = hi(Math.abs(i))) ? i[1] : NaN;
}
var Rs, In = /^(?:(.)?([<>=^]))?([+\-( ])?([$#])?(0)?(\d+)?(,)?(\.\d+)?(~)?([a-z%])?$/i;
function ai(i) {
  if (!(t = In.exec(i)))
    throw new Error("invalid format: " + i);
  var t;
  return new Ki({ fill: t[1], align: t[2], sign: t[3], symbol: t[4], zero: t[5], width: t[6], comma: t[7], precision: t[8] && t[8].slice(1), trim: t[9], type: t[10] });
}
function Ki(i) {
  this.fill = i.fill === void 0 ? " " : i.fill + "", this.align = i.align === void 0 ? ">" : i.align + "", this.sign = i.sign === void 0 ? "-" : i.sign + "", this.symbol = i.symbol === void 0 ? "" : i.symbol + "", this.zero = !!i.zero, this.width = i.width === void 0 ? void 0 : +i.width, this.comma = !!i.comma, this.precision = i.precision === void 0 ? void 0 : +i.precision, this.trim = !!i.trim, this.type = i.type === void 0 ? "" : i.type + "";
}
function De(i, t) {
  var e = hi(i, t);
  if (!e)
    return i + "";
  var s = e[0], n = e[1];
  return n < 0 ? "0." + new Array(-n).join("0") + s : s.length > n + 1 ? s.slice(0, n + 1) + "." + s.slice(n + 1) : s + new Array(n - s.length + 2).join("0");
}
ai.prototype = Ki.prototype, Ki.prototype.toString = function() {
  return this.fill + this.align + this.sign + this.symbol + (this.zero ? "0" : "") + (this.width === void 0 ? "" : Math.max(1, 0 | this.width)) + (this.comma ? "," : "") + (this.precision === void 0 ? "" : "." + Math.max(0, 0 | this.precision)) + (this.trim ? "~" : "") + this.type;
};
const Te = { "%": (i, t) => (100 * i).toFixed(t), b: (i) => Math.round(i).toString(2), c: (i) => i + "", d: function(i) {
  return Math.abs(i = Math.round(i)) >= 1e21 ? i.toLocaleString("en").replace(/,/g, "") : i.toString(10);
}, e: (i, t) => i.toExponential(t), f: (i, t) => i.toFixed(t), g: (i, t) => i.toPrecision(t), o: (i) => Math.round(i).toString(8), p: (i, t) => De(100 * i, t), r: De, s: function(i, t) {
  var e = hi(i, t);
  if (!e)
    return i + "";
  var s = e[0], n = e[1], r = n - (Rs = 3 * Math.max(-8, Math.min(8, Math.floor(n / 3)))) + 1, a = s.length;
  return r === a ? s : r > a ? s + new Array(r - a + 1).join("0") : r > 0 ? s.slice(0, r) + "." + s.slice(r) : "0." + new Array(1 - r).join("0") + hi(i, Math.max(0, t + r - 1))[0];
}, X: (i) => Math.round(i).toString(16).toUpperCase(), x: (i) => Math.round(i).toString(16) };
function qe(i) {
  return i;
}
var ji, Ps, zs, Ne = Array.prototype.map, Xe = ["y", "z", "a", "f", "p", "n", "µ", "m", "", "k", "M", "G", "T", "P", "E", "Z", "Y"];
function Zn(i) {
  var t, e, s = i.grouping === void 0 || i.thousands === void 0 ? qe : (t = Ne.call(i.grouping, Number), e = i.thousands + "", function(c, g) {
    for (var p = c.length, f = [], y = 0, m = t[0], A = 0; p > 0 && m > 0 && (A + m + 1 > g && (m = Math.max(1, g - A)), f.push(c.substring(p -= m, p + m)), !((A += m + 1) > g)); )
      m = t[y = (y + 1) % t.length];
    return f.reverse().join(e);
  }), n = i.currency === void 0 ? "" : i.currency[0] + "", r = i.currency === void 0 ? "" : i.currency[1] + "", a = i.decimal === void 0 ? "." : i.decimal + "", h = i.numerals === void 0 ? qe : function(c) {
    return function(g) {
      return g.replace(/[0-9]/g, function(p) {
        return c[+p];
      });
    };
  }(Ne.call(i.numerals, String)), o = i.percent === void 0 ? "%" : i.percent + "", l = i.minus === void 0 ? "−" : i.minus + "", u = i.nan === void 0 ? "NaN" : i.nan + "";
  function d(c) {
    var g = (c = ai(c)).fill, p = c.align, f = c.sign, y = c.symbol, m = c.zero, A = c.width, b = c.comma, S = c.precision, w = c.trim, v = c.type;
    v === "n" ? (b = !0, v = "g") : Te[v] || (S === void 0 && (S = 12), w = !0, v = "g"), (m || g === "0" && p === "=") && (m = !0, g = "0", p = "=");
    var M = y === "$" ? n : y === "#" && /[boxX]/.test(v) ? "0" + v.toLowerCase() : "", j = y === "$" ? r : /[%p]/.test(v) ? o : "", C = Te[v], P = /[defgprs%]/.test(v);
    function F(k) {
      var R, O, I, L = M, E = j;
      if (v === "c")
        E = C(k) + E, k = "";
      else {
        var T = (k = +k) < 0 || 1 / k < 0;
        if (k = isNaN(k) ? u : C(Math.abs(k), S), w && (k = function(H) {
          t:
            for (var et, gt = H.length, B = 1, V = -1; B < gt; ++B)
              switch (H[B]) {
                case ".":
                  V = et = B;
                  break;
                case "0":
                  V === 0 && (V = B), et = B;
                  break;
                default:
                  if (!+H[B])
                    break t;
                  V > 0 && (V = 0);
              }
          return V > 0 ? H.slice(0, V) + H.slice(et + 1) : H;
        }(k)), T && +k == 0 && f !== "+" && (T = !1), L = (T ? f === "(" ? f : l : f === "-" || f === "(" ? "" : f) + L, E = (v === "s" ? Xe[8 + Rs / 3] : "") + E + (T && f === "(" ? ")" : ""), P) {
          for (R = -1, O = k.length; ++R < O; )
            if (48 > (I = k.charCodeAt(R)) || I > 57) {
              E = (I === 46 ? a + k.slice(R + 1) : k.slice(R)) + E, k = k.slice(0, R);
              break;
            }
        }
      }
      b && !m && (k = s(k, 1 / 0));
      var q = L.length + k.length + E.length, G = q < A ? new Array(A - q + 1).join(g) : "";
      switch (b && m && (k = s(G + k, G.length ? A - E.length : 1 / 0), G = ""), p) {
        case "<":
          k = L + k + E + G;
          break;
        case "=":
          k = L + G + k + E;
          break;
        case "^":
          k = G.slice(0, q = G.length >> 1) + L + k + E + G.slice(q);
          break;
        default:
          k = G + L + k + E;
      }
      return h(k);
    }
    return S = S === void 0 ? 6 : /[gprs]/.test(v) ? Math.max(1, Math.min(21, S)) : Math.max(0, Math.min(20, S)), F.toString = function() {
      return c + "";
    }, F;
  }
  return { format: d, formatPrefix: function(c, g) {
    var p = d(((c = ai(c)).type = "f", c)), f = 3 * Math.max(-8, Math.min(8, Math.floor(wt(g) / 3))), y = Math.pow(10, -f), m = Xe[8 + f / 3];
    return function(A) {
      return p(y * A) + m;
    };
  } };
}
function Bn(i, t, e, s) {
  var n, r = function(h, o, l) {
    var u = Math.abs(o - h) / Math.max(0, l), d = Math.pow(10, Math.floor(Math.log(u) / Math.LN10)), c = u / d;
    return c >= Vi ? d *= 10 : c >= Yi ? d *= 5 : c >= Hi && (d *= 2), o < h ? -d : d;
  }(i, t, e);
  switch ((s = ai(s ?? ",f")).type) {
    case "s":
      var a = Math.max(Math.abs(i), Math.abs(t));
      return s.precision != null || isNaN(n = function(h, o) {
        return Math.max(0, 3 * Math.max(-8, Math.min(8, Math.floor(wt(o) / 3))) - wt(Math.abs(h)));
      }(r, a)) || (s.precision = n), zs(s, a);
    case "":
    case "e":
    case "g":
    case "p":
    case "r":
      s.precision != null || isNaN(n = function(h, o) {
        return h = Math.abs(h), o = Math.abs(o) - h, Math.max(0, wt(o) - wt(h)) + 1;
      }(r, Math.max(Math.abs(i), Math.abs(t)))) || (s.precision = n - (s.type === "e"));
      break;
    case "f":
    case "%":
      s.precision != null || isNaN(n = function(h) {
        return Math.max(0, -wt(Math.abs(h)));
      }(r)) || (s.precision = n - 2 * (s.type === "%"));
  }
  return Ps(s);
}
function Vn(i) {
  var t = i.domain;
  return i.ticks = function(e) {
    var s = t();
    return function(n, r, a) {
      var h, o, l, u, d = -1;
      if (a = +a, (n = +n) == (r = +r) && a > 0)
        return [n];
      if ((h = r < n) && (o = n, n = r, r = o), (u = ve(n, r, a)) === 0 || !isFinite(u))
        return [];
      if (u > 0) {
        let c = Math.round(n / u), g = Math.round(r / u);
        for (c * u < n && ++c, g * u > r && --g, l = new Array(o = g - c + 1); ++d < o; )
          l[d] = (c + d) * u;
      } else {
        u = -u;
        let c = Math.round(n * u), g = Math.round(r * u);
        for (c / u < n && ++c, g / u > r && --g, l = new Array(o = g - c + 1); ++d < o; )
          l[d] = (c + d) / u;
      }
      return h && l.reverse(), l;
    }(s[0], s[s.length - 1], e ?? 10);
  }, i.tickFormat = function(e, s) {
    var n = t();
    return Bn(n[0], n[n.length - 1], e ?? 10, s);
  }, i.nice = function(e) {
    e == null && (e = 10);
    var s, n, r = t(), a = 0, h = r.length - 1, o = r[a], l = r[h], u = 10;
    for (l < o && (n = o, o = l, l = n, n = a, a = h, h = n); u-- > 0; ) {
      if ((n = ve(o, l, e)) === s)
        return r[a] = o, r[h] = l, t(r);
      if (n > 0)
        o = Math.floor(o / n) * n, l = Math.ceil(l / n) * n;
      else {
        if (!(n < 0))
          break;
        o = Math.ceil(o * n) / n, l = Math.floor(l * n) / n;
      }
      s = n;
    }
    return i;
  }, i;
}
function Z() {
  var i = Gn()(Mt, Mt);
  return i.copy = function() {
    return t = i, Z().domain(t.domain()).range(t.range()).interpolate(t.interpolate()).clamp(t.clamp()).unknown(t.unknown());
    var t;
  }, ne.apply(i, arguments), Vn(i);
}
(function(i) {
  ji = Zn(i), Ps = ji.format, zs = ji.formatPrefix;
})({ thousands: ",", grouping: [3], currency: ["$", ""] });
var Qi = "http://www.w3.org/1999/xhtml";
const Ke = { svg: "http://www.w3.org/2000/svg", xhtml: Qi, xlink: "http://www.w3.org/1999/xlink", xml: "http://www.w3.org/XML/1998/namespace", xmlns: "http://www.w3.org/2000/xmlns/" };
function Fs(i) {
  var t = i += "", e = t.indexOf(":");
  return e >= 0 && (t = i.slice(0, e)) !== "xmlns" && (i = i.slice(e + 1)), Ke.hasOwnProperty(t) ? { space: Ke[t], local: i } : i;
}
function Yn(i) {
  return function() {
    var t = this.ownerDocument, e = this.namespaceURI;
    return e === Qi && t.documentElement.namespaceURI === Qi ? t.createElement(i) : t.createElementNS(e, i);
  };
}
function Hn(i) {
  return function() {
    return this.ownerDocument.createElementNS(i.space, i.local);
  };
}
function Qe(i) {
  var t = Fs(i);
  return (t.local ? Hn : Yn)(t);
}
function Dn() {
}
function Ue(i) {
  return i == null ? Dn : function() {
    return this.querySelector(i);
  };
}
function Tn() {
  return [];
}
function Je(i) {
  return new Array(i.length);
}
function oi(i, t) {
  this.ownerDocument = i.ownerDocument, this.namespaceURI = i.namespaceURI, this._next = null, this._parent = i, this.__data__ = t;
}
oi.prototype = { constructor: oi, appendChild: function(i) {
  return this._parent.insertBefore(i, this._next);
}, insertBefore: function(i, t) {
  return this._parent.insertBefore(i, t);
}, querySelector: function(i) {
  return this._parent.querySelector(i);
}, querySelectorAll: function(i) {
  return this._parent.querySelectorAll(i);
} };
function qn(i, t, e, s, n, r) {
  for (var a, h = 0, o = t.length, l = r.length; h < l; ++h)
    (a = t[h]) ? (a.__data__ = r[h], s[h] = a) : e[h] = new oi(i, r[h]);
  for (; h < o; ++h)
    (a = t[h]) && (n[h] = a);
}
function Nn(i, t, e, s, n, r, a) {
  var h, o, l, u = {}, d = t.length, c = r.length, g = new Array(d);
  for (h = 0; h < d; ++h)
    (o = t[h]) && (g[h] = l = "$" + a.call(o, o.__data__, h, t), l in u ? n[h] = o : u[l] = o);
  for (h = 0; h < c; ++h)
    (o = u[l = "$" + a.call(i, r[h], h, r)]) ? (s[h] = o, o.__data__ = r[h], u[l] = null) : e[h] = new oi(i, r[h]);
  for (h = 0; h < d; ++h)
    (o = t[h]) && u[g[h]] === o && (n[h] = o);
}
function Xn(i, t) {
  return i < t ? -1 : i > t ? 1 : i >= t ? 0 : NaN;
}
function Kn(i) {
  return function() {
    this.removeAttribute(i);
  };
}
function Qn(i) {
  return function() {
    this.removeAttributeNS(i.space, i.local);
  };
}
function Un(i, t) {
  return function() {
    this.setAttribute(i, t);
  };
}
function Jn(i, t) {
  return function() {
    this.setAttributeNS(i.space, i.local, t);
  };
}
function _n(i, t) {
  return function() {
    var e = t.apply(this, arguments);
    e == null ? this.removeAttribute(i) : this.setAttribute(i, e);
  };
}
function $n(i, t) {
  return function() {
    var e = t.apply(this, arguments);
    e == null ? this.removeAttributeNS(i.space, i.local) : this.setAttributeNS(i.space, i.local, e);
  };
}
function Ws(i) {
  return i.ownerDocument && i.ownerDocument.defaultView || i.document && i || i.defaultView;
}
function tr(i) {
  return function() {
    this.style.removeProperty(i);
  };
}
function ir(i, t, e) {
  return function() {
    this.style.setProperty(i, t, e);
  };
}
function er(i, t, e) {
  return function() {
    var s = t.apply(this, arguments);
    s == null ? this.style.removeProperty(i) : this.style.setProperty(i, s, e);
  };
}
function sr(i) {
  return function() {
    delete this[i];
  };
}
function nr(i, t) {
  return function() {
    this[i] = t;
  };
}
function rr(i, t) {
  return function() {
    var e = t.apply(this, arguments);
    e == null ? delete this[i] : this[i] = e;
  };
}
function js(i) {
  return i.trim().split(/^|\s+/);
}
function ae(i) {
  return i.classList || new Cs(i);
}
function Cs(i) {
  this._node = i, this._names = js(i.getAttribute("class") || "");
}
function Es(i, t) {
  for (var e = ae(i), s = -1, n = t.length; ++s < n; )
    e.add(t[s]);
}
function Os(i, t) {
  for (var e = ae(i), s = -1, n = t.length; ++s < n; )
    e.remove(t[s]);
}
function hr(i) {
  return function() {
    Es(this, i);
  };
}
function ar(i) {
  return function() {
    Os(this, i);
  };
}
function or(i, t) {
  return function() {
    (t.apply(this, arguments) ? Es : Os)(this, i);
  };
}
function lr() {
  this.textContent = "";
}
function ur(i) {
  return function() {
    this.textContent = i;
  };
}
function cr(i) {
  return function() {
    var t = i.apply(this, arguments);
    this.textContent = t ?? "";
  };
}
function dr() {
  this.innerHTML = "";
}
function gr(i) {
  return function() {
    this.innerHTML = i;
  };
}
function fr(i) {
  return function() {
    var t = i.apply(this, arguments);
    this.innerHTML = t ?? "";
  };
}
function pr() {
  this.nextSibling && this.parentNode.appendChild(this);
}
function mr() {
  this.previousSibling && this.parentNode.insertBefore(this, this.parentNode.firstChild);
}
function Ar() {
  return null;
}
function yr() {
  var i = this.parentNode;
  i && i.removeChild(this);
}
function xr() {
  var i = this.cloneNode(!1), t = this.parentNode;
  return t ? t.insertBefore(i, this.nextSibling) : i;
}
function vr() {
  var i = this.cloneNode(!0), t = this.parentNode;
  return t ? t.insertBefore(i, this.nextSibling) : i;
}
Cs.prototype = { add: function(i) {
  this._names.indexOf(i) < 0 && (this._names.push(i), this._node.setAttribute("class", this._names.join(" ")));
}, remove: function(i) {
  var t = this._names.indexOf(i);
  t >= 0 && (this._names.splice(t, 1), this._node.setAttribute("class", this._names.join(" ")));
}, contains: function(i) {
  return this._names.indexOf(i) >= 0;
} };
var Ls = {}, $t = null;
typeof document < "u" && ("onmouseenter" in document.documentElement || (Ls = { mouseenter: "mouseover", mouseleave: "mouseout" }));
function br(i, t, e) {
  return i = Gs(i, t, e), function(s) {
    var n = s.relatedTarget;
    n && (n === this || 8 & n.compareDocumentPosition(this)) || i.call(this, s);
  };
}
function Gs(i, t, e) {
  return function(s) {
    var n = $t;
    $t = s;
    try {
      i.call(this, this.__data__, t, e);
    } finally {
      $t = n;
    }
  };
}
function kr(i) {
  return function() {
    var t = this.__on;
    if (t) {
      for (var e, s = 0, n = -1, r = t.length; s < r; ++s)
        e = t[s], i.type && e.type !== i.type || e.name !== i.name ? t[++n] = e : this.removeEventListener(e.type, e.listener, e.capture);
      ++n ? t.length = n : delete this.__on;
    }
  };
}
function Sr(i, t, e) {
  var s = Ls.hasOwnProperty(i.type) ? br : Gs;
  return function(n, r, a) {
    var h, o = this.__on, l = s(t, r, a);
    if (o) {
      for (var u = 0, d = o.length; u < d; ++u)
        if ((h = o[u]).type === i.type && h.name === i.name)
          return this.removeEventListener(h.type, h.listener, h.capture), this.addEventListener(h.type, h.listener = l, h.capture = e), void (h.value = t);
    }
    this.addEventListener(i.type, l, e), h = { type: i.type, name: i.name, value: t, listener: l, capture: e }, o ? o.push(h) : this.__on = [h];
  };
}
function Is(i, t, e) {
  var s = Ws(i), n = s.CustomEvent;
  typeof n == "function" ? n = new n(t, e) : (n = s.document.createEvent("Event"), e ? (n.initEvent(t, e.bubbles, e.cancelable), n.detail = e.detail) : n.initEvent(t, !1, !1)), i.dispatchEvent(n);
}
function wr(i, t) {
  return function() {
    return Is(this, i, t);
  };
}
function Mr(i, t) {
  return function() {
    return Is(this, i, t.apply(this, arguments));
  };
}
var Zs = [null];
function $(i, t) {
  this._groups = i, this._parents = t;
}
function x(i) {
  return typeof i == "string" ? new $([[document.querySelector(i)]], [document.documentElement]) : new $([[i]], Zs);
}
function Rr() {
  for (var i, t = $t; i = t.sourceEvent; )
    t = i;
  return t;
}
function kt(i) {
  var t = Rr();
  return t.changedTouches && (t = t.changedTouches[0]), function(e, s) {
    var n = e.ownerSVGElement || e;
    if (n.createSVGPoint) {
      var r = n.createSVGPoint();
      return r.x = s.clientX, r.y = s.clientY, [(r = r.matrixTransform(e.getScreenCTM().inverse())).x, r.y];
    }
    var a = e.getBoundingClientRect();
    return [s.clientX - a.left - e.clientLeft, s.clientY - a.top - e.clientTop];
  }(i, t);
}
function z(i) {
  return typeof i == "string" ? new $([document.querySelectorAll(i)], [document.documentElement]) : new $([i ?? []], Zs);
}
function Ci(i, t, e) {
  if (i && i.length) {
    const [s, n] = t, r = Math.PI / 180 * e, a = Math.cos(r), h = Math.sin(r);
    for (const o of i) {
      const [l, u] = o;
      o[0] = (l - s) * a - (u - n) * h + s, o[1] = (l - s) * h + (u - n) * a + n;
    }
  }
}
function Pr(i, t) {
  return i[0] === t[0] && i[1] === t[1];
}
function zr(i, t, e, s = 1) {
  const n = e, r = Math.max(t, 0.1), a = i[0] && i[0][0] && typeof i[0][0] == "number" ? [i] : i, h = [0, 0];
  if (n)
    for (const l of a)
      Ci(l, h, n);
  const o = function(l, u, d) {
    const c = [];
    for (const A of l) {
      const b = [...A];
      Pr(b[0], b[b.length - 1]) || b.push([b[0][0], b[0][1]]), b.length > 2 && c.push(b);
    }
    const g = [];
    u = Math.max(u, 0.1);
    const p = [];
    for (const A of c)
      for (let b = 0; b < A.length - 1; b++) {
        const S = A[b], w = A[b + 1];
        if (S[1] !== w[1]) {
          const v = Math.min(S[1], w[1]);
          p.push({ ymin: v, ymax: Math.max(S[1], w[1]), x: v === S[1] ? S[0] : w[0], islope: (w[0] - S[0]) / (w[1] - S[1]) });
        }
      }
    if (p.sort((A, b) => A.ymin < b.ymin ? -1 : A.ymin > b.ymin ? 1 : A.x < b.x ? -1 : A.x > b.x ? 1 : A.ymax === b.ymax ? 0 : (A.ymax - b.ymax) / Math.abs(A.ymax - b.ymax)), !p.length)
      return g;
    let f = [], y = p[0].ymin, m = 0;
    for (; f.length || p.length; ) {
      if (p.length) {
        let A = -1;
        for (let b = 0; b < p.length && !(p[b].ymin > y); b++)
          A = b;
        p.splice(0, A + 1).forEach((b) => {
          f.push({ s: y, edge: b });
        });
      }
      if (f = f.filter((A) => !(A.edge.ymax <= y)), f.sort((A, b) => A.edge.x === b.edge.x ? 0 : (A.edge.x - b.edge.x) / Math.abs(A.edge.x - b.edge.x)), (d !== 1 || m % u == 0) && f.length > 1)
        for (let A = 0; A < f.length; A += 2) {
          const b = A + 1;
          if (b >= f.length)
            break;
          const S = f[A].edge, w = f[b].edge;
          g.push([[Math.round(S.x), y], [Math.round(w.x), y]]);
        }
      y += d, f.forEach((A) => {
        A.edge.x = A.edge.x + d * A.edge.islope;
      }), m++;
    }
    return g;
  }(a, r, s);
  if (n) {
    for (const l of a)
      Ci(l, h, -n);
    (function(l, u, d) {
      const c = [];
      l.forEach((g) => c.push(...g)), Ci(c, u, d);
    })(o, h, -n);
  }
  return o;
}
function qt(i, t) {
  var e;
  const s = t.hachureAngle + 90;
  let n = t.hachureGap;
  n < 0 && (n = 4 * t.strokeWidth), n = Math.round(Math.max(n, 0.1));
  let r = 1;
  return t.roughness >= 1 && (((e = t.randomizer) === null || e === void 0 ? void 0 : e.next()) || Math.random()) > 0.7 && (r = n), zr(i, n, s, r || 1);
}
$.prototype = { constructor: $, select: function(i) {
  typeof i != "function" && (i = Ue(i));
  for (var t = this._groups, e = t.length, s = new Array(e), n = 0; n < e; ++n)
    for (var r, a, h = t[n], o = h.length, l = s[n] = new Array(o), u = 0; u < o; ++u)
      (r = h[u]) && (a = i.call(r, r.__data__, u, h)) && ("__data__" in r && (a.__data__ = r.__data__), l[u] = a);
  return new $(s, this._parents);
}, selectAll: function(i) {
  typeof i != "function" && (i = function(u) {
    return u == null ? Tn : function() {
      return this.querySelectorAll(u);
    };
  }(i));
  for (var t = this._groups, e = t.length, s = [], n = [], r = 0; r < e; ++r)
    for (var a, h = t[r], o = h.length, l = 0; l < o; ++l)
      (a = h[l]) && (s.push(i.call(a, a.__data__, l, h)), n.push(a));
  return new $(s, n);
}, filter: function(i) {
  typeof i != "function" && (i = function(u) {
    return function() {
      return this.matches(u);
    };
  }(i));
  for (var t = this._groups, e = t.length, s = new Array(e), n = 0; n < e; ++n)
    for (var r, a = t[n], h = a.length, o = s[n] = [], l = 0; l < h; ++l)
      (r = a[l]) && i.call(r, r.__data__, l, a) && o.push(r);
  return new $(s, this._parents);
}, data: function(i, t) {
  if (!i)
    return g = new Array(this.size()), l = -1, this.each(function(w) {
      g[++l] = w;
    }), g;
  var e = t ? Nn : qn, s = this._parents, n = this._groups;
  typeof i != "function" && (i = function(w) {
    return function() {
      return w;
    };
  }(i));
  for (var r = n.length, a = new Array(r), h = new Array(r), o = new Array(r), l = 0; l < r; ++l) {
    var u = s[l], d = n[l], c = d.length, g = i.call(u, u && u.__data__, l, s), p = g.length, f = h[l] = new Array(p), y = a[l] = new Array(p);
    e(u, d, f, y, o[l] = new Array(c), g, t);
    for (var m, A, b = 0, S = 0; b < p; ++b)
      if (m = f[b]) {
        for (b >= S && (S = b + 1); !(A = y[S]) && ++S < p; )
          ;
        m._next = A || null;
      }
  }
  return (a = new $(a, s))._enter = h, a._exit = o, a;
}, enter: function() {
  return new $(this._enter || this._groups.map(Je), this._parents);
}, exit: function() {
  return new $(this._exit || this._groups.map(Je), this._parents);
}, join: function(i, t, e) {
  var s = this.enter(), n = this, r = this.exit();
  return s = typeof i == "function" ? i(s) : s.append(i + ""), t != null && (n = t(n)), e == null ? r.remove() : e(r), s && n ? s.merge(n).order() : n;
}, merge: function(i) {
  for (var t = this._groups, e = i._groups, s = t.length, n = e.length, r = Math.min(s, n), a = new Array(s), h = 0; h < r; ++h)
    for (var o, l = t[h], u = e[h], d = l.length, c = a[h] = new Array(d), g = 0; g < d; ++g)
      (o = l[g] || u[g]) && (c[g] = o);
  for (; h < s; ++h)
    a[h] = t[h];
  return new $(a, this._parents);
}, order: function() {
  for (var i = this._groups, t = -1, e = i.length; ++t < e; )
    for (var s, n = i[t], r = n.length - 1, a = n[r]; --r >= 0; )
      (s = n[r]) && (a && 4 ^ s.compareDocumentPosition(a) && a.parentNode.insertBefore(s, a), a = s);
  return this;
}, sort: function(i) {
  function t(d, c) {
    return d && c ? i(d.__data__, c.__data__) : !d - !c;
  }
  i || (i = Xn);
  for (var e = this._groups, s = e.length, n = new Array(s), r = 0; r < s; ++r) {
    for (var a, h = e[r], o = h.length, l = n[r] = new Array(o), u = 0; u < o; ++u)
      (a = h[u]) && (l[u] = a);
    l.sort(t);
  }
  return new $(n, this._parents).order();
}, call: function() {
  var i = arguments[0];
  return arguments[0] = this, i.apply(null, arguments), this;
}, nodes: function() {
  var i = new Array(this.size()), t = -1;
  return this.each(function() {
    i[++t] = this;
  }), i;
}, node: function() {
  for (var i = this._groups, t = 0, e = i.length; t < e; ++t)
    for (var s = i[t], n = 0, r = s.length; n < r; ++n) {
      var a = s[n];
      if (a)
        return a;
    }
  return null;
}, size: function() {
  var i = 0;
  return this.each(function() {
    ++i;
  }), i;
}, empty: function() {
  return !this.node();
}, each: function(i) {
  for (var t = this._groups, e = 0, s = t.length; e < s; ++e)
    for (var n, r = t[e], a = 0, h = r.length; a < h; ++a)
      (n = r[a]) && i.call(n, n.__data__, a, r);
  return this;
}, attr: function(i, t) {
  var e = Fs(i);
  if (arguments.length < 2) {
    var s = this.node();
    return e.local ? s.getAttributeNS(e.space, e.local) : s.getAttribute(e);
  }
  return this.each((t == null ? e.local ? Qn : Kn : typeof t == "function" ? e.local ? $n : _n : e.local ? Jn : Un)(e, t));
}, style: function(i, t, e) {
  return arguments.length > 1 ? this.each((t == null ? tr : typeof t == "function" ? er : ir)(i, t, e ?? "")) : function(s, n) {
    return s.style.getPropertyValue(n) || Ws(s).getComputedStyle(s, null).getPropertyValue(n);
  }(this.node(), i);
}, property: function(i, t) {
  return arguments.length > 1 ? this.each((t == null ? sr : typeof t == "function" ? rr : nr)(i, t)) : this.node()[i];
}, classed: function(i, t) {
  var e = js(i + "");
  if (arguments.length < 2) {
    for (var s = ae(this.node()), n = -1, r = e.length; ++n < r; )
      if (!s.contains(e[n]))
        return !1;
    return !0;
  }
  return this.each((typeof t == "function" ? or : t ? hr : ar)(e, t));
}, text: function(i) {
  return arguments.length ? this.each(i == null ? lr : (typeof i == "function" ? cr : ur)(i)) : this.node().textContent;
}, html: function(i) {
  return arguments.length ? this.each(i == null ? dr : (typeof i == "function" ? fr : gr)(i)) : this.node().innerHTML;
}, raise: function() {
  return this.each(pr);
}, lower: function() {
  return this.each(mr);
}, append: function(i) {
  var t = typeof i == "function" ? i : Qe(i);
  return this.select(function() {
    return this.appendChild(t.apply(this, arguments));
  });
}, insert: function(i, t) {
  var e = typeof i == "function" ? i : Qe(i), s = t == null ? Ar : typeof t == "function" ? t : Ue(t);
  return this.select(function() {
    return this.insertBefore(e.apply(this, arguments), s.apply(this, arguments) || null);
  });
}, remove: function() {
  return this.each(yr);
}, clone: function(i) {
  return this.select(i ? vr : xr);
}, datum: function(i) {
  return arguments.length ? this.property("__data__", i) : this.node().__data__;
}, on: function(i, t, e) {
  var s, n, r = function(d) {
    return d.trim().split(/^|\s+/).map(function(c) {
      var g = "", p = c.indexOf(".");
      return p >= 0 && (g = c.slice(p + 1), c = c.slice(0, p)), { type: c, name: g };
    });
  }(i + ""), a = r.length;
  if (!(arguments.length < 2)) {
    for (h = t ? Sr : kr, e == null && (e = !1), s = 0; s < a; ++s)
      this.each(h(r[s], t, e));
    return this;
  }
  var h = this.node().__on;
  if (h) {
    for (var o, l = 0, u = h.length; l < u; ++l)
      for (s = 0, o = h[l]; s < a; ++s)
        if ((n = r[s]).type === o.type && n.name === o.name)
          return o.value;
  }
}, dispatch: function(i, t) {
  return this.each((typeof t == "function" ? Mr : wr)(i, t));
} };
class oe {
  constructor(t) {
    this.helper = t;
  }
  fillPolygons(t, e) {
    return this._fillPolygons(t, e);
  }
  _fillPolygons(t, e) {
    const s = qt(t, e);
    return { type: "fillSketch", ops: this.renderLines(s, e) };
  }
  renderLines(t, e) {
    const s = [];
    for (const n of t)
      s.push(...this.helper.doubleLineOps(n[0][0], n[0][1], n[1][0], n[1][1], e));
    return s;
  }
}
function yi(i) {
  const t = i[0], e = i[1];
  return Math.sqrt(Math.pow(t[0] - e[0], 2) + Math.pow(t[1] - e[1], 2));
}
class Fr extends oe {
  fillPolygons(t, e) {
    let s = e.hachureGap;
    s < 0 && (s = 4 * e.strokeWidth), s = Math.max(s, 0.1);
    const n = qt(t, Object.assign({}, e, { hachureGap: s })), r = Math.PI / 180 * e.hachureAngle, a = [], h = 0.5 * s * Math.cos(r), o = 0.5 * s * Math.sin(r);
    for (const [l, u] of n)
      yi([l, u]) && a.push([[l[0] - h, l[1] + o], [...u]], [[l[0] + h, l[1] - o], [...u]]);
    return { type: "fillSketch", ops: this.renderLines(a, e) };
  }
}
class Wr extends oe {
  fillPolygons(t, e) {
    const s = this._fillPolygons(t, e), n = Object.assign({}, e, { hachureAngle: e.hachureAngle + 90 }), r = this._fillPolygons(t, n);
    return s.ops = s.ops.concat(r.ops), s;
  }
}
class jr {
  constructor(t) {
    this.helper = t;
  }
  fillPolygons(t, e) {
    const s = qt(t, e = Object.assign({}, e, { hachureAngle: 0 }));
    return this.dotsOnLines(s, e);
  }
  dotsOnLines(t, e) {
    const s = [];
    let n = e.hachureGap;
    n < 0 && (n = 4 * e.strokeWidth), n = Math.max(n, 0.1);
    let r = e.fillWeight;
    r < 0 && (r = e.strokeWidth / 2);
    const a = n / 4;
    for (const h of t) {
      const o = yi(h), l = o / n, u = Math.ceil(l) - 1, d = o - u * n, c = (h[0][0] + h[1][0]) / 2 - n / 4, g = Math.min(h[0][1], h[1][1]);
      for (let p = 0; p < u; p++) {
        const f = g + d + p * n, y = c - a + 2 * Math.random() * a, m = f - a + 2 * Math.random() * a, A = this.helper.ellipse(y, m, r, r, e);
        s.push(...A.ops);
      }
    }
    return { type: "fillSketch", ops: s };
  }
}
let Cr = class {
  constructor(i) {
    this.helper = i;
  }
  fillPolygons(i, t) {
    const e = qt(i, t);
    return { type: "fillSketch", ops: this.dashedLine(e, t) };
  }
  dashedLine(i, t) {
    const e = t.dashOffset < 0 ? t.hachureGap < 0 ? 4 * t.strokeWidth : t.hachureGap : t.dashOffset, s = t.dashGap < 0 ? t.hachureGap < 0 ? 4 * t.strokeWidth : t.hachureGap : t.dashGap, n = [];
    return i.forEach((r) => {
      const a = yi(r), h = Math.floor(a / (e + s)), o = (a + s - h * (e + s)) / 2;
      let l = r[0], u = r[1];
      l[0] > u[0] && (l = r[1], u = r[0]);
      const d = Math.atan((u[1] - l[1]) / (u[0] - l[0]));
      for (let c = 0; c < h; c++) {
        const g = c * (e + s), p = g + e, f = [l[0] + g * Math.cos(d) + o * Math.cos(d), l[1] + g * Math.sin(d) + o * Math.sin(d)], y = [l[0] + p * Math.cos(d) + o * Math.cos(d), l[1] + p * Math.sin(d) + o * Math.sin(d)];
        n.push(...this.helper.doubleLineOps(f[0], f[1], y[0], y[1], t));
      }
    }), n;
  }
};
class Er {
  constructor(t) {
    this.helper = t;
  }
  fillPolygons(t, e) {
    const s = e.hachureGap < 0 ? 4 * e.strokeWidth : e.hachureGap, n = e.zigzagOffset < 0 ? s : e.zigzagOffset, r = qt(t, e = Object.assign({}, e, { hachureGap: s + n }));
    return { type: "fillSketch", ops: this.zigzagLines(r, n, e) };
  }
  zigzagLines(t, e, s) {
    const n = [];
    return t.forEach((r) => {
      const a = yi(r), h = Math.round(a / (2 * e));
      let o = r[0], l = r[1];
      o[0] > l[0] && (o = r[1], l = r[0]);
      const u = Math.atan((l[1] - o[1]) / (l[0] - o[0]));
      for (let d = 0; d < h; d++) {
        const c = 2 * d * e, g = 2 * (d + 1) * e, p = Math.sqrt(2 * Math.pow(e, 2)), f = [o[0] + c * Math.cos(u), o[1] + c * Math.sin(u)], y = [o[0] + g * Math.cos(u), o[1] + g * Math.sin(u)], m = [f[0] + p * Math.cos(u + Math.PI / 4), f[1] + p * Math.sin(u + Math.PI / 4)];
        n.push(...this.helper.doubleLineOps(f[0], f[1], m[0], m[1], s), ...this.helper.doubleLineOps(m[0], m[1], y[0], y[1], s));
      }
    }), n;
  }
}
const J = {};
class Or {
  constructor(t) {
    this.seed = t;
  }
  next() {
    return this.seed ? (2 ** 31 - 1 & (this.seed = Math.imul(48271, this.seed))) / 2 ** 31 : Math.random();
  }
}
const Kt = { A: 7, a: 7, C: 6, c: 6, H: 1, h: 1, L: 2, l: 2, M: 2, m: 2, Q: 4, q: 4, S: 4, s: 4, T: 2, t: 2, V: 1, v: 1, Z: 0, z: 0 };
function Ei(i, t) {
  return i.type === t;
}
function le(i) {
  const t = [], e = function(a) {
    const h = new Array();
    for (; a !== ""; )
      if (a.match(/^([ \t\r\n,]+)/))
        a = a.substr(RegExp.$1.length);
      else if (a.match(/^([aAcChHlLmMqQsStTvVzZ])/))
        h[h.length] = { type: 0, text: RegExp.$1 }, a = a.substr(RegExp.$1.length);
      else {
        if (!a.match(/^(([-+]?[0-9]+(\.[0-9]*)?|[-+]?\.[0-9]+)([eE][-+]?[0-9]+)?)/))
          return [];
        h[h.length] = { type: 1, text: `${parseFloat(RegExp.$1)}` }, a = a.substr(RegExp.$1.length);
      }
    return h[h.length] = { type: 2, text: "" }, h;
  }(i);
  let s = "BOD", n = 0, r = e[n];
  for (; !Ei(r, 2); ) {
    let a = 0;
    const h = [];
    if (s === "BOD") {
      if (r.text !== "M" && r.text !== "m")
        return le("M0,0" + i);
      n++, a = Kt[r.text], s = r.text;
    } else
      Ei(r, 1) ? a = Kt[s] : (n++, a = Kt[r.text], s = r.text);
    if (!(n + a < e.length))
      throw new Error("Path data ended short");
    for (let o = n; o < n + a; o++) {
      const l = e[o];
      if (!Ei(l, 1))
        throw new Error("Param not a number: " + s + "," + l.text);
      h[h.length] = +l.text;
    }
    if (typeof Kt[s] != "number")
      throw new Error("Bad segment: " + s);
    {
      const o = { key: s, data: h };
      t.push(o), n += a, r = e[n], s === "M" && (s = "L"), s === "m" && (s = "l");
    }
  }
  return t;
}
function Bs(i) {
  let t = 0, e = 0, s = 0, n = 0;
  const r = [];
  for (const { key: a, data: h } of i)
    switch (a) {
      case "M":
        r.push({ key: "M", data: [...h] }), [t, e] = h, [s, n] = h;
        break;
      case "m":
        t += h[0], e += h[1], r.push({ key: "M", data: [t, e] }), s = t, n = e;
        break;
      case "L":
        r.push({ key: "L", data: [...h] }), [t, e] = h;
        break;
      case "l":
        t += h[0], e += h[1], r.push({ key: "L", data: [t, e] });
        break;
      case "C":
        r.push({ key: "C", data: [...h] }), t = h[4], e = h[5];
        break;
      case "c": {
        const o = h.map((l, u) => u % 2 ? l + e : l + t);
        r.push({ key: "C", data: o }), t = o[4], e = o[5];
        break;
      }
      case "Q":
        r.push({ key: "Q", data: [...h] }), t = h[2], e = h[3];
        break;
      case "q": {
        const o = h.map((l, u) => u % 2 ? l + e : l + t);
        r.push({ key: "Q", data: o }), t = o[2], e = o[3];
        break;
      }
      case "A":
        r.push({ key: "A", data: [...h] }), t = h[5], e = h[6];
        break;
      case "a":
        t += h[5], e += h[6], r.push({ key: "A", data: [h[0], h[1], h[2], h[3], h[4], t, e] });
        break;
      case "H":
        r.push({ key: "H", data: [...h] }), t = h[0];
        break;
      case "h":
        t += h[0], r.push({ key: "H", data: [t] });
        break;
      case "V":
        r.push({ key: "V", data: [...h] }), e = h[0];
        break;
      case "v":
        e += h[0], r.push({ key: "V", data: [e] });
        break;
      case "S":
        r.push({ key: "S", data: [...h] }), t = h[2], e = h[3];
        break;
      case "s": {
        const o = h.map((l, u) => u % 2 ? l + e : l + t);
        r.push({ key: "S", data: o }), t = o[2], e = o[3];
        break;
      }
      case "T":
        r.push({ key: "T", data: [...h] }), t = h[0], e = h[1];
        break;
      case "t":
        t += h[0], e += h[1], r.push({ key: "T", data: [t, e] });
        break;
      case "Z":
      case "z":
        r.push({ key: "Z", data: [] }), t = s, e = n;
    }
  return r;
}
function Vs(i) {
  const t = [];
  let e = "", s = 0, n = 0, r = 0, a = 0, h = 0, o = 0;
  for (const { key: l, data: u } of i) {
    switch (l) {
      case "M":
        t.push({ key: "M", data: [...u] }), [s, n] = u, [r, a] = u;
        break;
      case "C":
        t.push({ key: "C", data: [...u] }), s = u[4], n = u[5], h = u[2], o = u[3];
        break;
      case "L":
        t.push({ key: "L", data: [...u] }), [s, n] = u;
        break;
      case "H":
        s = u[0], t.push({ key: "L", data: [s, n] });
        break;
      case "V":
        n = u[0], t.push({ key: "L", data: [s, n] });
        break;
      case "S": {
        let d = 0, c = 0;
        e === "C" || e === "S" ? (d = s + (s - h), c = n + (n - o)) : (d = s, c = n), t.push({ key: "C", data: [d, c, ...u] }), h = u[0], o = u[1], s = u[2], n = u[3];
        break;
      }
      case "T": {
        const [d, c] = u;
        let g = 0, p = 0;
        e === "Q" || e === "T" ? (g = s + (s - h), p = n + (n - o)) : (g = s, p = n);
        const f = s + 2 * (g - s) / 3, y = n + 2 * (p - n) / 3, m = d + 2 * (g - d) / 3, A = c + 2 * (p - c) / 3;
        t.push({ key: "C", data: [f, y, m, A, d, c] }), h = g, o = p, s = d, n = c;
        break;
      }
      case "Q": {
        const [d, c, g, p] = u, f = s + 2 * (d - s) / 3, y = n + 2 * (c - n) / 3, m = g + 2 * (d - g) / 3, A = p + 2 * (c - p) / 3;
        t.push({ key: "C", data: [f, y, m, A, g, p] }), h = d, o = c, s = g, n = p;
        break;
      }
      case "A": {
        const d = Math.abs(u[0]), c = Math.abs(u[1]), g = u[2], p = u[3], f = u[4], y = u[5], m = u[6];
        d === 0 || c === 0 ? (t.push({ key: "C", data: [s, n, y, m, y, m] }), s = y, n = m) : s === y && n === m || (Ys(s, n, y, m, d, c, g, p, f).forEach(function(A) {
          t.push({ key: "C", data: A });
        }), s = y, n = m);
        break;
      }
      case "Z":
        t.push({ key: "Z", data: [] }), s = r, n = a;
    }
    e = l;
  }
  return t;
}
function Wt(i, t, e) {
  return [i * Math.cos(e) - t * Math.sin(e), i * Math.sin(e) + t * Math.cos(e)];
}
function Ys(i, t, e, s, n, r, a, h, o, l) {
  const u = (d = a, Math.PI * d / 180);
  var d;
  let c = [], g = 0, p = 0, f = 0, y = 0;
  if (l)
    [g, p, f, y] = l;
  else {
    [i, t] = Wt(i, t, -u), [e, s] = Wt(e, s, -u);
    const R = (i - e) / 2, O = (t - s) / 2;
    let I = R * R / (n * n) + O * O / (r * r);
    I > 1 && (I = Math.sqrt(I), n *= I, r *= I);
    const L = n * n, E = r * r, T = L * E - L * O * O - E * R * R, q = L * O * O + E * R * R, G = (h === o ? -1 : 1) * Math.sqrt(Math.abs(T / q));
    f = G * n * O / r + (i + e) / 2, y = G * -r * R / n + (t + s) / 2, g = Math.asin(parseFloat(((t - y) / r).toFixed(9))), p = Math.asin(parseFloat(((s - y) / r).toFixed(9))), i < f && (g = Math.PI - g), e < f && (p = Math.PI - p), g < 0 && (g = 2 * Math.PI + g), p < 0 && (p = 2 * Math.PI + p), o && g > p && (g -= 2 * Math.PI), !o && p > g && (p -= 2 * Math.PI);
  }
  let m = p - g;
  if (Math.abs(m) > 120 * Math.PI / 180) {
    const R = p, O = e, I = s;
    p = o && p > g ? g + 120 * Math.PI / 180 * 1 : g + 120 * Math.PI / 180 * -1, c = Ys(e = f + n * Math.cos(p), s = y + r * Math.sin(p), O, I, n, r, a, 0, o, [p, R, f, y]);
  }
  m = p - g;
  const A = Math.cos(g), b = Math.sin(g), S = Math.cos(p), w = Math.sin(p), v = Math.tan(m / 4), M = 4 / 3 * n * v, j = 4 / 3 * r * v, C = [i, t], P = [i + M * b, t - j * A], F = [e + M * w, s - j * S], k = [e, s];
  if (P[0] = 2 * C[0] - P[0], P[1] = 2 * C[1] - P[1], l)
    return [P, F, k].concat(c);
  {
    c = [P, F, k].concat(c);
    const R = [];
    for (let O = 0; O < c.length; O += 3) {
      const I = Wt(c[O][0], c[O][1], u), L = Wt(c[O + 1][0], c[O + 1][1], u), E = Wt(c[O + 2][0], c[O + 2][1], u);
      R.push([I[0], I[1], L[0], L[1], E[0], E[1]]);
    }
    return R;
  }
}
const Lr = { randOffset: function(i, t) {
  return W(i, t);
}, randOffsetWithRange: function(i, t, e) {
  return li(i, t, e);
}, ellipse: function(i, t, e, s, n) {
  return Ui(i, t, n, Ds(e, s, n)).opset;
}, doubleLineOps: function(i, t, e, s, n) {
  return ct(i, t, e, s, n, !0);
} };
function Hs(i, t, e, s, n) {
  return { type: "path", ops: ct(i, t, e, s, n) };
}
function Qt(i, t, e) {
  const s = (i || []).length;
  if (s > 2) {
    const n = [];
    for (let r = 0; r < s - 1; r++)
      n.push(...ct(i[r][0], i[r][1], i[r + 1][0], i[r + 1][1], e));
    return t && n.push(...ct(i[s - 1][0], i[s - 1][1], i[0][0], i[0][1], e)), { type: "path", ops: n };
  }
  return s === 2 ? Hs(i[0][0], i[0][1], i[1][0], i[1][1], e) : { type: "path", ops: [] };
}
function _e(i, t) {
  if (i.length) {
    const e = typeof i[0][0] == "number" ? [i] : i, s = Ut(e[0], 1 * (1 + 0.2 * t.roughness), t), n = t.disableMultiStroke ? [] : Ut(e[0], 1.5 * (1 + 0.22 * t.roughness), is(t));
    for (let r = 1; r < e.length; r++) {
      const a = e[r];
      if (a.length) {
        const h = Ut(a, 1 * (1 + 0.2 * t.roughness), t), o = t.disableMultiStroke ? [] : Ut(a, 1.5 * (1 + 0.22 * t.roughness), is(t));
        for (const l of h)
          l.op !== "move" && s.push(l);
        for (const l of o)
          l.op !== "move" && n.push(l);
      }
    }
    return { type: "path", ops: s.concat(n) };
  }
  return { type: "path", ops: [] };
}
function Ds(i, t, e) {
  const s = Math.sqrt(2 * Math.PI * Math.sqrt((Math.pow(i / 2, 2) + Math.pow(t / 2, 2)) / 2)), n = Math.ceil(Math.max(e.curveStepCount, e.curveStepCount / Math.sqrt(200) * s)), r = 2 * Math.PI / n;
  let a = Math.abs(i / 2), h = Math.abs(t / 2);
  const o = 1 - e.curveFitting;
  return a += W(a * o, e), h += W(h * o, e), { increment: r, rx: a, ry: h };
}
function Ui(i, t, e, s) {
  const [n, r] = es(s.increment, i, t, s.rx, s.ry, 1, s.increment * li(0.1, li(0.4, 1, e), e), e);
  let a = ui(n, null, e);
  if (!e.disableMultiStroke && e.roughness !== 0) {
    const [h] = es(s.increment, i, t, s.rx, s.ry, 1.5, 0, e), o = ui(h, null, e);
    a = a.concat(o);
  }
  return { estimatedPoints: r, opset: { type: "path", ops: a } };
}
function $e(i, t, e, s, n, r, a, h, o) {
  const l = i, u = t;
  let d = Math.abs(e / 2), c = Math.abs(s / 2);
  d += W(0.01 * d, o), c += W(0.01 * c, o);
  let g = n, p = r;
  for (; g < 0; )
    g += 2 * Math.PI, p += 2 * Math.PI;
  p - g > 2 * Math.PI && (g = 0, p = 2 * Math.PI);
  const f = 2 * Math.PI / o.curveStepCount, y = Math.min(f / 2, (p - g) / 2), m = ss(y, l, u, d, c, g, p, 1, o);
  if (!o.disableMultiStroke) {
    const A = ss(y, l, u, d, c, g, p, 1.5, o);
    m.push(...A);
  }
  return a && (h ? m.push(...ct(l, u, l + d * Math.cos(g), u + c * Math.sin(g), o), ...ct(l, u, l + d * Math.cos(p), u + c * Math.sin(p), o)) : m.push({ op: "lineTo", data: [l, u] }, { op: "lineTo", data: [l + d * Math.cos(g), u + c * Math.sin(g)] })), { type: "path", ops: m };
}
function ts(i, t) {
  const e = Vs(Bs(le(i))), s = [];
  let n = [0, 0], r = [0, 0];
  for (const { key: a, data: h } of e)
    switch (a) {
      case "M":
        r = [h[0], h[1]], n = [h[0], h[1]];
        break;
      case "L":
        s.push(...ct(r[0], r[1], h[0], h[1], t)), r = [h[0], h[1]];
        break;
      case "C": {
        const [o, l, u, d, c, g] = h;
        s.push(...Gr(o, l, u, d, c, g, r, t)), r = [c, g];
        break;
      }
      case "Z":
        s.push(...ct(r[0], r[1], n[0], n[1], t)), r = [n[0], n[1]];
    }
  return { type: "path", ops: s };
}
function Oi(i, t) {
  const e = [];
  for (const s of i)
    if (s.length) {
      const n = t.maxRandomnessOffset || 0, r = s.length;
      if (r > 2) {
        e.push({ op: "move", data: [s[0][0] + W(n, t), s[0][1] + W(n, t)] });
        for (let a = 1; a < r; a++)
          e.push({ op: "lineTo", data: [s[a][0] + W(n, t), s[a][1] + W(n, t)] });
      }
    }
  return { type: "fillPath", ops: e };
}
function St(i, t) {
  return function(e, s) {
    let n = e.fillStyle || "hachure";
    if (!J[n])
      switch (n) {
        case "zigzag":
          J[n] || (J[n] = new Fr(s));
          break;
        case "cross-hatch":
          J[n] || (J[n] = new Wr(s));
          break;
        case "dots":
          J[n] || (J[n] = new jr(s));
          break;
        case "dashed":
          J[n] || (J[n] = new Cr(s));
          break;
        case "zigzag-line":
          J[n] || (J[n] = new Er(s));
          break;
        default:
          n = "hachure", J[n] || (J[n] = new oe(s));
      }
    return J[n];
  }(t, Lr).fillPolygons(i, t);
}
function is(i) {
  const t = Object.assign({}, i);
  return t.randomizer = void 0, i.seed && (t.seed = i.seed + 1), t;
}
function Ts(i) {
  return i.randomizer || (i.randomizer = new Or(i.seed || 0)), i.randomizer.next();
}
function li(i, t, e, s = 1) {
  return e.roughness * s * (Ts(e) * (t - i) + i);
}
function W(i, t, e = 1) {
  return li(-i, i, t, e);
}
function ct(i, t, e, s, n, r = !1) {
  const a = r ? n.disableMultiStrokeFill : n.disableMultiStroke, h = Ji(i, t, e, s, n, !0, !1);
  if (a)
    return h;
  const o = Ji(i, t, e, s, n, !0, !0);
  return h.concat(o);
}
function Ji(i, t, e, s, n, r, a) {
  const h = Math.pow(i - e, 2) + Math.pow(t - s, 2), o = Math.sqrt(h);
  let l = 1;
  l = o < 200 ? 1 : o > 500 ? 0.4 : -16668e-7 * o + 1.233334;
  let u = n.maxRandomnessOffset || 0;
  u * u * 100 > h && (u = o / 10);
  const d = u / 2, c = 0.2 + 0.2 * Ts(n);
  let g = n.bowing * n.maxRandomnessOffset * (s - t) / 200, p = n.bowing * n.maxRandomnessOffset * (i - e) / 200;
  g = W(g, n, l), p = W(p, n, l);
  const f = [], y = () => W(d, n, l), m = () => W(u, n, l), A = n.preserveVertices;
  return r && (a ? f.push({ op: "move", data: [i + (A ? 0 : y()), t + (A ? 0 : y())] }) : f.push({ op: "move", data: [i + (A ? 0 : W(u, n, l)), t + (A ? 0 : W(u, n, l))] })), a ? f.push({ op: "bcurveTo", data: [g + i + (e - i) * c + y(), p + t + (s - t) * c + y(), g + i + 2 * (e - i) * c + y(), p + t + 2 * (s - t) * c + y(), e + (A ? 0 : y()), s + (A ? 0 : y())] }) : f.push({ op: "bcurveTo", data: [g + i + (e - i) * c + m(), p + t + (s - t) * c + m(), g + i + 2 * (e - i) * c + m(), p + t + 2 * (s - t) * c + m(), e + (A ? 0 : m()), s + (A ? 0 : m())] }), f;
}
function Ut(i, t, e) {
  if (!i.length)
    return [];
  const s = [];
  s.push([i[0][0] + W(t, e), i[0][1] + W(t, e)]), s.push([i[0][0] + W(t, e), i[0][1] + W(t, e)]);
  for (let n = 1; n < i.length; n++)
    s.push([i[n][0] + W(t, e), i[n][1] + W(t, e)]), n === i.length - 1 && s.push([i[n][0] + W(t, e), i[n][1] + W(t, e)]);
  return ui(s, null, e);
}
function ui(i, t, e) {
  const s = i.length, n = [];
  if (s > 3) {
    const r = [], a = 1 - e.curveTightness;
    n.push({ op: "move", data: [i[1][0], i[1][1]] });
    for (let h = 1; h + 2 < s; h++) {
      const o = i[h];
      r[0] = [o[0], o[1]], r[1] = [o[0] + (a * i[h + 1][0] - a * i[h - 1][0]) / 6, o[1] + (a * i[h + 1][1] - a * i[h - 1][1]) / 6], r[2] = [i[h + 1][0] + (a * i[h][0] - a * i[h + 2][0]) / 6, i[h + 1][1] + (a * i[h][1] - a * i[h + 2][1]) / 6], r[3] = [i[h + 1][0], i[h + 1][1]], n.push({ op: "bcurveTo", data: [r[1][0], r[1][1], r[2][0], r[2][1], r[3][0], r[3][1]] });
    }
    if (t && t.length === 2) {
      const h = e.maxRandomnessOffset;
      n.push({ op: "lineTo", data: [t[0] + W(h, e), t[1] + W(h, e)] });
    }
  } else
    s === 3 ? (n.push({ op: "move", data: [i[1][0], i[1][1]] }), n.push({ op: "bcurveTo", data: [i[1][0], i[1][1], i[2][0], i[2][1], i[2][0], i[2][1]] })) : s === 2 && n.push(...Ji(i[0][0], i[0][1], i[1][0], i[1][1], e, !0, !0));
  return n;
}
function es(i, t, e, s, n, r, a, h) {
  const o = [], l = [];
  if (h.roughness === 0) {
    i /= 4, l.push([t + s * Math.cos(-i), e + n * Math.sin(-i)]);
    for (let u = 0; u <= 2 * Math.PI; u += i) {
      const d = [t + s * Math.cos(u), e + n * Math.sin(u)];
      o.push(d), l.push(d);
    }
    l.push([t + s * Math.cos(0), e + n * Math.sin(0)]), l.push([t + s * Math.cos(i), e + n * Math.sin(i)]);
  } else {
    const u = W(0.5, h) - Math.PI / 2;
    l.push([W(r, h) + t + 0.9 * s * Math.cos(u - i), W(r, h) + e + 0.9 * n * Math.sin(u - i)]);
    const d = 2 * Math.PI + u - 0.01;
    for (let c = u; c < d; c += i) {
      const g = [W(r, h) + t + s * Math.cos(c), W(r, h) + e + n * Math.sin(c)];
      o.push(g), l.push(g);
    }
    l.push([W(r, h) + t + s * Math.cos(u + 2 * Math.PI + 0.5 * a), W(r, h) + e + n * Math.sin(u + 2 * Math.PI + 0.5 * a)]), l.push([W(r, h) + t + 0.98 * s * Math.cos(u + a), W(r, h) + e + 0.98 * n * Math.sin(u + a)]), l.push([W(r, h) + t + 0.9 * s * Math.cos(u + 0.5 * a), W(r, h) + e + 0.9 * n * Math.sin(u + 0.5 * a)]);
  }
  return [l, o];
}
function ss(i, t, e, s, n, r, a, h, o) {
  const l = r + W(0.1, o), u = [];
  u.push([W(h, o) + t + 0.9 * s * Math.cos(l - i), W(h, o) + e + 0.9 * n * Math.sin(l - i)]);
  for (let d = l; d <= a; d += i)
    u.push([W(h, o) + t + s * Math.cos(d), W(h, o) + e + n * Math.sin(d)]);
  return u.push([t + s * Math.cos(a), e + n * Math.sin(a)]), u.push([t + s * Math.cos(a), e + n * Math.sin(a)]), ui(u, null, o);
}
function Gr(i, t, e, s, n, r, a, h) {
  const o = [], l = [h.maxRandomnessOffset || 1, (h.maxRandomnessOffset || 1) + 0.3];
  let u = [0, 0];
  const d = h.disableMultiStroke ? 1 : 2, c = h.preserveVertices;
  for (let g = 0; g < d; g++)
    g === 0 ? o.push({ op: "move", data: [a[0], a[1]] }) : o.push({ op: "move", data: [a[0] + (c ? 0 : W(l[0], h)), a[1] + (c ? 0 : W(l[0], h))] }), u = c ? [n, r] : [n + W(l[g], h), r + W(l[g], h)], o.push({ op: "bcurveTo", data: [i + W(l[g], h), t + W(l[g], h), e + W(l[g], h), s + W(l[g], h), u[0], u[1]] });
  return o;
}
function jt(i) {
  return [...i];
}
function ns(i, t = 0) {
  const e = i.length;
  if (e < 3)
    throw new Error("A curve must have at least three points.");
  const s = [];
  if (e === 3)
    s.push(jt(i[0]), jt(i[1]), jt(i[2]), jt(i[2]));
  else {
    const n = [];
    n.push(i[0], i[0]);
    for (let h = 1; h < i.length; h++)
      n.push(i[h]), h === i.length - 1 && n.push(i[h]);
    const r = [], a = 1 - t;
    s.push(jt(n[0]));
    for (let h = 1; h + 2 < n.length; h++) {
      const o = n[h];
      r[0] = [o[0], o[1]], r[1] = [o[0] + (a * n[h + 1][0] - a * n[h - 1][0]) / 6, o[1] + (a * n[h + 1][1] - a * n[h - 1][1]) / 6], r[2] = [n[h + 1][0] + (a * n[h][0] - a * n[h + 2][0]) / 6, n[h + 1][1] + (a * n[h][1] - a * n[h + 2][1]) / 6], r[3] = [n[h + 1][0], n[h + 1][1]], s.push(r[1], r[2], r[3]);
    }
  }
  return s;
}
function ti(i, t) {
  return Math.pow(i[0] - t[0], 2) + Math.pow(i[1] - t[1], 2);
}
function Ir(i, t, e) {
  const s = ti(t, e);
  if (s === 0)
    return ti(i, t);
  let n = ((i[0] - t[0]) * (e[0] - t[0]) + (i[1] - t[1]) * (e[1] - t[1])) / s;
  return n = Math.max(0, Math.min(1, n)), ti(i, mt(t, e, n));
}
function mt(i, t, e) {
  return [i[0] + (t[0] - i[0]) * e, i[1] + (t[1] - i[1]) * e];
}
function _i(i, t, e, s) {
  const n = s || [];
  if (function(h, o) {
    const l = h[o + 0], u = h[o + 1], d = h[o + 2], c = h[o + 3];
    let g = 3 * u[0] - 2 * l[0] - c[0];
    g *= g;
    let p = 3 * u[1] - 2 * l[1] - c[1];
    p *= p;
    let f = 3 * d[0] - 2 * c[0] - l[0];
    f *= f;
    let y = 3 * d[1] - 2 * c[1] - l[1];
    return y *= y, g < f && (g = f), p < y && (p = y), g + p;
  }(i, t) < e) {
    const h = i[t + 0];
    n.length ? (r = n[n.length - 1], a = h, Math.sqrt(ti(r, a)) > 1 && n.push(h)) : n.push(h), n.push(i[t + 3]);
  } else {
    const o = i[t + 0], l = i[t + 1], u = i[t + 2], d = i[t + 3], c = mt(o, l, 0.5), g = mt(l, u, 0.5), p = mt(u, d, 0.5), f = mt(c, g, 0.5), y = mt(g, p, 0.5), m = mt(f, y, 0.5);
    _i([o, c, f, m], 0, e, n), _i([m, y, p, d], 0, e, n);
  }
  var r, a;
  return n;
}
function Zr(i, t) {
  return ci(i, 0, i.length, t);
}
function ci(i, t, e, s, n) {
  const r = n || [], a = i[t], h = i[e - 1];
  let o = 0, l = 1;
  for (let u = t + 1; u < e - 1; ++u) {
    const d = Ir(i[u], a, h);
    d > o && (o = d, l = u);
  }
  return Math.sqrt(o) > s ? (ci(i, t, l + 1, s, r), ci(i, l, e, s, r)) : (r.length || r.push(a), r.push(h)), r;
}
function Li(i, t = 0.15, e) {
  const s = [], n = (i.length - 1) / 3;
  for (let r = 0; r < n; r++)
    _i(i, 3 * r, t, s);
  return e && e > 0 ? ci(s, 0, s.length, e) : s;
}
const tt = "none";
class Br {
  constructor(t) {
    this.defaultOptions = { maxRandomnessOffset: 2, roughness: 1, bowing: 1, stroke: "#000", strokeWidth: 1, curveTightness: 0, curveFitting: 0.95, curveStepCount: 9, fillStyle: "hachure", fillWeight: -1, hachureAngle: -41, hachureGap: -1, dashOffset: -1, dashGap: -1, zigzagOffset: -1, seed: 0, disableMultiStroke: !1, disableMultiStrokeFill: !1, preserveVertices: !1, fillShapeRoughnessGain: 0.8 }, this.config = t || {}, this.config.options && (this.defaultOptions = this._o(this.config.options));
  }
  static newSeed() {
    return Math.floor(Math.random() * 2 ** 31);
  }
  _o(t) {
    return t ? Object.assign({}, this.defaultOptions, t) : this.defaultOptions;
  }
  _d(t, e, s) {
    return { shape: t, sets: e || [], options: s || this.defaultOptions };
  }
  line(t, e, s, n, r) {
    const a = this._o(r);
    return this._d("line", [Hs(t, e, s, n, a)], a);
  }
  rectangle(t, e, s, n, r) {
    const a = this._o(r), h = [], o = function(l, u, d, c, g) {
      return function(p, f) {
        return Qt(p, !0, f);
      }([[l, u], [l + d, u], [l + d, u + c], [l, u + c]], g);
    }(t, e, s, n, a);
    if (a.fill) {
      const l = [[t, e], [t + s, e], [t + s, e + n], [t, e + n]];
      a.fillStyle === "solid" ? h.push(Oi([l], a)) : h.push(St([l], a));
    }
    return a.stroke !== tt && h.push(o), this._d("rectangle", h, a);
  }
  ellipse(t, e, s, n, r) {
    const a = this._o(r), h = [], o = Ds(s, n, a), l = Ui(t, e, a, o);
    if (a.fill)
      if (a.fillStyle === "solid") {
        const u = Ui(t, e, a, o).opset;
        u.type = "fillPath", h.push(u);
      } else
        h.push(St([l.estimatedPoints], a));
    return a.stroke !== tt && h.push(l.opset), this._d("ellipse", h, a);
  }
  circle(t, e, s, n) {
    const r = this.ellipse(t, e, s, s, n);
    return r.shape = "circle", r;
  }
  linearPath(t, e) {
    const s = this._o(e);
    return this._d("linearPath", [Qt(t, !1, s)], s);
  }
  arc(t, e, s, n, r, a, h = !1, o) {
    const l = this._o(o), u = [], d = $e(t, e, s, n, r, a, h, !0, l);
    if (h && l.fill)
      if (l.fillStyle === "solid") {
        const c = Object.assign({}, l);
        c.disableMultiStroke = !0;
        const g = $e(t, e, s, n, r, a, !0, !1, c);
        g.type = "fillPath", u.push(g);
      } else
        u.push(function(c, g, p, f, y, m, A) {
          const b = c, S = g;
          let w = Math.abs(p / 2), v = Math.abs(f / 2);
          w += W(0.01 * w, A), v += W(0.01 * v, A);
          let M = y, j = m;
          for (; M < 0; )
            M += 2 * Math.PI, j += 2 * Math.PI;
          j - M > 2 * Math.PI && (M = 0, j = 2 * Math.PI);
          const C = (j - M) / A.curveStepCount, P = [];
          for (let F = M; F <= j; F += C)
            P.push([b + w * Math.cos(F), S + v * Math.sin(F)]);
          return P.push([b + w * Math.cos(j), S + v * Math.sin(j)]), P.push([b, S]), St([P], A);
        }(t, e, s, n, r, a, l));
    return l.stroke !== tt && u.push(d), this._d("arc", u, l);
  }
  curve(t, e) {
    const s = this._o(e), n = [], r = _e(t, s);
    if (s.fill && s.fill !== tt)
      if (s.fillStyle === "solid") {
        const a = _e(t, Object.assign(Object.assign({}, s), { disableMultiStroke: !0, roughness: s.roughness ? s.roughness + s.fillShapeRoughnessGain : 0 }));
        n.push({ type: "fillPath", ops: this._mergedShape(a.ops) });
      } else {
        const a = [], h = t;
        if (h.length) {
          const o = typeof h[0][0] == "number" ? [h] : h;
          for (const l of o)
            l.length < 3 ? a.push(...l) : l.length === 3 ? a.push(...Li(ns([l[0], l[0], l[1], l[2]]), 10, (1 + s.roughness) / 2)) : a.push(...Li(ns(l), 10, (1 + s.roughness) / 2));
        }
        a.length && n.push(St([a], s));
      }
    return s.stroke !== tt && n.push(r), this._d("curve", n, s);
  }
  polygon(t, e) {
    const s = this._o(e), n = [], r = Qt(t, !0, s);
    return s.fill && (s.fillStyle === "solid" ? n.push(Oi([t], s)) : n.push(St([t], s))), s.stroke !== tt && n.push(r), this._d("polygon", n, s);
  }
  path(t, e) {
    const s = this._o(e), n = [];
    if (!t)
      return this._d("path", n, s);
    t = (t || "").replace(/\n/g, " ").replace(/(-\s)/g, "-").replace("/(ss)/g", " ");
    const r = s.fill && s.fill !== "transparent" && s.fill !== tt, a = s.stroke !== tt, h = !!(s.simplification && s.simplification < 1), o = function(u, d, c) {
      const g = Vs(Bs(le(u))), p = [];
      let f = [], y = [0, 0], m = [];
      const A = () => {
        m.length >= 4 && f.push(...Li(m, 1)), m = [];
      }, b = () => {
        A(), f.length && (p.push(f), f = []);
      };
      for (const { key: w, data: v } of g)
        switch (w) {
          case "M":
            b(), y = [v[0], v[1]], f.push(y);
            break;
          case "L":
            A(), f.push([v[0], v[1]]);
            break;
          case "C":
            if (!m.length) {
              const M = f.length ? f[f.length - 1] : y;
              m.push([M[0], M[1]]);
            }
            m.push([v[0], v[1]]), m.push([v[2], v[3]]), m.push([v[4], v[5]]);
            break;
          case "Z":
            A(), f.push([y[0], y[1]]);
        }
      if (b(), !c)
        return p;
      const S = [];
      for (const w of p) {
        const v = Zr(w, c);
        v.length && S.push(v);
      }
      return S;
    }(t, 0, h ? 4 - 4 * (s.simplification || 1) : (1 + s.roughness) / 2), l = ts(t, s);
    if (r)
      if (s.fillStyle === "solid")
        if (o.length === 1) {
          const u = ts(t, Object.assign(Object.assign({}, s), { disableMultiStroke: !0, roughness: s.roughness ? s.roughness + s.fillShapeRoughnessGain : 0 }));
          n.push({ type: "fillPath", ops: this._mergedShape(u.ops) });
        } else
          n.push(Oi(o, s));
      else
        n.push(St(o, s));
    return a && (h ? o.forEach((u) => {
      n.push(Qt(u, !1, s));
    }) : n.push(l)), this._d("path", n, s);
  }
  opsToPath(t, e) {
    let s = "";
    for (const n of t.ops) {
      const r = typeof e == "number" && e >= 0 ? n.data.map((a) => +a.toFixed(e)) : n.data;
      switch (n.op) {
        case "move":
          s += `M${r[0]} ${r[1]} `;
          break;
        case "bcurveTo":
          s += `C${r[0]} ${r[1]}, ${r[2]} ${r[3]}, ${r[4]} ${r[5]} `;
          break;
        case "lineTo":
          s += `L${r[0]} ${r[1]} `;
      }
    }
    return s.trim();
  }
  toPaths(t) {
    const e = t.sets || [], s = t.options || this.defaultOptions, n = [];
    for (const r of e) {
      let a = null;
      switch (r.type) {
        case "path":
          a = { d: this.opsToPath(r), stroke: s.stroke, strokeWidth: s.strokeWidth, fill: tt };
          break;
        case "fillPath":
          a = { d: this.opsToPath(r), stroke: tt, strokeWidth: 0, fill: s.fill || tt };
          break;
        case "fillSketch":
          a = this.fillSketch(r, s);
      }
      a && n.push(a);
    }
    return n;
  }
  fillSketch(t, e) {
    let s = e.fillWeight;
    return s < 0 && (s = e.strokeWidth / 2), { d: this.opsToPath(t), stroke: e.fill || tt, strokeWidth: s, fill: tt };
  }
  _mergedShape(t) {
    return t.filter((e, s) => s === 0 || e.op !== "move");
  }
}
const Jt = "http://www.w3.org/2000/svg";
class Vr {
  constructor(t, e) {
    this.svg = t, this.gen = new Br(e);
  }
  draw(t) {
    const e = t.sets || [], s = t.options || this.getDefaultOptions(), n = this.svg.ownerDocument || window.document, r = n.createElementNS(Jt, "g"), a = t.options.fixedDecimalPlaceDigits;
    for (const h of e) {
      let o = null;
      switch (h.type) {
        case "path":
          o = n.createElementNS(Jt, "path"), o.setAttribute("d", this.opsToPath(h, a)), o.setAttribute("stroke", s.stroke), o.setAttribute("stroke-width", s.strokeWidth + ""), o.setAttribute("fill", "none"), s.strokeLineDash && o.setAttribute("stroke-dasharray", s.strokeLineDash.join(" ").trim()), s.strokeLineDashOffset && o.setAttribute("stroke-dashoffset", `${s.strokeLineDashOffset}`);
          break;
        case "fillPath":
          o = n.createElementNS(Jt, "path"), o.setAttribute("d", this.opsToPath(h, a)), o.setAttribute("stroke", "none"), o.setAttribute("stroke-width", "0"), o.setAttribute("fill", s.fill || ""), t.shape !== "curve" && t.shape !== "polygon" || o.setAttribute("fill-rule", "evenodd");
          break;
        case "fillSketch":
          o = this.fillSketch(n, h, s);
      }
      o && r.appendChild(o);
    }
    return r;
  }
  fillSketch(t, e, s) {
    let n = s.fillWeight;
    n < 0 && (n = s.strokeWidth / 2);
    const r = t.createElementNS(Jt, "path");
    return r.setAttribute("d", this.opsToPath(e, s.fixedDecimalPlaceDigits)), r.setAttribute("stroke", s.fill || ""), r.setAttribute("stroke-width", n + ""), r.setAttribute("fill", "none"), s.fillLineDash && r.setAttribute("stroke-dasharray", s.fillLineDash.join(" ").trim()), s.fillLineDashOffset && r.setAttribute("stroke-dashoffset", `${s.fillLineDashOffset}`), r;
  }
  get generator() {
    return this.gen;
  }
  getDefaultOptions() {
    return this.gen.defaultOptions;
  }
  opsToPath(t, e) {
    return this.gen.opsToPath(t, e);
  }
  line(t, e, s, n, r) {
    const a = this.gen.line(t, e, s, n, r);
    return this.draw(a);
  }
  rectangle(t, e, s, n, r) {
    const a = this.gen.rectangle(t, e, s, n, r);
    return this.draw(a);
  }
  ellipse(t, e, s, n, r) {
    const a = this.gen.ellipse(t, e, s, n, r);
    return this.draw(a);
  }
  circle(t, e, s, n) {
    const r = this.gen.circle(t, e, s, n);
    return this.draw(r);
  }
  linearPath(t, e) {
    const s = this.gen.linearPath(t, e);
    return this.draw(s);
  }
  polygon(t, e) {
    const s = this.gen.polygon(t, e);
    return this.draw(s);
  }
  arc(t, e, s, n, r, a, h = !1, o) {
    const l = this.gen.arc(t, e, s, n, r, a, h, o);
    return this.draw(l);
  }
  curve(t, e) {
    const s = this.gen.curve(t, e);
    return this.draw(s);
  }
  path(t, e) {
    const s = this.gen.path(t, e);
    return this.draw(s);
  }
}
var D = (i, t) => new Vr(i, t);
class lt {
  constructor(t) {
    this.el = t.element, this.element = t.element, this.title = t.title, this.titleFontSize = t.titleFontSize || "17px", this.font = t.font || 0, this.fillStyle = t.fillStyle, this.tooltipFontSize = t.tooltipFontSize || "0.95rem", this.bowing = t.bowing || 0, this.simplification = t.simplification || 0.2, this.interactive = t.interactive !== !1, this.dataFormat = typeof t.data == "object" ? "object" : "file";
  }
  setSvg() {
    this.svg = x(this.el).append("svg").attr("viewBox", `0 0 ${this.width + this.margin.left + this.margin.right}
       ${this.height + this.margin.top + this.margin.bottom}`).append("g").attr("id", this.roughId).attr("transform", "translate(" + this.margin.left + "," + this.margin.top + ")");
  }
  resolveFont() {
    this.font === 0 || this.font === void 0 || this.font.toString().toLowerCase() === "gaegu" ? (this.svg.append("defs").append("style").attr("type", "text/css").text(`@font-face {
    font-family: 'gaeguregular';
    src: url(data:application/font-woff2;charset=utf-8;base64,) format('woff2'),
         url(data:application/font-woff;charset=utf-8;base64,) format('woff');
    font-weight: normal;
    font-style: normal;
}`), this.fontFamily = "gaeguregular") : this.font === 1 || this.font.toString().toLowerCase() === "indie flower" ? (((t) => {
      t.append("defs").append("style").attr("type", "text/css").text(`@font-face {
    font-family: 'indie_flowerregular';
    src: url(data:application/font-woff2;charset=utf-8;base64,) format('woff2'),
         url(data:application/font-woff;charset=utf-8;base64,) format('woff');
    font-weight: normal;
    font-style: normal;
	}`);
    })(this.svg), this.fontFamily = "indie_flowerregular") : this.fontFamily = this.font;
  }
}
const at = ({ roughness: i, ceiling: t = 20, defaultValue: e = 1 }) => i === void 0 || typeof i != "number" ? e : i > t ? t : i;
class lh extends lt {
  constructor(t) {
    super(t), this.data = t.data, this.margin = t.margin || { top: 20, right: 10, bottom: 20, left: 20 }, this.color = t.color || "red", this.highlight = t.highlight || "coral", this.roughness = at({ roughness: t.roughness }), this.stroke = t.stroke || "black", this.strokeWidth = t.strokeWidth || 1, this.axisStrokeWidth = t.axisStrokeWidth || 0.5, this.axisRoughness = t.axisRoughness || 0.5, this.innerStrokeWidth = t.innerStrokeWidth || 1, this.fillWeight = t.fillWeight || 0.5, this.axisFontSize = t.axisFontSize, this.labels = this.dataFormat === "object" ? "labels" : t.labels, this.values = this.dataFormat === "object" ? "values" : t.values, this.xValueFormat = t.xValueFormat, this.yValueFormat = t.yValueFormat, this.padding = t.padding || 0.1, this.xLabel = t.xLabel || "", this.yLabel = t.yLabel || "", this.labelFontSize = t.labelFontSize || "1rem", this.responsive = !0, this.boundRedraw = this.redraw.bind(this, t), this.initChartValues(t) && (this.resolveFont(), this.drawChart = this.resolveData(t.data), this.drawChart(), t.title !== "undefined" && this.setTitle(t.title), this.boundResizeHandler = this.resizeHandler.bind(this), window.addEventListener("resize", this.boundResizeHandler));
  }
  resizeHandler() {
    x(this.el).node() ? this.responsive && this.boundRedraw() : this.boundResizeHandler && (window.removeEventListener("resize", this.boundResizeHandler), this.boundResizeHandler = null);
  }
  remove() {
    x(this.el).select("svg").remove(), x(this.el).select(".tooltip").remove();
  }
  redraw(t) {
    this.remove(), this.initChartValues(t) && (this.resolveFont(), this.drawChart = this.resolveData(t.data), this.drawChart(), t.title !== "undefined" && this.setTitle(t.title));
  }
  initChartValues(t) {
    this.roughness = t.roughness || this.roughness, this.color = t.color || this.color, this.stroke = t.stroke || this.stroke, this.strokeWidth = t.strokeWidth || this.strokeWidth, this.axisStrokeWidth = t.axisStrokeWidth || this.axisStrokeWidth, this.axisRoughness = t.axisRoughness || this.axisRoughness, this.innerStrokeWidth = t.innerStrokeWidth || this.innerStrokeWidth, this.fillWeight = t.fillWeight || this.fillWeight, this.fillStyle = t.fillStyle || this.fillStyle, this.title = t.title || this.title;
    const e = x(this.el).node();
    if (!e)
      return this.width = 0, this.height = 0, !1;
    const s = e.getBoundingClientRect(), n = s.width, r = s.height;
    return this.width = n - this.margin.left - this.margin.right, this.height = r - this.margin.top - this.margin.bottom, this.roughId = this.el + "_svg", this.graphClass = this.el.substring(1, this.el.length), this.interactionG = "g." + this.graphClass, this.setSvg(), !0;
  }
  resolveData(t) {
    return typeof t != "string" ? () => {
      this.data = t, this.drawFromObject();
    } : t.includes(".csv") ? () => {
      vt(t).then((e) => {
        this.data = e, this.drawFromFile();
      });
    } : t.includes(".tsv") ? () => {
      bt(t).then((e) => {
        this.data = e, this.drawFromFile();
      });
    } : void 0;
  }
  addScales() {
    const t = this;
    this.xScale = Tt().rangeRound([0, this.width]).padding(this.padding).domain(this.dataFormat === "file" ? this.data.map((e) => e[t.labels]) : this.data[t.labels]), this.yScale = Z().rangeRound([this.height, 0]).domain(this.dataFormat === "file" ? [0, Q(this.data, (e) => +e[t.values])] : [0, Q(this.data[t.values])]);
  }
  addLabels() {
    this.xLabel !== "" && this.svg.append("text").attr("x", this.width / 2).attr("y", this.height + this.margin.bottom / 2).attr("dx", "1em").attr("class", "labelText").style("text-anchor", "middle").style("font-family", this.fontFamily).style("font-size", this.labelFontSize).text(this.xLabel), this.yLabel !== "" && this.svg.append("text").attr("transform", "rotate(-90)").attr("y", 0 - this.margin.left / 1.4).attr("x", 0 - this.height / 2).attr("dy", "1em").attr("class", "labelText").style("text-anchor", "middle").style("font-family", this.fontFamily).style("font-size", this.labelFontSize).text(this.yLabel);
  }
  addAxes() {
    const t = Ht(this.xScale).tickSize(0).tickFormat((s) => this.xValueFormat ? ot(this.xValueFormat)(s) : s), e = Dt(this.yScale).tickSize(0).tickFormat((s) => this.yValueFormat ? ot(this.yValueFormat)(s) : s);
    this.svg.append("g").attr("transform", "translate(0," + this.height + ")").call(t).attr("class", `xAxis${this.graphClass}`).selectAll("text").attr("transform", "translate(-10,0)rotate(-45)").style("text-anchor", "end").style("font-family", this.fontFamily).style("font-size", this.axisFontSize === void 0 ? `${Math.min(0.8, Math.min(this.width, this.height) / 140)}rem` : this.axisFontSize).style("opacity", 0.9), this.svg.append("g").call(e).attr("class", `yAxis${this.graphClass}`).selectAll("text").style("font-family", this.fontFamily).style("font-size", this.axisFontSize === void 0 ? `${Math.min(0.95, Math.min(this.width, this.height) / 140)}rem` : this.axisFontSize).style("opacity", 0.9), z("path.domain").attr("stroke", "transparent");
  }
  makeAxesRough(t, e) {
    const s = `xAxis${this.graphClass}`, n = `yAxis${this.graphClass}`, r = `rough-${s}`, a = `rough-${n}`;
    x(`.${s}`).selectAll("path.domain").each(function(h, o) {
      const l = x(this).node().getAttribute("d"), u = e.path(l, { fillStyle: "hachure" });
      u.setAttribute("class", r), t.appendChild(u);
    }), z(`.${r}`).attr("transform", `translate(0, ${this.height})`), x(`.${n}`).selectAll("path.domain").each(function(h, o) {
      const l = x(this).node().getAttribute("d"), u = e.path(l, { fillStyle: "hachure" });
      u.setAttribute("class", a), t.appendChild(u);
    });
  }
  setTitle(t) {
    this.svg.append("text").attr("x", this.width / 2).attr("y", 0 - this.margin.top / 2).attr("class", "title").attr("text-anchor", "middle").style("font-size", this.titleFontSize === void 0 ? `${Math.min(40, Math.min(this.width, this.height) / 5)}px` : this.titleFontSize).style("font-family", this.fontFamily).style("opacity", 0.8).text(t);
  }
  addInteraction() {
    z(this.interactionG).data(this.dataFormat === "file" ? this.data : this.data.values).append("rect").attr("x", (s, n) => this.dataFormat === "file" ? this.xScale(s[this.labels]) : this.xScale(this.data[this.labels][n])).attr("y", (s, n) => this.dataFormat === "file" ? this.yScale(+s[this.values]) : this.yScale(this.data[this.values][n])).attr("width", this.xScale.bandwidth()).attr("height", (s, n) => this.dataFormat === "file" ? this.height - this.yScale(+s[this.values]) : this.height - this.yScale(this.data[this.values][n])).attr("fill", "transparent");
    const t = x(this.el).append("div").style("opacity", 0).attr("class", "tooltip").style("position", "absolute").style("background-color", "white").style("border", "solid").style("border-width", "1px").style("border-radius", "5px").style("padding", "3px").style("font-family", this.fontFamily).style("font-size", this.tooltipFontSize).style("pointer-events", "none"), e = this;
    z(this.interactionG).on("mouseover", function() {
      t.style("opacity", 1), x(this).select("path").style("stroke", e.highlight), x(this).selectAll("path:nth-child(2)").style("stroke-width", e.strokeWidth + 1.2);
    }), z(this.interactionG).on("mouseout", function() {
      t.style("opacity", 0), x(this).select("path").style("stroke", e.color), x(this).selectAll("path:nth-child(2)").style("stroke-width", e.strokeWidth);
    }), z(this.interactionG).on("mousemove", function(s) {
      const n = x(this).attr("attrX"), r = x(this).attr("attrY"), a = kt(this);
      t.html(`<b>${n}</b>: ${r}`).style("opacity", 0.95).style("transform", `translate(${a[0] + 10 + e.margin.left}px, 
              ${a[1] - 10 - (e.height + e.margin.top + e.margin.bottom / 2)}px)`);
    });
  }
  initRoughObjects() {
    this.roughSvg = document.getElementById(this.roughId), this.rcAxis = D(this.roughSvg, { options: { strokeWidth: this.axisStrokeWidth, roughness: this.axisRoughness } }), this.rc = D(this.roughSvg, { options: { fill: this.color, stroke: this.stroke === "none" ? void 0 : this.stroke, strokeWidth: this.innerStrokeWidth, roughness: this.roughness, bowing: this.bowing, fillStyle: this.fillStyle } });
  }
  drawFromObject() {
    this.initRoughObjects(), this.addScales(), this.addAxes(), this.makeAxesRough(this.roughSvg, this.rcAxis), this.addLabels(), this.data.values.forEach((t, e) => {
      const s = this.rc.rectangle(this.xScale(this.data[this.labels][e]), this.yScale(+t), this.xScale.bandwidth(), this.height - this.yScale(+t), { simplification: this.simplification, fillWeight: this.fillWeight }), n = this.roughSvg.appendChild(s);
      n.setAttribute("class", this.graphClass), n.setAttribute("attrX", this.data[this.labels][e]), n.setAttribute("attrY", +t);
    }), z(this.interactionG).selectAll("path:nth-child(2)").style("stroke-width", this.strokeWidth), this.interactive === !0 && this.addInteraction();
  }
  drawFromFile() {
    this.initRoughObjects(), this.addScales(), this.addAxes(), this.makeAxesRough(this.roughSvg, this.rcAxis), this.addLabels(), this.data.forEach((t) => {
      const e = this.rc.rectangle(this.xScale(t[this.labels]), this.yScale(+t[this.values]), this.xScale.bandwidth(), this.height - this.yScale(+t[this.values]), { simplification: this.simplification, fillWeight: this.fillWeight }), s = this.roughSvg.appendChild(e);
      s.setAttribute("class", this.graphClass), s.setAttribute("attrX", t[this.labels]), s.setAttribute("attrY", +t[this.values]);
    }), z(this.interactionG).selectAll("path:nth-child(2)").style("stroke-width", this.strokeWidth), this.interactive === !0 && this.addInteraction();
  }
}
class uh extends lt {
  constructor(t) {
    super(t), this.margin = t.margin || { top: 50, right: 20, bottom: 50, left: 100 }, this.color = t.color || "red", this.highlight = t.highlight || "coral", this.roughness = at({ roughness: t.roughness }), this.stroke = t.stroke || "black", this.strokeWidth = t.strokeWidth || 1, this.axisStrokeWidth = t.axisStrokeWidth || 0.5, this.axisRoughness = t.axisRoughness || 0.5, this.innerStrokeWidth = t.innerStrokeWidth || 1, this.fillWeight = t.fillWeight || 0.5, this.axisFontSize = t.axisFontSize, this.labels = this.dataFormat === "object" ? "labels" : t.labels, this.values = this.dataFormat === "object" ? "values" : t.values, this.xValueFormat = t.xValueFormat, this.yValueFormat = t.yValueFormat, this.padding = t.padding || 0.1, this.xLabel = t.xLabel || "", this.yLabel = t.yLabel || "", this.labelFontSize = t.labelFontSize || "1rem", this.responsive = !0, this.boundRedraw = this.redraw.bind(this, t), this.initChartValues(t) && (this.resolveFont(), this.drawChart = this.resolveData(t.data), this.drawChart(), t.title !== "undefined" && this.setTitle(t.title), this.boundResizeHandler = this.resizeHandler.bind(this), window.addEventListener("resize", this.boundResizeHandler));
  }
  resizeHandler() {
    x(this.el).node() ? this.responsive && this.boundRedraw() : this.boundResizeHandler && (window.removeEventListener("resize", this.boundResizeHandler), this.boundResizeHandler = null);
  }
  remove() {
    x(this.el).select("svg").remove();
  }
  redraw(t) {
    this.remove(), this.initChartValues(t) && (this.resolveFont(), this.drawChart = this.resolveData(t.data), this.drawChart(), t.title !== "undefined" && this.setTitle(t.title));
  }
  initChartValues(t) {
    this.roughness = t.roughness || this.roughness, this.stroke = t.stroke || this.stroke, this.color = t.color || this.color, this.strokeWidth = t.strokeWidth || this.strokeWidth, this.axisStrokeWidth = t.axisStrokeWidth || this.axisStrokeWidth, this.axisRoughness = t.axisRoughness || this.axisRoughness, this.innerStrokeWidth = t.innerStrokeWidth || this.innerStrokeWidth, this.fillWeight = t.fillWeight || this.fillWeight, this.fillStyle = t.fillStyle || this.fillStyle;
    const e = x(this.el).node();
    if (!e)
      return this.width = 0, this.height = 0, !1;
    const s = e.getBoundingClientRect(), n = s.width, r = s.height;
    return this.width = n - this.margin.left - this.margin.right, this.height = r - this.margin.top - this.margin.bottom, this.roughId = this.el + "_svg", this.graphClass = this.el.substring(1, this.el.length), this.interactionG = "g." + this.graphClass, this.setSvg(), !0;
  }
  resolveData(t) {
    return typeof t != "string" ? () => {
      this.data = t, this.drawFromObject();
    } : t.includes(".csv") ? () => {
      vt(t).then((e) => {
        this.data = e, this.drawFromFile();
      });
    } : t.includes(".tsv") ? () => {
      bt(t).then((e) => {
        this.data = e, this.drawFromFile();
      });
    } : void 0;
  }
  addScales() {
    const t = this;
    this.yScale = Tt().rangeRound([0, this.height]).padding(this.padding).domain(this.dataFormat === "file" ? this.data.map((e) => e[t.labels]) : this.data[t.labels]), this.xScale = Z().rangeRound([0, this.width]).domain(this.dataFormat === "file" ? [0, Q(this.data, (e) => +e[t.values])] : [0, Q(this.data[t.values])]);
  }
  addLabels() {
    this.xLabel !== "" && this.svg.append("text").attr("x", this.width / 2).attr("y", this.height + this.margin.bottom / 2.4).attr("dx", "1em").attr("class", "labelText").style("text-anchor", "middle").style("font-family", this.fontFamily).style("font-size", this.labelFontSize).text(this.xLabel), this.yLabel !== "" && this.svg.append("text").attr("transform", "rotate(-90)").attr("y", 0 - this.margin.left / 1.5).attr("x", 0 - this.height / 2).attr("dy", "1em").attr("class", "labelText").style("text-anchor", "middle").style("font-family", this.fontFamily).style("font-size", this.labelFontSize).text(this.yLabel);
  }
  addAxes() {
    const t = Ht(this.xScale).tickSize(0).tickFormat((s) => this.xValueFormat ? ot(this.xValueFormat)(s) : s), e = Dt(this.yScale).tickSize(0).tickFormat((s) => this.yValueFormat ? ot(this.yValueFormat)(s) : s);
    this.svg.append("g").attr("transform", `translate(0, ${this.height})`).call(t).attr("class", `xAxis${this.graphClass}`).selectAll("text").attr("transform", "translate(-10,0)rotate(-45)").style("text-anchor", "end").style("font-family", this.fontFamily).style("font-size", this.axisFontSize === void 0 ? `${Math.min(0.95, Math.min(this.width, this.height) / 140)}rem` : this.axisFontSize).style("opacity", 0.85), this.svg.append("g").call(e).attr("class", `yAxis${this.graphClass}`).selectAll("text").style("font-family", this.fontFamily).style("font-size", this.axisFontSize === void 0 ? `${Math.min(0.95, Math.min(this.width, this.height) / 140)}rem` : this.axisFontSize).style("opacity", 0.85), z("path.domain").attr("stroke", "transparent");
  }
  makeAxesRough(t, e) {
    const s = `xAxis${this.graphClass}`, n = `yAxis${this.graphClass}`, r = `rough-${s}`, a = `rough-${n}`;
    x(`.${s}`).selectAll("path.domain").each(function(h, o) {
      const l = x(this).node().getAttribute("d"), u = e.path(l, { stroke: "black", fillStyle: "hachure" });
      u.setAttribute("class", r), t.appendChild(u);
    }), z(`.${r}`).attr("transform", `translate(0, ${this.height})`), x(`.${n}`).selectAll("path.domain").each(function(h, o) {
      const l = x(this).node().getAttribute("d"), u = e.path(l, { stroke: "black", fillStyle: "hachure" });
      u.setAttribute("class", a), t.appendChild(u);
    });
  }
  setTitle(t) {
    this.svg.append("text").attr("x", this.width / 2).attr("y", 0 - this.margin.top / 2).attr("class", "title").attr("text-anchor", "middle").style("font-size", this.titleFontSize === void 0 ? `${Math.min(40, Math.min(this.width, this.height) / 5)}px` : this.titleFontSize).style("font-family", this.fontFamily).style("opacity", 0.8).text(t);
  }
  addInteraction() {
    z(this.interactionG).data(this.dataFormat === "file" ? this.data : this.data.values).append("rect").attr("x", 0).attr("y", (s, n) => this.dataFormat === "file" ? this.yScale(s[this.labels]) : this.yScale(this.data[this.labels][n])).attr("width", (s, n) => this.dataFormat === "file" ? this.xScale(+s[this.values]) : this.xScale(this.data[this.values][n])).attr("height", this.yScale.bandwidth()).attr("fill", "transparent");
    const t = x(this.el).append("div").style("opacity", 0).attr("class", "tooltip").style("position", "absolute").style("background-color", "white").style("border", "solid").style("border-width", "1px").style("border-radius", "5px").style("padding", "3px").style("font-family", this.fontFamily).style("font-size", this.tooltipFontSize).style("pointer-events", "none"), e = this;
    z(this.interactionG).on("mouseover", function() {
      t.style("opacity", 1), x(this).select("path").style("stroke", e.highlight), x(this).selectAll("path:nth-child(2)").style("stroke-width", e.strokeWidth + 1.2);
    }), z(this.interactionG).on("mouseout", function() {
      t.style("opacity", 0), x(this).select("path").style("stroke", e.color), x(this).selectAll("path:nth-child(2)").style("stroke-width", e.strokeWidth);
    }), z(this.interactionG).on("mousemove", function(s) {
      const n = x(this).attr("attrX"), r = x(this).attr("attrY"), a = kt(this);
      t.html(`<b>${n}</b>: ${r}`).style("opacity", 0.95).style("transform", `translate(${a[0] + e.margin.left}px, 
              ${a[1] - (e.height + e.margin.top + e.margin.bottom / 2)}px)`);
    });
  }
  initRoughObjects() {
    this.roughSvg = document.getElementById(this.roughId), this.rcAxis = D(this.roughSvg, { options: { strokeWidth: this.axisStrokeWidth, roughness: this.axisRoughness } }), this.rc = D(this.roughSvg, { options: { fill: this.color, stroke: this.stroke === "none" ? void 0 : this.stroke, strokeWidth: this.innerStrokeWidth, roughness: this.roughness, bowing: this.bowing, fillStyle: this.fillStyle } });
  }
  drawFromObject() {
    this.initRoughObjects(), this.addScales(), this.addAxes(), this.makeAxesRough(this.roughSvg, this.rcAxis), this.addLabels(), this.data.values.forEach((t, e) => {
      const s = this.rc.rectangle(0, this.yScale(this.data[this.labels][e]), this.xScale(t), this.yScale.bandwidth(), { simplification: this.simplification, fillWeight: this.fillWeight }), n = this.roughSvg.appendChild(s);
      n.setAttribute("class", this.graphClass), n.setAttribute("attrX", this.data[this.labels][e]), n.setAttribute("attrY", +t);
    }), z(this.interactionG).selectAll("path:nth-child(2)").style("stroke-width", this.strokeWidth), this.interactive === !0 && this.addInteraction();
  }
  drawFromFile() {
    this.initRoughObjects(), this.addScales(), this.addAxes(), this.makeAxesRough(this.roughSvg, this.rcAxis), this.addLabels(), this.data.forEach((t) => {
      const e = this.rc.rectangle(0, this.yScale(t[this.labels]), this.xScale(+t[this.values]), this.yScale.bandwidth(), { simplification: this.simplification, fillWeight: this.fillWeight }), s = this.roughSvg.appendChild(e);
      s.setAttribute("class", this.graphClass), s.setAttribute("attrX", t[this.labels]), s.setAttribute("attrY", +t[this.values]);
    }), z(this.interactionG).selectAll("path:nth-child(2)").style("stroke-width", this.strokeWidth), this.interactive === !0 && this.addInteraction();
  }
}
var $i = Math.PI, te = 2 * $i, At = 1e-6, Yr = te - At;
function ie() {
  this._x0 = this._y0 = this._x1 = this._y1 = null, this._ = "";
}
function ue() {
  return new ie();
}
function Y(i) {
  return function() {
    return i;
  };
}
ie.prototype = ue.prototype = { constructor: ie, moveTo: function(i, t) {
  this._ += "M" + (this._x0 = this._x1 = +i) + "," + (this._y0 = this._y1 = +t);
}, closePath: function() {
  this._x1 !== null && (this._x1 = this._x0, this._y1 = this._y0, this._ += "Z");
}, lineTo: function(i, t) {
  this._ += "L" + (this._x1 = +i) + "," + (this._y1 = +t);
}, quadraticCurveTo: function(i, t, e, s) {
  this._ += "Q" + +i + "," + +t + "," + (this._x1 = +e) + "," + (this._y1 = +s);
}, bezierCurveTo: function(i, t, e, s, n, r) {
  this._ += "C" + +i + "," + +t + "," + +e + "," + +s + "," + (this._x1 = +n) + "," + (this._y1 = +r);
}, arcTo: function(i, t, e, s, n) {
  i = +i, t = +t, e = +e, s = +s, n = +n;
  var r = this._x1, a = this._y1, h = e - i, o = s - t, l = r - i, u = a - t, d = l * l + u * u;
  if (n < 0)
    throw new Error("negative radius: " + n);
  if (this._x1 === null)
    this._ += "M" + (this._x1 = i) + "," + (this._y1 = t);
  else if (d > At)
    if (Math.abs(u * h - o * l) > At && n) {
      var c = e - r, g = s - a, p = h * h + o * o, f = c * c + g * g, y = Math.sqrt(p), m = Math.sqrt(d), A = n * Math.tan(($i - Math.acos((p + d - f) / (2 * y * m))) / 2), b = A / m, S = A / y;
      Math.abs(b - 1) > At && (this._ += "L" + (i + b * l) + "," + (t + b * u)), this._ += "A" + n + "," + n + ",0,0," + +(u * c > l * g) + "," + (this._x1 = i + S * h) + "," + (this._y1 = t + S * o);
    } else
      this._ += "L" + (this._x1 = i) + "," + (this._y1 = t);
}, arc: function(i, t, e, s, n, r) {
  i = +i, t = +t, r = !!r;
  var a = (e = +e) * Math.cos(s), h = e * Math.sin(s), o = i + a, l = t + h, u = 1 ^ r, d = r ? s - n : n - s;
  if (e < 0)
    throw new Error("negative radius: " + e);
  this._x1 === null ? this._ += "M" + o + "," + l : (Math.abs(this._x1 - o) > At || Math.abs(this._y1 - l) > At) && (this._ += "L" + o + "," + l), e && (d < 0 && (d = d % te + te), d > Yr ? this._ += "A" + e + "," + e + ",0,1," + u + "," + (i - a) + "," + (t - h) + "A" + e + "," + e + ",0,1," + u + "," + (this._x1 = o) + "," + (this._y1 = l) : d > At && (this._ += "A" + e + "," + e + ",0," + +(d >= $i) + "," + u + "," + (this._x1 = i + e * Math.cos(n)) + "," + (this._y1 = t + e * Math.sin(n))));
}, rect: function(i, t, e, s) {
  this._ += "M" + (this._x0 = this._x1 = +i) + "," + (this._y0 = this._y1 = +t) + "h" + +e + "v" + +s + "h" + -e + "Z";
}, toString: function() {
  return this._;
} };
var rs = Math.abs, N = Math.atan2, pt = Math.cos, Hr = Math.max, Gi = Math.min, nt = Math.sin, Rt = Math.sqrt, _ = 1e-12, It = Math.PI, di = It / 2, ii = 2 * It;
function hs(i) {
  return i >= 1 ? di : i <= -1 ? -di : Math.asin(i);
}
function Dr(i) {
  return i.innerRadius;
}
function Tr(i) {
  return i.outerRadius;
}
function qr(i) {
  return i.startAngle;
}
function Nr(i) {
  return i.endAngle;
}
function Xr(i) {
  return i && i.padAngle;
}
function _t(i, t, e, s, n, r, a) {
  var h = i - e, o = t - s, l = (a ? r : -r) / Rt(h * h + o * o), u = l * o, d = -l * h, c = i + u, g = t + d, p = e + u, f = s + d, y = (c + p) / 2, m = (g + f) / 2, A = p - c, b = f - g, S = A * A + b * b, w = n - r, v = c * f - p * g, M = (b < 0 ? -1 : 1) * Rt(Hr(0, w * w * S - v * v)), j = (v * b - A * M) / S, C = (-v * A - b * M) / S, P = (v * b + A * M) / S, F = (-v * A + b * M) / S, k = j - y, R = C - m, O = P - y, I = F - m;
  return k * k + R * R > O * O + I * I && (j = P, C = F), { cx: j, cy: C, x01: -u, y01: -d, x11: j * (n / w - 1), y11: C * (n / w - 1) };
}
function gi() {
  var i = Dr, t = Tr, e = Y(0), s = null, n = qr, r = Nr, a = Xr, h = null;
  function o() {
    var l, u, d = +i.apply(this, arguments), c = +t.apply(this, arguments), g = n.apply(this, arguments) - di, p = r.apply(this, arguments) - di, f = rs(p - g), y = p > g;
    if (h || (h = l = ue()), c < d && (u = c, c = d, d = u), c > _)
      if (f > ii - _)
        h.moveTo(c * pt(g), c * nt(g)), h.arc(0, 0, c, g, p, !y), d > _ && (h.moveTo(d * pt(p), d * nt(p)), h.arc(0, 0, d, p, g, y));
      else {
        var m, A, b = g, S = p, w = g, v = p, M = f, j = f, C = a.apply(this, arguments) / 2, P = C > _ && (s ? +s.apply(this, arguments) : Rt(d * d + c * c)), F = Gi(rs(c - d) / 2, +e.apply(this, arguments)), k = F, R = F;
        if (P > _) {
          var O = hs(P / d * nt(C)), I = hs(P / c * nt(C));
          (M -= 2 * O) > _ ? (w += O *= y ? 1 : -1, v -= O) : (M = 0, w = v = (g + p) / 2), (j -= 2 * I) > _ ? (b += I *= y ? 1 : -1, S -= I) : (j = 0, b = S = (g + p) / 2);
        }
        var L = c * pt(b), E = c * nt(b), T = d * pt(v), q = d * nt(v);
        if (F > _) {
          var G, H = c * pt(S), et = c * nt(S), gt = d * pt(w), B = d * nt(w);
          if (f < It && (G = function(ft, Si, tn, en, fe, pe, sn, nn) {
            var me = tn - ft, Ae = en - Si, ye = sn - fe, xe = nn - pe, Ft = xe * me - ye * Ae;
            if (!(Ft * Ft < _))
              return [ft + (Ft = (ye * (Si - pe) - xe * (ft - fe)) / Ft) * me, Si + Ft * Ae];
          }(L, E, gt, B, H, et, T, q))) {
            var V = L - G[0], vi = E - G[1], bi = H - G[0], ki = et - G[1], de = 1 / nt(function(ft) {
              return ft > 1 ? 0 : ft < -1 ? It : Math.acos(ft);
            }((V * bi + vi * ki) / (Rt(V * V + vi * vi) * Rt(bi * bi + ki * ki))) / 2), ge = Rt(G[0] * G[0] + G[1] * G[1]);
            k = Gi(F, (d - ge) / (de - 1)), R = Gi(F, (c - ge) / (de + 1));
          }
        }
        j > _ ? R > _ ? (m = _t(gt, B, L, E, c, R, y), A = _t(H, et, T, q, c, R, y), h.moveTo(m.cx + m.x01, m.cy + m.y01), R < F ? h.arc(m.cx, m.cy, R, N(m.y01, m.x01), N(A.y01, A.x01), !y) : (h.arc(m.cx, m.cy, R, N(m.y01, m.x01), N(m.y11, m.x11), !y), h.arc(0, 0, c, N(m.cy + m.y11, m.cx + m.x11), N(A.cy + A.y11, A.cx + A.x11), !y), h.arc(A.cx, A.cy, R, N(A.y11, A.x11), N(A.y01, A.x01), !y))) : (h.moveTo(L, E), h.arc(0, 0, c, b, S, !y)) : h.moveTo(L, E), d > _ && M > _ ? k > _ ? (m = _t(T, q, H, et, d, -k, y), A = _t(L, E, gt, B, d, -k, y), h.lineTo(m.cx + m.x01, m.cy + m.y01), k < F ? h.arc(m.cx, m.cy, k, N(m.y01, m.x01), N(A.y01, A.x01), !y) : (h.arc(m.cx, m.cy, k, N(m.y01, m.x01), N(m.y11, m.x11), !y), h.arc(0, 0, d, N(m.cy + m.y11, m.cx + m.x11), N(A.cy + A.y11, A.cx + A.x11), y), h.arc(A.cx, A.cy, k, N(A.y11, A.x11), N(A.y01, A.x01), !y))) : h.arc(0, 0, d, v, w, y) : h.lineTo(T, q);
      }
    else
      h.moveTo(0, 0);
    if (h.closePath(), l)
      return h = null, l + "" || null;
  }
  return o.centroid = function() {
    var l = (+i.apply(this, arguments) + +t.apply(this, arguments)) / 2, u = (+n.apply(this, arguments) + +r.apply(this, arguments)) / 2 - It / 2;
    return [pt(u) * l, nt(u) * l];
  }, o.innerRadius = function(l) {
    return arguments.length ? (i = typeof l == "function" ? l : Y(+l), o) : i;
  }, o.outerRadius = function(l) {
    return arguments.length ? (t = typeof l == "function" ? l : Y(+l), o) : t;
  }, o.cornerRadius = function(l) {
    return arguments.length ? (e = typeof l == "function" ? l : Y(+l), o) : e;
  }, o.padRadius = function(l) {
    return arguments.length ? (s = l == null ? null : typeof l == "function" ? l : Y(+l), o) : s;
  }, o.startAngle = function(l) {
    return arguments.length ? (n = typeof l == "function" ? l : Y(+l), o) : n;
  }, o.endAngle = function(l) {
    return arguments.length ? (r = typeof l == "function" ? l : Y(+l), o) : r;
  }, o.padAngle = function(l) {
    return arguments.length ? (a = typeof l == "function" ? l : Y(+l), o) : a;
  }, o.context = function(l) {
    return arguments.length ? (h = l ?? null, o) : h;
  }, o;
}
function qs(i) {
  this._context = i;
}
function Kr(i) {
  return new qs(i);
}
function Qr(i) {
  return i[0];
}
function Ur(i) {
  return i[1];
}
function Jr(i, t) {
  return t < i ? -1 : t > i ? 1 : t >= i ? 0 : NaN;
}
function _r(i) {
  return i;
}
function fi() {
  var i = _r, t = Jr, e = null, s = Y(0), n = Y(ii), r = Y(0);
  function a(h) {
    var o, l, u, d, c, g = h.length, p = 0, f = new Array(g), y = new Array(g), m = +s.apply(this, arguments), A = Math.min(ii, Math.max(-ii, n.apply(this, arguments) - m)), b = Math.min(Math.abs(A) / g, r.apply(this, arguments)), S = b * (A < 0 ? -1 : 1);
    for (o = 0; o < g; ++o)
      (c = y[f[o] = o] = +i(h[o], o, h)) > 0 && (p += c);
    for (t != null ? f.sort(function(w, v) {
      return t(y[w], y[v]);
    }) : e != null && f.sort(function(w, v) {
      return e(h[w], h[v]);
    }), o = 0, u = p ? (A - g * S) / p : 0; o < g; ++o, m = d)
      l = f[o], d = m + ((c = y[l]) > 0 ? c * u : 0) + S, y[l] = { data: h[l], index: o, value: c, startAngle: m, endAngle: d, padAngle: b };
    return y;
  }
  return a.value = function(h) {
    return arguments.length ? (i = typeof h == "function" ? h : Y(+h), a) : i;
  }, a.sortValues = function(h) {
    return arguments.length ? (t = h, e = null, a) : t;
  }, a.sort = function(h) {
    return arguments.length ? (e = h, t = null, a) : e;
  }, a.startAngle = function(h) {
    return arguments.length ? (s = typeof h == "function" ? h : Y(+h), a) : s;
  }, a.endAngle = function(h) {
    return arguments.length ? (n = typeof h == "function" ? h : Y(+h), a) : n;
  }, a.padAngle = function(h) {
    return arguments.length ? (r = typeof h == "function" ? h : Y(+h), a) : r;
  }, a;
}
qs.prototype = { areaStart: function() {
  this._line = 0;
}, areaEnd: function() {
  this._line = NaN;
}, lineStart: function() {
  this._point = 0;
}, lineEnd: function() {
  (this._line || this._line !== 0 && this._point === 1) && this._context.closePath(), this._line = 1 - this._line;
}, point: function(i, t) {
  switch (i = +i, t = +t, this._point) {
    case 0:
      this._point = 1, this._line ? this._context.lineTo(i, t) : this._context.moveTo(i, t);
      break;
    case 1:
      this._point = 2;
    default:
      this._context.lineTo(i, t);
  }
} };
const xt = ["coral", "skyblue", "#66c2a5", "tan", "#8da0cb", "#e78ac3", "#a6d854", "#ffd92f", "coral", "skyblue", "tan", "orange"], dt = (i, t, e, s, n) => {
  i.svg.append("svg").attr("x", i.legendPosition === "left" ? 5 : i.width - (e + 2)).attr("y", 0);
  const r = n === void 0 ? 5 - i.margin.left : n, a = i.rc.rectangle(i.legendPosition === "left" ? r : i.width + i.margin.right - 2 - e, -i.margin.top / 3, e, s, { fill: "white", fillWeight: 0.1, strokeWidth: 0.75, roughness: 2 }), h = i.roughSvg.appendChild(a), o = "rough" + i.el.substring(1, i.el.length);
  h.setAttribute("class", o), t.forEach((l, u) => {
    const d = x("." + o).append("g").attr("transform", `translate(
        ${i.legendPosition === "left" ? 5 : i.width - (e + 2)},
        0)`);
    d.append("rect").style("fill", i.colors[u]).attr("width", 20).attr("height", 8).attr("x", i.legendPosition === "left" ? r : i.margin.right + 5).attr("y", 6 + 11 * u - i.margin.top / 3), d.append("text").style("font-size", ".8rem").style("font-family", i.fontFamily).attr("x", i.legendPosition === "left" ? r + 25 : i.margin.right + 30).attr("y", 6 + 11 * u + 8 - i.margin.top / 3).text(l.text);
  });
}, Ii = "http://www.w3.org/2000/svg", as = "data-roughviz-slice-clip";
class ch extends lt {
  constructor(t) {
    super(t), this.margin = t.margin || { top: 50, right: 20, bottom: 10, left: 20 }, this.colors = t.colors || xt, this.highlight = t.highlight, this.roughness = at({ roughness: t.roughness, ceiling: 30 }), this.strokeWidth = t.strokeWidth || 0.75, this.innerStrokeWidth = t.innerStrokeWidth || 0.75, this.fillWeight = t.fillWeight || 0.85, this.labels = this.dataFormat === "object" ? "labels" : t.labels, this.values = this.dataFormat === "object" ? "values" : t.values, this.labels !== void 0 && this.values !== void 0 ? (this.legend = t.legend !== !1, this.legendPosition = t.legendPosition || "right", this.responsive = !0, this.boundRedraw = this.redraw.bind(this, t), this.initChartValues(t) && (this.resolveFont(), this.drawChart = this.resolveData(t.data), this.drawChart(), t.title !== "undefined" && this.setTitle(t.title), this.boundResizeHandler = this.resizeHandler.bind(this), window.addEventListener("resize", this.boundResizeHandler))) : console.log(`Error for ${this.el}: Must include labels and values when        instantiating Donut chart. Skipping chart.`);
  }
  resizeHandler() {
    x(this.el).node() ? this.responsive && this.boundRedraw() : this.boundResizeHandler && (window.removeEventListener("resize", this.boundResizeHandler), this.boundResizeHandler = null);
  }
  remove() {
    x(this.el).select("svg").remove();
  }
  redraw(t) {
    this.remove(), this.initChartValues(t) && (this.resolveFont(), this.drawChart = this.resolveData(t.data), this.drawChart(), t.title !== "undefined" && this.setTitle(t.title));
  }
  initChartValues(t) {
    this.roughness = t.roughness || this.roughness, this.stroke = t.stroke || this.stroke, this.strokeWidth = t.strokeWidth || this.strokeWidth, this.axisStrokeWidth = t.axisStrokeWidth || this.axisStrokeWidth, this.axisRoughness = t.axisRoughness || this.axisRoughness, this.innerStrokeWidth = t.innerStrokeWidth || this.innerStrokeWidth, this.fillWeight = t.fillWeight || this.fillWeight, this.fillStyle = t.fillStyle || this.fillStyle;
    const e = x(this.el).node();
    if (!e)
      return this.width = 0, this.height = 0, !1;
    const s = e.getBoundingClientRect(), n = s.width, r = s.height;
    return this.width = n - this.margin.left - this.margin.right, this.height = r - this.margin.top - this.margin.bottom, this.roughId = this.el + "_svg", this.graphClass = this.el.substring(1, this.el.length), this.interactionG = "g." + this.graphClass, this.radius = Math.min(this.width, this.height) / 2, this.setSvg(), !0;
  }
  resolveData(t) {
    return typeof t != "string" ? () => {
      this.data = t, this.drawFromObject();
    } : t.includes(".csv") ? () => {
      vt(t).then((e) => {
        this.data = e, this.drawFromFile();
      });
    } : t.includes(".tsv") ? () => {
      bt(t).then((e) => {
        this.data = e, this.drawFromFile();
      });
    } : t.includes(".json") ? () => {
      ks(t).then((e) => {
        this.data = e, this.drawFromFile();
      });
    } : void 0;
  }
  setTitle(t) {
    this.svg.append("text").attr("x", this.width / 2).attr("y", 0 - this.margin.top / 3).attr("class", "title").attr("text-anchor", "middle").style("font-size", this.titleFontSize === void 0 ? `${Math.min(40, Math.min(this.width, this.height) / 4)}px` : this.titleFontSize).style("font-family", this.fontFamily).style("opacity", 0.8).text(t);
  }
  addInteraction() {
    z(this.interactionG).append("g").attr("transform", `translate(${this.width / 2}, ${this.height / 2})`).data(this.dataFormat === "object" ? this.makePie(this.data[this.values]) : this.makePie(this.data)).append("path").attr("d", this.makeArc).attr("stroke-width", "0px").attr("fill", "transparent");
    const t = x(this.el).append("div").style("opacity", 0).attr("class", "tooltip").style("position", "absolute").style("background-color", "white").style("border", "solid").style("border-width", "1px").style("border-radius", "5px").style("padding", "3px").style("font-family", this.fontFamily).style("font-size", this.tooltipFontSize).style("pointer-events", "none"), e = this;
    let s;
    z(this.interactionG).on("mouseover", function() {
      t.style("opacity", 1), s = x(this).selectAll("path").style("stroke"), e.highlight === void 0 ? x(this).selectAll("path").style("opacity", 0.5) : x(this).selectAll("path").style("stroke", e.highlight);
    }), z(this.interactionG).on("mouseout", function() {
      t.style("opacity", 0), x(this).selectAll("path").style("stroke", s), x(this).selectAll("path").style("opacity", 1);
    }), z(this.interactionG).on("mousemove", function(n) {
      const r = x(this).attr("attrX"), a = x(this).attr("attrY"), h = kt(this);
      t.html(`<b>${r}</b>: ${a}`).style("opacity", 0.95).style("transform", `translate(${h[0] + e.margin.left}px, 
              ${h[1] - (e.height + e.margin.top + e.margin.bottom / 2)}px)`);
    });
  }
  initRoughObjects() {
    this.roughSvg = document.getElementById(this.roughId), this.rcAxis = D(this.roughSvg, { options: { strokeWidth: this.strokeWidth >= 3 ? 3 : this.strokeWidth } }), this.rc = D(this.roughSvg, { options: { fill: this.color, strokeWidth: this.innerStrokeWidth, roughness: this.roughness, bowing: this.bowing, fillStyle: this.fillStyle, fillWeight: this.fillWeight } });
  }
  getSliceClipKey() {
    return this.roughId.replace(/[^a-zA-Z0-9_-]/g, "_");
  }
  getSliceClipDefs() {
    const t = this.roughSvg && this.roughSvg.ownerSVGElement;
    if (!t)
      return null;
    let e = t.querySelector("defs");
    return e || (e = document.createElementNS(Ii, "defs"), t.insertBefore(e, t.firstChild)), e;
  }
  resetSliceClipPaths() {
    const t = this.getSliceClipDefs();
    if (!t)
      return;
    const e = this.getSliceClipKey();
    t.querySelectorAll(`[${as}="${e}"]`).forEach((s) => s.remove());
  }
  applySliceClip(t, e, s) {
    const n = this.getSliceClipDefs();
    if (!n || !t)
      return;
    const r = this.makeArc(e);
    if (!r)
      return;
    const a = this.getSliceClipKey(), h = `${a}_slice_clip_${s}`, o = document.createElementNS(Ii, "clipPath");
    o.setAttribute("id", h), o.setAttribute(as, a);
    const l = document.createElementNS(Ii, "path");
    l.setAttribute("d", r), l.setAttribute("transform", `translate(${this.width / 2}, ${this.height / 2})`), o.appendChild(l), n.appendChild(o), x(t).selectAll("path").attr("clip-path", `url(#${h})`);
  }
  drawFromObject() {
    this.initRoughObjects(), this.makePie = fi(), this.makeArc = gi().innerRadius(0).outerRadius(this.radius), this.arcs = this.makePie(this.data[this.values]), this.resetSliceClipPaths(), this.arcs.forEach((r, a) => {
      if (r.value !== 0) {
        const h = this.rc.arc(this.width / 2, this.height / 2, 2 * this.radius, 2 * this.radius, r.startAngle - Math.PI / 2, r.endAngle - Math.PI / 2, !0, { fill: this.colors[a], stroke: this.colors[a] });
        h.setAttribute("class", this.graphClass);
        const o = this.roughSvg.appendChild(h);
        o.setAttribute("attrY", this.data[this.values][a]), o.setAttribute("attrX", this.data[this.labels][a]), this.applySliceClip(o, r, a);
      }
    });
    const t = this.rc.circle(this.width / 2, this.height / 2, this.radius, { fill: "white", strokeWidth: 0.05, fillWeight: 10, fillStyle: "solid" });
    this.roughSvg.appendChild(t), z(this.interactionG).selectAll("path:nth-child(2)").style("stroke-width", this.strokeWidth);
    const e = this.data.labels.map((r, a) => ({ color: this.colors[a], text: r })), s = 6 * e.reduce((r, a) => r > a.text.length ? r : a.text.length, 0) + 35, n = 11 * e.length + 8;
    this.legend === !0 && dt(this, e, s, n), this.interactive === !0 && this.addInteraction();
  }
  drawFromFile() {
    this.initRoughObjects(), this.makePie = fi().value((a) => a[this.values]).sort(null);
    const t = [];
    this.makeArc = gi().innerRadius(0).outerRadius(this.radius), this.arcs = this.makePie(this.data), this.resetSliceClipPaths(), this.arcs.forEach((a, h) => {
      if (a.value !== 0) {
        const o = this.rc.arc(this.width / 2, this.height / 2, 2 * this.radius, 2 * this.radius, a.startAngle - Math.PI / 2, a.endAngle - Math.PI / 2, !0, { fill: this.colors[h], stroke: this.colors[h] });
        o.setAttribute("class", this.graphClass);
        const l = this.roughSvg.appendChild(o);
        l.setAttribute("attrY", a.data[this.values]), l.setAttribute("attrX", a.data[this.labels]), this.applySliceClip(l, a, h);
      }
      t.push(a.data[this.labels]);
    });
    const e = this.rc.circle(this.width / 2, this.height / 2, this.radius, { fill: "white", strokeWidth: 0.05, fillWeight: 10, fillStyle: "solid" });
    this.roughSvg.appendChild(e), z(this.interactionG).selectAll("path:nth-child(2)").style("stroke-width", this.strokeWidth);
    const s = t.map((a, h) => ({ color: this.colors[h], text: a })), n = 6 * s.reduce((a, h) => a > h.text.length ? a : h.text.length, 0) + 35, r = 11 * s.length + 8;
    this.legend === !0 && dt(this, s, n, r), this.interactive === !0 && this.addInteraction();
  }
}
class dh extends lt {
  constructor(t) {
    super(t), this.margin = t.margin || { top: 50, right: 20, bottom: 50, left: 100 }, this.roughness = at({ roughness: t.roughness, defaultValue: 2.2 }), this.axisStrokeWidth = t.axisStrokeWidth || 0.5, this.axisRoughness = t.axisRoughness || 0.5, this.stroke = t.stroke || "black", this.fillWeight = t.fillWeight || 0.5, this.colors = t.colors, this.strokeWidth = t.strokeWidth || 1, this.axisFontSize = t.axisFontSize, this.x = t.x, this.y = this.dataFormat === "object" ? "y" : t.y, this.xValueFormat = t.xValueFormat, this.yValueFormat = t.yValueFormat, this.legend = t.legend !== !1, this.legendPosition = t.legendPosition || "right", this.circle = t.circle !== !1, this.circleRadius = t.circleRadius || 10, this.circleRoughness = at({ roughness: t.circleRoughness, defaultValue: 2 }), this.xLabel = t.xLabel || "", this.yLabel = t.yLabel || "", this.labelFontSize = t.labelFontSize || "1rem", this.dataFormat === "file" && (this.dataSources = [], this.yKeys = Object.keys(t).filter((e) => /y/.test(e)), this.yKeys.map((e, s) => {
      e !== "yLabel" && this.dataSources.push(t[e]);
    })), this.responsive = !0, this.boundRedraw = this.redraw.bind(this, t), this.initChartValues(t) && (this.resolveFont(), this.drawChart = this.resolveData(t.data), this.drawChart(), t.title !== "undefined" && this.setTitle(t.title), this.boundResizeHandler = this.resizeHandler.bind(this), window.addEventListener("resize", this.boundResizeHandler));
  }
  resizeHandler() {
    x(this.el).node() ? this.responsive && this.boundRedraw() : this.boundResizeHandler && (window.removeEventListener("resize", this.boundResizeHandler), this.boundResizeHandler = null);
  }
  remove() {
    x(this.el).select("svg").remove();
  }
  redraw(t) {
    this.remove(), this.initChartValues(t) && (this.resolveFont(), this.drawChart = this.resolveData(t.data), this.drawChart(), t.title !== "undefined" && this.setTitle(t.title));
  }
  initChartValues(t) {
    this.roughness = t.roughness || this.roughness, this.stroke = t.stroke || this.stroke, this.strokeWidth = t.strokeWidth || this.strokeWidth, this.axisStrokeWidth = t.axisStrokeWidth || this.axisStrokeWidth, this.axisRoughness = t.axisRoughness || this.axisRoughness, this.innerStrokeWidth = t.innerStrokeWidth || this.innerStrokeWidth, this.fillWeight = t.fillWeight || this.fillWeight, this.fillStyle = t.fillStyle || this.fillStyle;
    const e = x(this.el).node();
    if (!e)
      return this.width = 0, this.height = 0, !1;
    const s = e.getBoundingClientRect(), n = s.width, r = s.height;
    return this.width = n - this.margin.left - this.margin.right, this.height = r - this.margin.top - this.margin.bottom, this.roughId = this.el + "_svg", this.graphClass = this.el.substring(1, this.el.length), this.interactionG = "g." + this.graphClass, this.setSvg(), !0;
  }
  resolveData(t) {
    return typeof t != "string" ? () => {
      this.data = t, this.drawFromObject();
    } : t.includes(".csv") ? () => {
      vt(t).then((e) => {
        this.data = e, this.drawFromFile();
      });
    } : t.includes(".tsv") ? () => {
      bt(t).then((e) => {
        this.data = e, this.drawFromFile();
      });
    } : void 0;
  }
  addScales() {
    let t, e;
    if (this.dataFormat !== "file")
      t = ((r) => {
        const a = Object.keys(r).map((l) => st(r[l])), h = ut(a, (l) => l[0]), o = Q(a, (l) => l[1]);
        return [h, o];
      })(this.data);
    else {
      const r = this.dataSources.map((o) => st(this.data, (l) => +l[o])), a = ut(r, (o) => o[0]), h = Q(r, (o) => o[1]);
      t = [a, h];
    }
    if (this.x === void 0) {
      const r = Q(Object.keys(this.data).map((a) => this.data[a].length));
      e = this.dataFormat === "file" ? [0, this.data.length] : [0, r];
    } else
      e = st(this.x);
    const s = t, n = s[1] - s[0];
    this.xScale = this.x === void 0 ? je().range([0, this.width]).domain([...Array(e[1]).keys()]) : je().range([0, this.width]).domain(this.x), this.yScale = Z().range([this.height, 0]).domain([0, s[1] + 0.05 * n]);
  }
  addLabels() {
    this.xLabel !== "" && this.svg.append("text").attr("x", this.width / 2).attr("y", this.height + this.margin.bottom / 1.3).attr("dx", "1em").attr("class", "labelText").style("text-anchor", "middle").style("font-family", this.fontFamily).style("font-size", this.labelFontSize).text(this.xLabel), this.yLabel !== "" && this.svg.append("text").attr("transform", "rotate(-90)").attr("y", 0 - this.margin.left / 2).attr("x", 0 - this.height / 2).attr("dy", "1em").attr("class", "labelText").style("text-anchor", "middle").style("font-family", this.fontFamily).style("font-size", this.labelFontSize).text(this.yLabel);
  }
  addAxes() {
    const t = Ht(this.xScale).tickSize(0).tickFormat((s) => this.xValueFormat ? ot(this.xValueFormat)(s) : s), e = Dt(this.yScale).tickSize(0).tickFormat((s) => this.yValueFormat ? ot(this.yValueFormat)(s) : s);
    this.svg.append("g").attr("transform", "translate(0," + this.height + ")").call(t).attr("class", `xAxis${this.graphClass}`).selectAll("text").attr("transform", "translate(-10, 0)rotate(-45)").style("text-anchor", "end").style("font-family", this.fontFamily).style("font-size", this.axisFontSize === void 0 ? `${Math.min(0.95, Math.min(this.width, this.height) / 140)}rem` : this.axisFontSize), this.svg.append("g").call(e).attr("class", `yAxis${this.graphClass}`).selectAll("text").style("font-family", this.fontFamily).style("font-size", this.axisFontSize === void 0 ? `${Math.min(0.95, Math.min(this.width, this.height) / 140)}rem` : this.axisFontSize), z("path.domain").attr("stroke", "transparent"), z("g.tick").style("opacity", 1);
  }
  makeAxesRough(t, e) {
    const s = `xAxis${this.graphClass}`, n = `yAxis${this.graphClass}`, r = `rough-${s}`, a = `rough-${n}`;
    x(`.${s}`).selectAll("path.domain").each(function(h, o) {
      const l = x(this).node().getAttribute("d"), u = e.path(l, { stroke: "black", fillStyle: "hachure" });
      u.setAttribute("class", r), t.appendChild(u);
    }), z(`.${r}`).attr("transform", `translate(0, ${this.height})`), x(`.${n}`).selectAll("path.domain").each(function(h, o) {
      const l = x(this).node().getAttribute("d"), u = e.path(l, { stroke: "black", fillStyle: "hachure" });
      u.setAttribute("class", a), t.appendChild(u);
    });
  }
  setTitle(t) {
    this.svg.append("text").attr("x", this.width / 2).attr("y", 0 - this.margin.top / 2).attr("text-anchor", "middle").style("font-size", this.titleFontSize === void 0 ? `${Math.min(20, Math.min(this.width, this.height) / 4)}px` : this.titleFontSize).style("font-family", this.fontFamily).style("opacity", 0.8).text(t);
  }
  addInteraction() {
    const t = this;
    this.chartScreen = this.svg.append("g").attr("pointer-events", "all"), this.dataSources.map((e, s) => {
      const n = (this.dataFormat === "file" ? this.data : this.data[e]).map((o, l) => this.x === void 0 ? [this.xScale(l), this.yScale(o[e])] : [this.xScale(this.x[l]), this.yScale(+o[e])]), r = n.filter((o) => o[0] !== void 0), a = function() {
        var o = Qr, l = Ur, u = Y(!0), d = null, c = Kr, g = null;
        function p(f) {
          var y, m, A, b = f.length, S = !1;
          for (d == null && (g = c(A = ue())), y = 0; y <= b; ++y)
            !(y < b && u(m = f[y], y, f)) === S && ((S = !S) ? g.lineStart() : g.lineEnd()), S && g.point(+o(m, y, f), +l(m, y, f));
          if (A)
            return g = null, A + "" || null;
        }
        return p.x = function(f) {
          return arguments.length ? (o = typeof f == "function" ? f : Y(+f), p) : o;
        }, p.y = function(f) {
          return arguments.length ? (l = typeof f == "function" ? f : Y(+f), p) : l;
        }, p.defined = function(f) {
          return arguments.length ? (u = typeof f == "function" ? f : Y(!!f), p) : u;
        }, p.curve = function(f) {
          return arguments.length ? (c = f, d != null && (g = c(d)), p) : c;
        }, p.context = function(f) {
          return arguments.length ? (f == null ? d = g = null : g = c(d = f), p) : d;
        }, p;
      }().x((o) => o[0]).y((o) => o[1]);
      this.svg.append("path").datum(r).attr("fill", "none").attr("stroke", "blue").attr("stroke-width", 1.5).attr("d", a).attr("visibility", "hidden");
      const h = e + "class";
      this.svg.append("g").attr("class", h + "text").append("text").style("font-size", this.tooltipFontSize).style("opacity", 0).style("font-family", this.fontFamily).attr("text-anchor", "middle").attr("alignment-baseline", "middle");
    }), this.chartScreen.append("rect").attr("width", this.width).attr("height", this.height).attr("fill", "none").on("mousemove", function(e) {
      const s = kt(this)[0], n = t.xScale.domain(), r = t.xScale.range(), a = ys(r[0], r[1] + 1, t.xScale.step()), h = As(a, s), o = n[h];
      t.dataSources.map((l, u) => {
        const d = t.dataFormat === "file" ? t.x === void 0 ? t.data[o] : t.data[h] : t.data[l][h], c = "." + l + "classtext";
        t.dataFormat === "file" ? x(c).selectAll("text").style("opacity", 1).html(t.x === void 0 ? `(${h},${d[l]})` : `(${t.x[h]}, ${d[l]})`).attr("x", t.x === void 0 ? t.xScale(h) : t.xScale(t.x[h])).attr("y", t.yScale(d[l]) - 6) : x(c).selectAll("text").style("opacity", 1).html(t.x === void 0 ? `(${h}, ${d})` : `(${t.x[h]}, ${d})`).attr("x", t.x === void 0 ? t.xScale(h) : t.xScale(t.x[h])).attr("y", t.yScale(d));
      });
    }).on("mouseout", () => {
      t.dataSources.map((e) => {
        x("." + e + "classtext").selectAll("text").style("opacity", 0);
      });
    });
  }
  initRoughObjects() {
    this.roughSvg = document.getElementById(this.roughId), this.rcAxis = D(this.roughSvg, { options: { strokeWidth: this.axisStrokeWidth, roughness: this.axisRoughness } }), this.rc = D(this.roughSvg, { options: { stroke: this.stroke === "none" ? void 0 : this.stroke, strokeWidth: this.strokeWidth, roughness: this.roughness, bowing: this.bowing, fillStyle: this.fillStyle } });
  }
  drawFromObject() {
    const t = this;
    this.colors === void 0 && (this.colors = xt), this.dataSources = Object.keys(this.data), this.initRoughObjects(), this.addScales(), this.dataSources.map((r, a) => {
      const h = this.data[r].map((u, d) => this.x === void 0 ? [this.xScale(d), this.yScale(+u)] : [this.xScale(this.x[d]), this.yScale(u)]), o = h.filter((u) => u[0] !== void 0), l = this.rc.curve(o, { stroke: t.colors.length === 1 ? t.colors[0] : t.colors[a], roughness: t.roughness, bowing: t.bowing });
      this.roughSvg.appendChild(l).setAttribute("class", this.graphClass), this.circle === !0 && h.forEach((u, d) => {
        const c = this.rc.circle(u[0], u[1], this.circleRadius, { stroke: this.colors[a], fill: this.colors[a], fillStyle: "solid", strokeWidth: 1, roughness: this.circleRoughness });
        this.roughSvg.appendChild(c);
      });
    });
    const e = this.dataSources.map((r, a) => ({ color: this.colors[a], text: r })), s = 6 * e.reduce((r, a) => r > a.text.length ? r : a.text.length, 0) + 35, n = 11 * e.length + 8;
    this.legend === !0 && dt(this, e, s, n, 2), this.addAxes(), this.addLabels(), this.makeAxesRough(this.roughSvg, this.rcAxis), this.interactive === !0 && this.addInteraction();
  }
  drawFromFile() {
    this.colors === void 0 && (this.colors = xt), this.initRoughObjects(), this.addScales(), this.dataSources.map((n, r) => {
      const a = this.data.map((l, u) => this.x === void 0 ? [this.xScale(u), this.yScale(l[n])] : [this.xScale(this.x[u]), this.yScale(+l[n])]), h = a.filter((l) => l[0] !== void 0), o = this.rc.curve(h, { stroke: this.colors[r], strokeWidth: this.strokeWidth, roughness: 1, bowing: 10 });
      this.roughSvg.appendChild(o), this.circle === !0 && h.forEach((l, u) => {
        const d = this.rc.circle(l[0], l[1], this.circleRadius, { stroke: this.colors[r], fill: this.colors[r], fillStyle: "solid", strokeWidth: 1, roughness: this.circleRoughness });
        this.roughSvg.appendChild(d);
      });
    });
    const t = this.dataSources.map((n, r) => ({ color: this.colors[r], text: n })), e = 6 * t.reduce((n, r) => n > r.text.length ? n : r.text.length, 0) + 35, s = 11 * t.length + 8;
    this.legend === !0 && dt(this, t, e, s, 2), this.addAxes(), this.addLabels(), this.makeAxesRough(this.roughSvg, this.rcAxis), this.interactive === !0 && this.addInteraction();
  }
}
function Ns(i, t) {
  var e, s = 1;
  function n() {
    var r, a, h = e.length, o = 0, l = 0;
    for (r = 0; r < h; ++r)
      o += (a = e[r]).x, l += a.y;
    for (o = (o / h - i) * s, l = (l / h - t) * s, r = 0; r < h; ++r)
      (a = e[r]).x -= o, a.y -= l;
  }
  return i == null && (i = 0), t == null && (t = 0), n.initialize = function(r) {
    e = r;
  }, n.x = function(r) {
    return arguments.length ? (i = +r, n) : i;
  }, n.y = function(r) {
    return arguments.length ? (t = +r, n) : t;
  }, n.strength = function(r) {
    return arguments.length ? (s = +r, n) : s;
  }, n;
}
function os(i, t, e, s) {
  if (isNaN(t) || isNaN(e))
    return i;
  var n, r, a, h, o, l, u, d, c, g = i._root, p = { data: s }, f = i._x0, y = i._y0, m = i._x1, A = i._y1;
  if (!g)
    return i._root = p, i;
  for (; g.length; )
    if ((l = t >= (r = (f + m) / 2)) ? f = r : m = r, (u = e >= (a = (y + A) / 2)) ? y = a : A = a, n = g, !(g = g[d = u << 1 | l]))
      return n[d] = p, i;
  if (h = +i._x.call(null, g.data), o = +i._y.call(null, g.data), t === h && e === o)
    return p.next = g, n ? n[d] = p : i._root = p, i;
  do
    n = n ? n[d] = new Array(4) : i._root = new Array(4), (l = t >= (r = (f + m) / 2)) ? f = r : m = r, (u = e >= (a = (y + A) / 2)) ? y = a : A = a;
  while ((d = u << 1 | l) == (c = (o >= a) << 1 | h >= r));
  return n[c] = g, n[d] = p, i;
}
function X(i, t, e, s, n) {
  this.node = i, this.x0 = t, this.y0 = e, this.x1 = s, this.y1 = n;
}
function $r(i) {
  return i[0];
}
function th(i) {
  return i[1];
}
function Xs(i, t, e) {
  var s = new ce(t ?? $r, e ?? th, NaN, NaN, NaN, NaN);
  return i == null ? s : s.addAll(i);
}
function ce(i, t, e, s, n, r) {
  this._x = i, this._y = t, this._x0 = e, this._y0 = s, this._x1 = n, this._y1 = r, this._root = void 0;
}
function ls(i) {
  for (var t = { data: i.data }, e = t; i = i.next; )
    e = e.next = { data: i.data };
  return t;
}
var K = Xs.prototype = ce.prototype;
function Zt(i) {
  return function() {
    return i;
  };
}
function pi(i) {
  return 1e-6 * (i() - 0.5);
}
function ih(i) {
  return i.x + i.vx;
}
function eh(i) {
  return i.y + i.vy;
}
function Ks(i) {
  var t, e, s, n = 1, r = 1;
  function a() {
    for (var l, u, d, c, g, p, f, y = t.length, m = 0; m < r; ++m)
      for (u = Xs(t, ih, eh).visitAfter(h), l = 0; l < y; ++l)
        d = t[l], p = e[d.index], f = p * p, c = d.x + d.vx, g = d.y + d.vy, u.visit(A);
    function A(b, S, w, v, M) {
      var j = b.data, C = b.r, P = p + C;
      if (!j)
        return S > c + P || v < c - P || w > g + P || M < g - P;
      if (j.index > d.index) {
        var F = c - j.x - j.vx, k = g - j.y - j.vy, R = F * F + k * k;
        R < P * P && (F === 0 && (R += (F = pi(s)) * F), k === 0 && (R += (k = pi(s)) * k), R = (P - (R = Math.sqrt(R))) / R * n, d.vx += (F *= R) * (P = (C *= C) / (f + C)), d.vy += (k *= R) * P, j.vx -= F * (P = 1 - P), j.vy -= k * P);
      }
    }
  }
  function h(l) {
    if (l.data)
      return l.r = e[l.data.index];
    for (var u = l.r = 0; u < 4; ++u)
      l[u] && l[u].r > l.r && (l.r = l[u].r);
  }
  function o() {
    if (t) {
      var l, u, d = t.length;
      for (e = new Array(d), l = 0; l < d; ++l)
        u = t[l], e[u.index] = +i(u, l, t);
    }
  }
  return typeof i != "function" && (i = Zt(i == null ? 1 : +i)), a.initialize = function(l, u) {
    t = l, s = u, o();
  }, a.iterations = function(l) {
    return arguments.length ? (r = +l, a) : r;
  }, a.strength = function(l) {
    return arguments.length ? (n = +l, a) : n;
  }, a.radius = function(l) {
    return arguments.length ? (i = typeof l == "function" ? l : Zt(+l), o(), a) : i;
  }, a;
}
function sh(i) {
  return i.index;
}
function us(i, t) {
  var e = i.get(t);
  if (!e)
    throw new Error("node not found: " + t);
  return e;
}
K.copy = function() {
  var i, t, e = new ce(this._x, this._y, this._x0, this._y0, this._x1, this._y1), s = this._root;
  if (!s)
    return e;
  if (!s.length)
    return e._root = ls(s), e;
  for (i = [{ source: s, target: e._root = new Array(4) }]; s = i.pop(); )
    for (var n = 0; n < 4; ++n)
      (t = s.source[n]) && (t.length ? i.push({ source: t, target: s.target[n] = new Array(4) }) : s.target[n] = ls(t));
  return e;
}, K.add = function(i) {
  const t = +this._x.call(null, i), e = +this._y.call(null, i);
  return os(this.cover(t, e), t, e, i);
}, K.addAll = function(i) {
  var t, e, s, n, r = i.length, a = new Array(r), h = new Array(r), o = 1 / 0, l = 1 / 0, u = -1 / 0, d = -1 / 0;
  for (e = 0; e < r; ++e)
    isNaN(s = +this._x.call(null, t = i[e])) || isNaN(n = +this._y.call(null, t)) || (a[e] = s, h[e] = n, s < o && (o = s), s > u && (u = s), n < l && (l = n), n > d && (d = n));
  if (o > u || l > d)
    return this;
  for (this.cover(o, l).cover(u, d), e = 0; e < r; ++e)
    os(this, a[e], h[e], i[e]);
  return this;
}, K.cover = function(i, t) {
  if (isNaN(i = +i) || isNaN(t = +t))
    return this;
  var e = this._x0, s = this._y0, n = this._x1, r = this._y1;
  if (isNaN(e))
    n = (e = Math.floor(i)) + 1, r = (s = Math.floor(t)) + 1;
  else {
    for (var a, h, o = n - e || 1, l = this._root; e > i || i >= n || s > t || t >= r; )
      switch (h = (t < s) << 1 | i < e, (a = new Array(4))[h] = l, l = a, o *= 2, h) {
        case 0:
          n = e + o, r = s + o;
          break;
        case 1:
          e = n - o, r = s + o;
          break;
        case 2:
          n = e + o, s = r - o;
          break;
        case 3:
          e = n - o, s = r - o;
      }
    this._root && this._root.length && (this._root = l);
  }
  return this._x0 = e, this._y0 = s, this._x1 = n, this._y1 = r, this;
}, K.data = function() {
  var i = [];
  return this.visit(function(t) {
    if (!t.length)
      do
        i.push(t.data);
      while (t = t.next);
  }), i;
}, K.extent = function(i) {
  return arguments.length ? this.cover(+i[0][0], +i[0][1]).cover(+i[1][0], +i[1][1]) : isNaN(this._x0) ? void 0 : [[this._x0, this._y0], [this._x1, this._y1]];
}, K.find = function(i, t, e) {
  var s, n, r, a, h, o, l, u = this._x0, d = this._y0, c = this._x1, g = this._y1, p = [], f = this._root;
  for (f && p.push(new X(f, u, d, c, g)), e == null ? e = 1 / 0 : (u = i - e, d = t - e, c = i + e, g = t + e, e *= e); o = p.pop(); )
    if (!(!(f = o.node) || (n = o.x0) > c || (r = o.y0) > g || (a = o.x1) < u || (h = o.y1) < d))
      if (f.length) {
        var y = (n + a) / 2, m = (r + h) / 2;
        p.push(new X(f[3], y, m, a, h), new X(f[2], n, m, y, h), new X(f[1], y, r, a, m), new X(f[0], n, r, y, m)), (l = (t >= m) << 1 | i >= y) && (o = p[p.length - 1], p[p.length - 1] = p[p.length - 1 - l], p[p.length - 1 - l] = o);
      } else {
        var A = i - +this._x.call(null, f.data), b = t - +this._y.call(null, f.data), S = A * A + b * b;
        if (S < e) {
          var w = Math.sqrt(e = S);
          u = i - w, d = t - w, c = i + w, g = t + w, s = f.data;
        }
      }
  return s;
}, K.remove = function(i) {
  if (isNaN(r = +this._x.call(null, i)) || isNaN(a = +this._y.call(null, i)))
    return this;
  var t, e, s, n, r, a, h, o, l, u, d, c, g = this._root, p = this._x0, f = this._y0, y = this._x1, m = this._y1;
  if (!g)
    return this;
  if (g.length)
    for (; ; ) {
      if ((l = r >= (h = (p + y) / 2)) ? p = h : y = h, (u = a >= (o = (f + m) / 2)) ? f = o : m = o, t = g, !(g = g[d = u << 1 | l]))
        return this;
      if (!g.length)
        break;
      (t[d + 1 & 3] || t[d + 2 & 3] || t[d + 3 & 3]) && (e = t, c = d);
    }
  for (; g.data !== i; )
    if (s = g, !(g = g.next))
      return this;
  return (n = g.next) && delete g.next, s ? (n ? s.next = n : delete s.next, this) : t ? (n ? t[d] = n : delete t[d], (g = t[0] || t[1] || t[2] || t[3]) && g === (t[3] || t[2] || t[1] || t[0]) && !g.length && (e ? e[c] = g : this._root = g), this) : (this._root = n, this);
}, K.removeAll = function(i) {
  for (var t = 0, e = i.length; t < e; ++t)
    this.remove(i[t]);
  return this;
}, K.root = function() {
  return this._root;
}, K.size = function() {
  var i = 0;
  return this.visit(function(t) {
    if (!t.length)
      do
        ++i;
      while (t = t.next);
  }), i;
}, K.visit = function(i) {
  var t, e, s, n, r, a, h = [], o = this._root;
  for (o && h.push(new X(o, this._x0, this._y0, this._x1, this._y1)); t = h.pop(); )
    if (!i(o = t.node, s = t.x0, n = t.y0, r = t.x1, a = t.y1) && o.length) {
      var l = (s + r) / 2, u = (n + a) / 2;
      (e = o[3]) && h.push(new X(e, l, u, r, a)), (e = o[2]) && h.push(new X(e, s, u, l, a)), (e = o[1]) && h.push(new X(e, l, n, r, u)), (e = o[0]) && h.push(new X(e, s, n, l, u));
    }
  return this;
}, K.visitAfter = function(i) {
  var t, e = [], s = [];
  for (this._root && e.push(new X(this._root, this._x0, this._y0, this._x1, this._y1)); t = e.pop(); ) {
    var n = t.node;
    if (n.length) {
      var r, a = t.x0, h = t.y0, o = t.x1, l = t.y1, u = (a + o) / 2, d = (h + l) / 2;
      (r = n[0]) && e.push(new X(r, a, h, u, d)), (r = n[1]) && e.push(new X(r, u, h, o, d)), (r = n[2]) && e.push(new X(r, a, d, u, l)), (r = n[3]) && e.push(new X(r, u, d, o, l));
    }
    s.push(t);
  }
  for (; t = s.pop(); )
    i(t.node, t.x0, t.y0, t.x1, t.y1);
  return this;
}, K.x = function(i) {
  return arguments.length ? (this._x = i, this) : this._x;
}, K.y = function(i) {
  return arguments.length ? (this._y = i, this) : this._y;
};
var nh = { value: () => {
} };
function Qs() {
  for (var i, t = 0, e = arguments.length, s = {}; t < e; ++t) {
    if (!(i = arguments[t] + "") || i in s || /[\s.]/.test(i))
      throw new Error("illegal type: " + i);
    s[i] = [];
  }
  return new ei(s);
}
function ei(i) {
  this._ = i;
}
function rh(i, t) {
  for (var e, s = 0, n = i.length; s < n; ++s)
    if ((e = i[s]).name === t)
      return e.value;
}
function cs(i, t, e) {
  for (var s = 0, n = i.length; s < n; ++s)
    if (i[s].name === t) {
      i[s] = nh, i = i.slice(0, s).concat(i.slice(s + 1));
      break;
    }
  return e != null && i.push({ name: t, value: e }), i;
}
ei.prototype = Qs.prototype = { constructor: ei, on: function(i, t) {
  var e, s, n = this._, r = (s = n, (i + "").trim().split(/^|\s+/).map(function(o) {
    var l = "", u = o.indexOf(".");
    if (u >= 0 && (l = o.slice(u + 1), o = o.slice(0, u)), o && !s.hasOwnProperty(o))
      throw new Error("unknown type: " + o);
    return { type: o, name: l };
  })), a = -1, h = r.length;
  if (!(arguments.length < 2)) {
    if (t != null && typeof t != "function")
      throw new Error("invalid callback: " + t);
    for (; ++a < h; )
      if (e = (i = r[a]).type)
        n[e] = cs(n[e], i.name, t);
      else if (t == null)
        for (e in n)
          n[e] = cs(n[e], i.name, null);
    return this;
  }
  for (; ++a < h; )
    if ((e = (i = r[a]).type) && (e = rh(n[e], i.name)))
      return e;
}, copy: function() {
  var i = {}, t = this._;
  for (var e in t)
    i[e] = t[e].slice();
  return new ei(i);
}, call: function(i, t) {
  if ((e = arguments.length - 2) > 0)
    for (var e, s, n = new Array(e), r = 0; r < e; ++r)
      n[r] = arguments[r + 2];
  if (!this._.hasOwnProperty(i))
    throw new Error("unknown type: " + i);
  for (r = 0, e = (s = this._[i]).length; r < e; ++r)
    s[r].value.apply(t, n);
}, apply: function(i, t, e) {
  if (!this._.hasOwnProperty(i))
    throw new Error("unknown type: " + i);
  for (var s = this._[i], n = 0, r = s.length; n < r; ++n)
    s[n].value.apply(t, e);
} };
var si, Ot, Pt = 0, Lt = 0, Ct = 0, mi = 0, yt = 0, xi = 0, Yt = typeof performance == "object" && performance.now ? performance : Date, Us = typeof window == "object" && window.requestAnimationFrame ? window.requestAnimationFrame.bind(window) : function(i) {
  setTimeout(i, 17);
};
function Js() {
  return yt || (Us(hh), yt = Yt.now() + xi);
}
function hh() {
  yt = 0;
}
function ee() {
  this._call = this._time = this._next = null;
}
function _s(i, t, e) {
  var s = new ee();
  return s.restart(i, t, e), s;
}
function ds() {
  yt = (mi = Yt.now()) + xi, Pt = Lt = 0;
  try {
    (function() {
      Js(), ++Pt;
      for (var i, t = si; t; )
        (i = yt - t._time) >= 0 && t._call.call(void 0, i), t = t._next;
      --Pt;
    })();
  } finally {
    Pt = 0, function() {
      for (var i, t, e = si, s = 1 / 0; e; )
        e._call ? (s > e._time && (s = e._time), i = e, e = e._next) : (t = e._next, e._next = null, e = i ? i._next = t : si = t);
      Ot = i, se(s);
    }(), yt = 0;
  }
}
function ah() {
  var i = Yt.now(), t = i - mi;
  t > 1e3 && (xi -= t, mi = i);
}
function se(i) {
  Pt || (Lt && (Lt = clearTimeout(Lt)), i - yt > 24 ? (i < 1 / 0 && (Lt = setTimeout(ds, i - Yt.now() - xi)), Ct && (Ct = clearInterval(Ct))) : (Ct || (mi = Yt.now(), Ct = setInterval(ah, 1e3)), Pt = 1, Us(ds)));
}
ee.prototype = _s.prototype = { constructor: ee, restart: function(i, t, e) {
  if (typeof i != "function")
    throw new TypeError("callback is not a function");
  e = (e == null ? Js() : +e) + (t == null ? 0 : +t), this._next || Ot === this || (Ot ? Ot._next = this : si = this, Ot = this), this._call = i, this._time = e, se();
}, stop: function() {
  this._call && (this._call = null, this._time = 1 / 0, se());
} };
const gs = 4294967296;
var oh = Math.PI * (3 - Math.sqrt(5));
function $s(i) {
  var t, e = 1, s = 1e-3, n = 1 - Math.pow(s, 1 / 300), r = 0, a = 0.6, h = /* @__PURE__ */ new Map(), o = _s(d), l = Qs("tick", "end"), u = function() {
    let f = 1;
    return () => (f = (1664525 * f + 1013904223) % gs) / gs;
  }();
  function d() {
    c(), l.call("tick", t), e < s && (o.stop(), l.call("end", t));
  }
  function c(f) {
    var y, m, A = i.length;
    f === void 0 && (f = 1);
    for (var b = 0; b < f; ++b)
      for (e += (r - e) * n, h.forEach(function(S) {
        S(e);
      }), y = 0; y < A; ++y)
        (m = i[y]).fx == null ? m.x += m.vx *= a : (m.x = m.fx, m.vx = 0), m.fy == null ? m.y += m.vy *= a : (m.y = m.fy, m.vy = 0);
    return t;
  }
  function g() {
    for (var f, y = 0, m = i.length; y < m; ++y) {
      if ((f = i[y]).index = y, f.fx != null && (f.x = f.fx), f.fy != null && (f.y = f.fy), isNaN(f.x) || isNaN(f.y)) {
        var A = 10 * Math.sqrt(0.5 + y), b = y * oh;
        f.x = A * Math.cos(b), f.y = A * Math.sin(b);
      }
      (isNaN(f.vx) || isNaN(f.vy)) && (f.vx = f.vy = 0);
    }
  }
  function p(f) {
    return f.initialize && f.initialize(i, u), f;
  }
  return i == null && (i = []), g(), t = { tick: c, restart: function() {
    return o.restart(d), t;
  }, stop: function() {
    return o.stop(), t;
  }, nodes: function(f) {
    return arguments.length ? (i = f, g(), h.forEach(p), t) : i;
  }, alpha: function(f) {
    return arguments.length ? (e = +f, t) : e;
  }, alphaMin: function(f) {
    return arguments.length ? (s = +f, t) : s;
  }, alphaDecay: function(f) {
    return arguments.length ? (n = +f, t) : +n;
  }, alphaTarget: function(f) {
    return arguments.length ? (r = +f, t) : r;
  }, velocityDecay: function(f) {
    return arguments.length ? (a = 1 - f, t) : 1 - a;
  }, randomSource: function(f) {
    return arguments.length ? (u = f, h.forEach(p), t) : u;
  }, force: function(f, y) {
    return arguments.length > 1 ? (y == null ? h.delete(f) : h.set(f, p(y)), t) : h.get(f);
  }, find: function(f, y, m) {
    var A, b, S, w, v, M = 0, j = i.length;
    for (m == null ? m = 1 / 0 : m *= m, M = 0; M < j; ++M)
      (S = (A = f - (w = i[M]).x) * A + (b = y - w.y) * b) < m && (v = w, m = S);
    return v;
  }, on: function(f, y) {
    return arguments.length > 1 ? (l.on(f, y), t) : l.on(f);
  } };
}
class gh extends lt {
  constructor(t) {
    super(t), this.data = t.data, this.links = t.links, this.margin = t.margin || { top: 50, right: 20, bottom: 10, left: 20 }, this.colors = t.colors || xt, this.highlight = t.highlight, this.roughness = at({ roughness: t.roughness, ceiling: 30, defaultValue: 0 }), this.strokeWidth = t.strokeWidth || 0.75, this.innerStrokeWidth = t.innerStrokeWidth || 0.75, this.fillWeight = t.fillWeight || 0.85, this.color = t.color || "skyblue", this.collision = t.collision || 1.4, this.radiusExtent = t.radiusExtent || [5, 20], this.radius = t.radius || "radius", this.textCallback = t.textCallback || ((e) => ""), this.colorCallback = t.colorCallback || ((e) => this.color), this.roughnessExtent = t.roughnessExtent || [0, 10], this.responsive = !0, this.boundRedraw = this.redraw.bind(this, t), this.legend = t.legend || !1, this.legendPosition = t.legendPosition || "right", this.initChartValues(t) && (this.resolveFont(), this.drawChart = this.resolveData(t.data, t.links), this.drawChart(), t.title !== "undefined" && this.setTitle(t.title));
  }
  resizeHandler() {
    this.responsive && this.boundRedraw();
  }
  remove() {
    x(this.el).select("svg").remove();
  }
  redraw(t) {
    this.remove(), this.initChartValues(t) && (this.resolveFont(), this.drawChart = this.resolveData(t.data, t.links), this.drawChart(), t.title !== "undefined" && this.setTitle(t.title));
  }
  initChartValues(t) {
    this.roughness = t.roughness || this.roughness, this.collision = t.collision || this.collision, this.color = t.color || this.color, this.stroke = t.stroke || this.stroke, this.strokeWidth = t.strokeWidth || this.strokeWidth, this.axisStrokeWidth = t.axisStrokeWidth || this.axisStrokeWidth, this.axisRoughness = t.axisRoughness || this.axisRoughness, this.innerStrokeWidth = t.innerStrokeWidth || this.innerStrokeWidth, this.fillWeight = t.fillWeight || this.fillWeight, this.fillStyle = t.fillStyle || this.fillStyle, this.title = t.title || this.title, this.textCallback = t.textCallback || ((a) => "");
    const e = x(this.el).node();
    if (!e)
      return this.width = 0, this.height = 0, !1;
    const s = e.getBoundingClientRect(), n = s.width, r = s.height;
    return this.width = n - this.margin.left - this.margin.right, this.height = r - this.margin.top - this.margin.bottom, this.roughId = this.el + "_svg", this.graphClass = this.el.substring(1, this.el.length), this.interactionG = "g." + this.graphClass, this.setSvg(), !0;
  }
  resolveData(t, e) {
    return () => {
      this.data = t, this.links = e, this.drawFromObject();
    };
  }
  setTitle(t) {
    this.svg.append("text").attr("x", this.width / 2).attr("y", 0 - this.margin.top / 3).attr("class", "title").attr("text-anchor", "middle").style("font-size", this.titleFontSize === void 0 ? `${Math.min(40, Math.min(this.width, this.height) / 4)}px` : this.titleFontSize).style("font-family", this.fontFamily).style("opacity", 0.8).text(t);
  }
  addInteraction() {
    const t = this;
    let e;
    z(".nodeGroup").on("mouseover", function(s) {
      x(this).raise(), e = x(this).selectAll("path").style("stroke"), t.highlight === void 0 ? x(this).selectAll("path:nth-child(1)").style("opacity", 0.4) : x(this).selectAll("path:nth-child(1)").style("stroke", t.highlight), x(this).selectAll("path:nth-child(2)").style("stroke-width", t.strokeWidth + 1.2), x(this).select(".node-text").attr("opacity", 1);
    }).on("mouseleave", function(s) {
      x(this).selectAll("path:nth-child(1)").style("opacity", 1), x(this).selectAll("path:nth-child(1)").style("stroke", e), x(this).selectAll("path:nth-child(2)").style("stroke-width", t.innerStrokeWidth), x(this).select(".node-text").attr("opacity", 0);
    });
  }
  initRoughObjects() {
    this.roughSvg = document.getElementById(this.roughId), this.rcAxis = D(this.roughSvg, { options: { strokeWidth: this.strokeWidth >= 3 ? 3 : this.strokeWidth } }), this.rc = D(this.roughSvg, { options: { strokeWidth: this.innerStrokeWidth, fill: this.color, stroke: this.stroke === "none" ? void 0 : this.stroke, roughness: this.roughness, bowing: this.bowing, fillStyle: this.fillStyle } });
  }
  drawFromObject() {
    const t = this;
    let e, s;
    if (typeof this.radius == "number")
      e = Z().domain([0, 1]).range([this.extent[0], this.radiusExtent[1]]);
    else {
      const h = ut(this.data, (l) => +l[this.radius]), o = Q(this.data, (l) => +l[this.radius]);
      e = Z().domain([h, o]).range([this.radiusExtent[0], this.radiusExtent[1]]);
    }
    if (typeof this.roughness == "number")
      s = Z().domain([0, 1]).range([this.roughnessExtent[0], this.roughnessExtent[1]]);
    else {
      const h = ut(this.data, (l) => +l[this.radius]), o = Q(this.data, (l) => +l[this.radius]);
      s = Z().domain([h, o]).range([this.roughnessExtent[0], this.roughnessExtent[1]]);
    }
    this.initRoughObjects();
    const n = this.svg.selectAll(".link").data(this.links).enter().append("line").attr("class", "link"), r = this.svg.selectAll(".nodeGroup").data(this.data).enter().append("g").attr("class", "nodeGroup");
    r.each(function(h, o) {
      const l = typeof t.radius == "number" ? t.radius : e(h[t.radius]), u = typeof t.roughness == "number" ? t.roughness : s(h[t.roughness]), d = t.rc.circle(0, 0, l, { fill: t.colorCallback(h), simplification: t.simplification, fillWeight: t.fillWeight, roughness: u });
      this.appendChild(d), d.setAttribute("class", t.graphClass + "_node"), x(this).append("circle").attr("class", "node-circle").attr("r", 0.5 * l).attr("fill", "transparent").attr("stroke-width", 0).attr("stroke", "none"), x(this).append("text").attr("class", "node-text").attr("x", 0).attr("y", -10).attr("text-anchor", "middle").style("pointer-events", "none").attr("stroke", "black").attr("fill", "white").attr("stroke-linejoin", "fill").attr("paint-order", "stroke fill").attr("stroke-width", "5px").attr("opacity", 0).text((c) => t.textCallback(c));
    });
    const a = $s(this.data);
    if (a.alpha(1).restart(), a.force("collide", Ks().radius((h) => h.radius * this.collision)).force("center", Ns(this.width / 2, this.height / 2)).force("link", function(h) {
      var o, l, u, d, c, g, p = sh, f = function(v) {
        return 1 / Math.min(d[v.source.index], d[v.target.index]);
      }, y = Zt(30), m = 1;
      function A(v) {
        for (var M = 0, j = h.length; M < m; ++M)
          for (var C, P, F, k, R, O, I, L = 0; L < j; ++L)
            P = (C = h[L]).source, k = (F = C.target).x + F.vx - P.x - P.vx || pi(g), R = F.y + F.vy - P.y - P.vy || pi(g), k *= O = ((O = Math.sqrt(k * k + R * R)) - l[L]) / O * v * o[L], R *= O, F.vx -= k * (I = c[L]), F.vy -= R * I, P.vx += k * (I = 1 - I), P.vy += R * I;
      }
      function b() {
        if (u) {
          var v, M, j = u.length, C = h.length, P = new Map(u.map((F, k) => [p(F, k, u), F]));
          for (v = 0, d = new Array(j); v < C; ++v)
            (M = h[v]).index = v, typeof M.source != "object" && (M.source = us(P, M.source)), typeof M.target != "object" && (M.target = us(P, M.target)), d[M.source.index] = (d[M.source.index] || 0) + 1, d[M.target.index] = (d[M.target.index] || 0) + 1;
          for (v = 0, c = new Array(C); v < C; ++v)
            M = h[v], c[v] = d[M.source.index] / (d[M.source.index] + d[M.target.index]);
          o = new Array(C), S(), l = new Array(C), w();
        }
      }
      function S() {
        if (u)
          for (var v = 0, M = h.length; v < M; ++v)
            o[v] = +f(h[v], v, h);
      }
      function w() {
        if (u)
          for (var v = 0, M = h.length; v < M; ++v)
            l[v] = +y(h[v], v, h);
      }
      return h == null && (h = []), A.initialize = function(v, M) {
        u = v, g = M, b();
      }, A.links = function(v) {
        return arguments.length ? (h = v, b(), A) : h;
      }, A.id = function(v) {
        return arguments.length ? (p = v, A) : p;
      }, A.iterations = function(v) {
        return arguments.length ? (m = +v, A) : m;
      }, A.strength = function(v) {
        return arguments.length ? (f = typeof v == "function" ? v : Zt(+v), S(), A) : f;
      }, A.distance = function(v) {
        return arguments.length ? (y = typeof v == "function" ? v : Zt(+v), w(), A) : y;
      }, A;
    }(this.links).distance(100)), a.on("tick", () => {
      r.attr("transform", (h) => `translate(${h.x}, ${h.y})`), n.attr("x1", (h) => h.source.x).attr("y1", (h) => h.source.y).attr("x2", (h) => h.target.x).attr("y2", (h) => h.target.y);
    }), z(".nodeGroup").selectAll("path:nth-child(2)").style("stroke-width", this.strokeWidth), this.interactive === !0 && this.addInteraction(), this.legend) {
      const h = this.legend;
      this.colors = this.legend.map((u) => u.color);
      const o = 6 * h.reduce((u, d) => u > d.text.length ? u : d.text.length, 0) + 35, l = 11 * h.length + 8;
      dt(this, h, o, l);
    }
  }
}
class fh extends lt {
  constructor(t) {
    super(t), this.data = t.data, this.margin = t.margin || { top: 50, right: 20, bottom: 10, left: 20 }, this.colors = t.colors || xt, this.highlight = t.highlight, this.roughness = at({ roughness: t.roughness, ceiling: 30, defaultValue: 0 }), this.strokeWidth = t.strokeWidth || 0.75, this.innerStrokeWidth = t.innerStrokeWidth || 0.75, this.fillWeight = t.fillWeight || 0.85, this.color = t.color || "pink", this.collision = t.collision || 1, this.radiusExtent = t.radiusExtent || [5, 20], this.radius = t.radius || "radius", this.roughnessExtent = t.roughnessExtent || [0, 10], this.responsive = !0, this.boundRedraw = this.redraw.bind(this, t), this.textCallback = t.textCallback || ((e) => ""), this.colorCallback = t.colorCallback || ((e) => this.color), this.legend = t.legend || !1, this.legendPosition = t.legendPosition || "right", this.initChartValues(t) && (this.resolveFont(), this.drawChart = this.resolveData(t.data), this.drawChart(), t.title !== "undefined" && this.setTitle(t.title));
  }
  resizeHandler() {
    this.responsive && this.boundRedraw();
  }
  remove() {
    x(this.el).select("svg").remove();
  }
  redraw(t) {
    this.remove(), this.initChartValues(t) && (this.resolveFont(), this.drawChart = this.resolveData(t.data), this.drawChart(), t.title !== "undefined" && this.setTitle(t.title));
  }
  initChartValues(t) {
    this.roughness = t.roughness || this.roughness, this.collision = t.collision || this.collision, this.color = t.color || this.color, this.stroke = t.stroke || this.stroke, this.strokeWidth = t.strokeWidth || this.strokeWidth, this.axisStrokeWidth = t.axisStrokeWidth || this.axisStrokeWidth, this.axisRoughness = t.axisRoughness || this.axisRoughness, this.innerStrokeWidth = t.innerStrokeWidth || this.innerStrokeWidth, this.fillWeight = t.fillWeight || this.fillWeight, this.fillStyle = t.fillStyle || this.fillStyle, this.title = t.title || this.title, this.textCallback = t.textCallback || ((a) => "");
    const e = x(this.el).node();
    if (!e)
      return this.width = 0, this.height = 0, !1;
    const s = e.getBoundingClientRect(), n = s.width, r = s.height;
    return this.width = n - this.margin.left - this.margin.right, this.height = r - this.margin.top - this.margin.bottom, this.roughId = this.el + "_svg", this.graphClass = this.el.substring(1, this.el.length), this.interactionG = "g." + this.graphClass, this.setSvg(), !0;
  }
  resolveData(t) {
    return () => {
      this.data = t, this.drawFromObject();
    };
  }
  setTitle(t) {
    this.svg.append("text").attr("x", this.width / 2).attr("y", 0 - this.margin.top / 3).attr("class", "title").attr("text-anchor", "middle").style("font-size", this.titleFontSize === void 0 ? `${Math.min(40, Math.min(this.width, this.height) / 4)}px` : this.titleFontSize).style("font-family", this.fontFamily).style("opacity", 0.8).text(t);
  }
  addInteraction() {
    const t = this;
    let e;
    z(".nodeGroup").on("mouseover", function(s) {
      e = x(this).selectAll("path").style("stroke"), x(this).raise(), t.highlight === void 0 ? x(this).selectAll("path:nth-child(1)").style("opacity", 0.4) : x(this).selectAll("path:nth-child(1)").style("stroke", t.highlight), x(this).selectAll("path:nth-child(2)").style("stroke-width", t.strokeWidth + 1.2), x(this).select(".node-text").attr("opacity", 1), x(this).select(".node-text").raise();
    }).on("mouseleave", function(s) {
      x(this).selectAll("path:nth-child(1)").style("opacity", 1), x(this).selectAll("path:nth-child(1)").style("stroke", e), x(this).selectAll("path:nth-child(2)").style("stroke-width", t.strokeWidth), x(this).select(".node-text").attr("opacity", 0);
    });
  }
  initRoughObjects() {
    this.roughSvg = document.getElementById(this.roughId), this.rcAxis = D(this.roughSvg, { options: { strokeWidth: this.strokeWidth >= 3 ? 3 : this.strokeWidth } }), this.rc = D(this.roughSvg, { options: { strokeWidth: this.innerStrokeWidth, fill: this.color, stroke: this.stroke === "none" ? void 0 : this.stroke, roughness: this.roughness, bowing: this.bowing, fillStyle: this.fillStyle } });
  }
  drawFromObject() {
    const t = this;
    let e, s;
    if (typeof this.radius == "number")
      e = Z().domain([0, 1]).range([this.radiusExtent[0], this.radiusExtent[1]]);
    else {
      const h = ut(this.data, (l) => +l[this.radius]), o = Q(this.data, (l) => +l[this.radius]);
      e = Z().domain([h, o]).range([this.radiusExtent[0], this.radiusExtent[1]]);
    }
    if (typeof this.roughness == "number")
      s = Z().domain([0, 1]).range([this.roughnessExtent[0], this.roughnessExtent[1]]);
    else {
      const h = ut(this.data, (l) => +l[this.radius]), o = Q(this.data, (l) => +l[this.radius]);
      s = Z().domain([h, o]).range([this.roughnessExtent[0], this.roughnessExtent[1]]);
    }
    this.initRoughObjects();
    let n = this.svg.selectAll(".nodeGroup").data(this.data), r = n.enter().append("g").attr("class", "nodeGroup");
    n = n.merge(r), n.each(function(h, o) {
      const l = typeof t.radius == "number" ? t.radius : e(h[t.radius]), u = typeof t.roughness == "number" ? t.roughness : s(h[t.roughness]), d = t.rc.circle(0, 0, l, { fill: t.colorCallback(h), simplification: t.simplification, fillWeight: t.fillWeight, roughness: u });
      this.appendChild(d).setAttribute("class", t.graphClass + "_node"), x(this).append("circle").attr("class", "node-circle").attr("r", 0.5 * l).attr("fill", "transparent").attr("stroke-width", 0).attr("stroke", "none"), x(this).append("text").attr("class", "node-text").attr("x", 0).attr("y", -10).attr("text-anchor", "middle").style("pointer-events", "none").attr("stroke", "black").attr("fill", "white").attr("stroke-linejoin", "fill").attr("paint-order", "stroke fill").attr("stroke-width", "5px").attr("opacity", 0).text((c) => t.textCallback(c));
    });
    const a = $s(this.data);
    if (a.alpha(1).restart(), a.force("collide", Ks().radius((h) => h.radius * this.collision * 1.2)).force("center", Ns(this.width / 2, this.height / 2)), a.on("tick", () => {
      n.attr("transform", (h) => `translate(${h.x}, ${h.y})`), n.attr("attrX", (h) => +h.x), n.attr("attrY", (h) => +h.y);
    }), z(".nodeGroup").selectAll("path:nth-child(2)").style("stroke-width", this.strokeWidth), this.interactive === !0 && this.addInteraction(), this.legend) {
      const h = this.legend;
      this.colors = this.legend.map((u) => u.color);
      const o = 6 * h.reduce((u, d) => u > d.text.length ? u : d.text.length, 0) + 35, l = 11 * h.length + 8;
      dt(this, h, o, l);
    }
  }
}
const Zi = "http://www.w3.org/2000/svg", fs = "data-roughviz-slice-clip";
class ph extends lt {
  constructor(t) {
    super(t), this.data = t.data, this.margin = t.margin || { top: 50, right: 20, bottom: 10, left: 20 }, this.colors = t.colors || xt, this.highlight = t.highlight, this.roughness = at({ roughness: t.roughness, ceiling: 30, defaultValue: 0 }), this.strokeWidth = t.strokeWidth || 0.75, this.innerStrokeWidth = t.innerStrokeWidth || 0.75, this.fillWeight = t.fillWeight || 0.85, this.labels = this.dataFormat === "object" ? "labels" : t.labels, this.values = this.dataFormat === "object" ? "values" : t.values, this.labels !== void 0 && this.values !== void 0 ? (this.legend = t.legend !== !1, this.legendPosition = t.legendPosition || "right", this.responsive = !0, this.boundRedraw = this.redraw.bind(this, t), this.initChartValues(t) && (this.resolveFont(), this.drawChart = this.resolveData(t.data), this.drawChart(), t.title !== "undefined" && this.setTitle(t.title))) : console.log(`Error for ${this.el}: Must include labels and values when        instantiating Donut chart. Skipping chart.`);
  }
  resizeHandler() {
    this.responsive && this.boundRedraw();
  }
  remove() {
    x(this.el).select("svg").remove();
  }
  redraw(t) {
    this.remove(), this.initChartValues(t) && (this.resolveFont(), this.drawChart = this.resolveData(t.data), this.drawChart(), t.title !== "undefined" && this.setTitle(t.title));
  }
  initChartValues(t) {
    this.roughness = t.roughness || this.roughness, this.stroke = t.stroke || this.stroke, this.strokeWidth = t.strokeWidth || this.strokeWidth, this.axisStrokeWidth = t.axisStrokeWidth || this.axisStrokeWidth, this.axisRoughness = t.axisRoughness || this.axisRoughness, this.innerStrokeWidth = t.innerStrokeWidth || this.innerStrokeWidth, this.fillWeight = t.fillWeight || this.fillWeight, this.fillStyle = t.fillStyle || this.fillStyle;
    const e = x(this.el).node();
    if (!e)
      return this.width = 0, this.height = 0, !1;
    const s = e.getBoundingClientRect(), n = s.width, r = s.height;
    return this.width = n - this.margin.left - this.margin.right, this.height = r - this.margin.top - this.margin.bottom, this.roughId = this.el + "_svg", this.graphClass = this.el.substring(1, this.el.length), this.interactionG = "g." + this.graphClass, this.radius = Math.min(this.width, this.height) / 2, this.setSvg(), !0;
  }
  resolveData(t) {
    return typeof t != "string" ? () => {
      this.data = t, this.drawFromObject();
    } : t.includes(".csv") ? () => {
      vt(t).then((e) => {
        this.data = e, this.drawFromFile();
      });
    } : t.includes(".tsv") ? () => {
      bt(t).then((e) => {
        this.data = e, this.drawFromFile();
      });
    } : t.includes(".json") ? () => {
      ks(t).then((e) => {
        this.data = e, this.drawFromFile();
      });
    } : void 0;
  }
  setTitle(t) {
    this.svg.append("text").attr("x", this.width / 2).attr("y", 0 - this.margin.top / 3).attr("class", "title").attr("text-anchor", "middle").style("font-size", this.titleFontSize === void 0 ? `${Math.min(40, Math.min(this.width, this.height) / 4)}px` : this.titleFontSize).style("font-family", this.fontFamily).style("opacity", 0.8).text(t);
  }
  addInteraction() {
    z(this.interactionG).append("g").attr("transform", `translate(${this.width / 2}, ${this.height / 2})`).data(this.dataFormat === "object" ? this.makePie(this.data[this.values]) : this.makePie(this.data)).append("path").attr("d", this.makeArc).attr("stroke-width", "0px").attr("fill", "transparent");
    const t = x(this.el).append("div").style("opacity", 0).attr("class", "tooltip").style("position", "absolute").style("background-color", "white").style("border", "solid").style("border-width", "1px").style("border-radius", "5px").style("padding", "3px").style("font-family", this.fontFamily).style("font-size", this.tooltipFontSize).style("pointer-events", "none"), e = this;
    let s;
    z(this.interactionG).on("mouseover", function() {
      t.style("opacity", 1), s = x(this).selectAll("path").style("stroke"), e.highlight === void 0 ? x(this).selectAll("path").style("opacity", 0.5) : x(this).selectAll("path").style("stroke", e.highlight);
    }), z(this.interactionG).on("mouseout", function() {
      t.style("opacity", 0), x(this).selectAll("path").style("stroke", s), x(this).selectAll("path").style("opacity", 1);
    }), z(this.interactionG).on("mousemove", function(n) {
      const r = x(this).attr("attrX"), a = x(this).attr("attrY"), h = kt(this);
      t.html(`<b>${r}</b>: ${a}`).style("opacity", 0.95).style("transform", `translate(${h[0] + e.margin.left}px, 
              ${h[1] - (e.height + e.margin.top + e.margin.bottom / 2)}px)`);
    });
  }
  initRoughObjects() {
    this.roughSvg = document.getElementById(this.roughId), this.rcAxis = D(this.roughSvg, { options: { strokeWidth: this.strokeWidth >= 3 ? 3 : this.strokeWidth } }), this.rc = D(this.roughSvg, { options: { fill: this.color, strokeWidth: this.innerStrokeWidth, roughness: this.roughness, bowing: this.bowing, fillStyle: this.fillStyle } });
  }
  getSliceClipKey() {
    return this.roughId.replace(/[^a-zA-Z0-9_-]/g, "_");
  }
  getSliceClipDefs() {
    const t = this.roughSvg && this.roughSvg.ownerSVGElement;
    if (!t)
      return null;
    let e = t.querySelector("defs");
    return e || (e = document.createElementNS(Zi, "defs"), t.insertBefore(e, t.firstChild)), e;
  }
  resetSliceClipPaths() {
    const t = this.getSliceClipDefs();
    if (!t)
      return;
    const e = this.getSliceClipKey();
    t.querySelectorAll(`[${fs}="${e}"]`).forEach((s) => s.remove());
  }
  applySliceClip(t, e, s) {
    const n = this.getSliceClipDefs();
    if (!n || !t)
      return;
    const r = this.makeArc(e);
    if (!r)
      return;
    const a = this.getSliceClipKey(), h = `${a}_slice_clip_${s}`, o = document.createElementNS(Zi, "clipPath");
    o.setAttribute("id", h), o.setAttribute(fs, a);
    const l = document.createElementNS(Zi, "path");
    l.setAttribute("d", r), l.setAttribute("transform", `translate(${this.width / 2}, ${this.height / 2})`), o.appendChild(l), n.appendChild(o), x(t).selectAll("path").attr("clip-path", `url(#${h})`);
  }
  drawFromObject() {
    this.initRoughObjects(), this.makePie = fi(), this.makeArc = gi().innerRadius(0).outerRadius(this.radius), this.arcs = this.makePie(this.data[this.values]), this.resetSliceClipPaths(), this.arcs.forEach((n, r) => {
      if (n.value !== 0) {
        const a = this.rc.arc(this.width / 2, this.height / 2, 2 * this.radius, 2 * this.radius, n.startAngle - Math.PI / 2, n.endAngle - Math.PI / 2, !0, { fill: this.colors[r], stroke: this.colors[r] });
        a.setAttribute("class", this.graphClass);
        const h = this.roughSvg.appendChild(a);
        h.setAttribute("attrY", this.data[this.values][r]), h.setAttribute("attrX", this.data[this.labels][r]), this.applySliceClip(h, n, r);
      }
    }), z(this.interactionG).selectAll("path:nth-child(2)").style("stroke-width", this.strokeWidth);
    const t = this.data.labels.map((n, r) => ({ color: this.colors[r], text: n })), e = 6 * t.reduce((n, r) => n > r.text.length ? n : r.text.length, 0) + 35, s = 11 * t.length + 8;
    this.legend === !0 && dt(this, t, e, s), this.interactive === !0 && this.addInteraction();
  }
  drawFromFile() {
    this.initRoughObjects(), this.makePie = fi().value((r) => r[this.values]).sort(null);
    const t = [];
    this.makeArc = gi().innerRadius(0).outerRadius(this.radius), this.arcs = this.makePie(this.data), this.resetSliceClipPaths(), this.arcs.forEach((r, a) => {
      if (r.value !== 0) {
        const h = this.rc.arc(this.width / 2, this.height / 2, 2 * this.radius, 2 * this.radius, r.startAngle - Math.PI / 2, r.endAngle - Math.PI / 2, !0, { fill: this.colors[a], stroke: this.colors[a] });
        h.setAttribute("class", this.graphClass);
        const o = this.roughSvg.appendChild(h);
        o.setAttribute("attrY", r.data[this.values]), o.setAttribute("attrX", r.data[this.labels]), this.applySliceClip(o, r, a);
      }
      t.push(r.data[this.labels]);
    }), z(this.interactionG).selectAll("path:nth-child(2)").style("stroke-width", this.strokeWidth);
    const e = t.map((r, a) => ({ color: this.colors[a], text: r })), s = 6 * e.reduce((r, a) => r > a.text.length ? r : a.text.length, 0) + 35, n = 11 * e.length + 8;
    this.legend === !0 && dt(this, e, s, n), this.interactive === !0 && this.addInteraction();
  }
}
const Bi = ["pink", "skyblue", "coral", "gold", "teal", "darkgreen", "brown", "slateblue", "orange"];
class mh extends lt {
  constructor(t) {
    super(t), this.margin = t.margin || { top: 50, right: 20, bottom: 50, left: 100 }, this.colorVar = t.colorVar, this.roughness = at({ roughness: t.roughness }), this.highlight = t.highlight, this.highlightLabel = t.highlightLabel || "xy", this.radiusExtent = t.radiusExtent || [5, 20], this.radius = t.radius || 20, this.axisStrokeWidth = t.axisStrokeWidth || 0.4, this.axisRoughness = t.axisRoughness || 0.9, this.curbZero = t.curbZero === !0, this.innerStrokeWidth = t.innerStrokeWidth || 1, this.stroke = t.stroke || "black", this.fillWeight = t.fillWeight || 0.85, this.colors = t.colors || Bi, this.strokeWidth = t.strokeWidth || 1, this.axisFontSize = t.axisFontSize, this.x = this.dataFormat === "object" ? "x" : t.x, this.y = this.dataFormat === "object" ? "y" : t.y, this.xValueFormat = t.xValueFormat, this.yValueFormat = t.yValueFormat, this.xLabel = t.xLabel || "", this.yLabel = t.yLabel || "", this.labelFontSize = t.labelFontSize || "1rem", this.responsive = !0, this.boundRedraw = this.redraw.bind(this, t), this.radiusScale, this.initChartValues(t) && (this.resolveFont(), this.drawChart = this.resolveData(t.data), this.drawChart(), t.title !== "undefined" && this.setTitle(t.title), this.boundResizeHandler = this.resizeHandler.bind(this), window.addEventListener("resize", this.boundResizeHandler));
  }
  resizeHandler() {
    x(this.el).node() ? this.responsive && this.boundRedraw() : this.boundResizeHandler && (window.removeEventListener("resize", this.boundResizeHandler), this.boundResizeHandler = null);
  }
  remove() {
    x(this.el).select("svg").remove();
  }
  redraw(t) {
    this.remove(), this.initChartValues(t) && (this.resolveFont(), this.drawChart = this.resolveData(t.data), this.drawChart());
  }
  initChartValues(t) {
    this.roughness = t.roughness || this.roughness, this.stroke = t.stroke || this.stroke, this.strokeWidth = t.strokeWidth || this.strokeWidth, this.axisStrokeWidth = t.axisStrokeWidth || this.axisStrokeWidth, this.axisRoughness = t.axisRoughness || this.axisRoughness, this.innerStrokeWidth = t.innerStrokeWidth || this.innerStrokeWidth, this.fillWeight = t.fillWeight || this.fillWeight, this.fillStyle = t.fillStyle || this.fillStyle, this.colors = t.colors || this.colors;
    const e = x(this.el).node();
    if (!e)
      return this.width = 0, this.height = 0, !1;
    const s = e.getBoundingClientRect(), n = s.width, r = s.height;
    return this.width = n - this.margin.left - this.margin.right, this.height = r - this.margin.top - this.margin.bottom, this.roughId = this.el + "_svg", this.graphClass = this.el.substring(1, this.el.length), this.interactionG = "g." + this.graphClass, this.setSvg(), !0;
  }
  resolveData(t) {
    return typeof t != "string" ? () => {
      this.data = t, this.drawFromObject();
    } : t.includes(".csv") ? () => {
      vt(t).then((e) => {
        this.data = e, this.drawFromFile();
      });
    } : t.includes(".tsv") ? () => {
      bt(t).then((e) => {
        this.data = e, this.drawFromFile();
      });
    } : void 0;
  }
  addScaleLine() {
    let t, e;
    if (this.dataFormat !== "file")
      t = allDataExtent(this.data);
    else {
      const r = this.dataSources.map((o) => st(this.data, (l) => +l[o])), a = ut(r, (o) => o[0]), h = Q(r, (o) => o[1]);
      t = [a, h];
    }
    if (this.x === void 0) {
      const r = Q(Object.keys(this.data).map((a) => this.data[a].length));
      e = this.dataFormat === "file" ? [0, this.data.length] : [0, r];
    } else
      e = st(this.x);
    const s = t, n = s[1] - s[0];
    this.xScale = this.x === void 0 ? scalePoint().range([0, this.width]).domain([...Array(e[1]).keys()]) : scalePoint().range([0, this.width]).domain(this.x), this.yScale = Z().range([this.height, 0]).domain([0, s[1] + 0.05 * n]);
  }
  addScales() {
    const t = this.dataFormat === "file" ? st(this.data, (a) => +a[this.x]) : st(this.data[this.x]), e = t[1] - t[0], s = this.dataFormat === "file" ? st(this.data, (a) => +a[this.y]) : st(this.data[this.y]), n = s[1] - s[0], r = this.dataFormat === "file" ? st(this.data, (a) => a[this.colorVar]) : [1, 1];
    if (this.dataFormat === "file") {
      const a = st(this.data, (o) => +o[this.radius]), h = Math.min(this.width, this.height) / 2 / 2;
      this.radiusScale = Z().range([8, h]).domain(a);
    } else
      this.radiusScale = Z().domain([0, 20]).range([this.radiusExtent[0], this.radiusExtent[1]]);
    this.curbZero === !0 && (s[0] > 0 && (s[0] = 0), t[0] > 0 && (t[0] = 0)), this.xScale = Z().range([0, this.width]).domain([t[0] - 0.05 * e, t[1] + 0.05 * e]), this.yScale = Z().range([this.height, 0]).domain([s[0] - 0.05 * n, s[1] + 0.05 * n]), this.colorScale = Ai().range(this.colors).domain(r);
  }
  addLabels() {
    this.xLabel !== "" && this.svg.append("text").attr("x", this.width / 2).attr("y", this.height + this.margin.bottom / 1.3).attr("dx", "1em").attr("class", "labelText").style("text-anchor", "middle").style("font-family", this.fontFamily).style("font-size", this.labelFontSize).text(this.xLabel), this.yLabel !== "" && this.svg.append("text").attr("transform", "rotate(-90)").attr("y", 0 - this.margin.left / 2).attr("x", 0 - this.height / 2).attr("dy", "1em").attr("class", "labelText").style("text-anchor", "middle").style("font-family", this.fontFamily).style("font-size", this.labelFontSize).text(this.yLabel);
  }
  addAxes() {
    const t = Ht(this.xScale).tickSize(0).tickFormat((s) => this.xValueFormat ? ot(this.xValueFormat)(s) : s), e = Dt(this.yScale).tickSize(0).tickFormat((s) => this.yValueFormat ? ot(this.yValueFormat)(s) : s);
    this.svg.append("g").attr("transform", "translate(0," + this.height + ")").call(t).attr("class", `xAxis${this.graphClass}`).selectAll("text").attr("transform", "translate(-10, 0)rotate(-45)").style("text-anchor", "end").style("font-family", this.fontFamily).style("font-size", this.axisFontSize === void 0 ? `${Math.min(0.95, Math.min(this.width, this.height) / 140)}rem` : this.axisFontSize), this.svg.append("g").call(e).attr("class", `yAxis${this.graphClass}`).selectAll("text").style("font-family", this.fontFamily).style("font-size", this.axisFontSize === void 0 ? `${Math.min(0.95, Math.min(this.width, this.height) / 140)}rem` : this.axisFontSize), z("path.domain").attr("stroke", "transparent"), z("g.tick").style("opacity", 1);
  }
  makeAxesRough(t, e) {
    const s = `xAxis${this.graphClass}`, n = `yAxis${this.graphClass}`, r = `rough-${s}`, a = `rough-${n}`;
    x(`.${s}`).selectAll("path.domain").each(function(h, o) {
      const l = x(this).node().getAttribute("d"), u = e.path(l, { stroke: "black", fillStyle: "hachure" });
      u.setAttribute("class", r), t.appendChild(u);
    }), z(`.${r}`).attr("transform", `translate(0, ${this.height})`), x(`.${n}`).selectAll("path.domain").each(function(h, o) {
      const l = x(this).node().getAttribute("d"), u = e.path(l, { stroke: "black", fillStyle: "hachure" });
      u.setAttribute("class", a), t.appendChild(u);
    });
  }
  setTitle(t) {
    this.svg.append("text").attr("x", this.width / 2).attr("y", 0 - this.margin.top / 2).attr("text-anchor", "middle").style("font-size", this.titleFontSize === void 0 ? `${Math.min(20, Math.min(this.width, this.height) / 4)}px` : this.titleFontSize).style("font-family", this.fontFamily).style("opacity", 0.8).text(t);
  }
  addInteraction() {
    const t = z(this.interactionG).data(this.dataFormat === "file" ? this.data : this.data.x).append("circle").attr("cx", (r, a) => this.dataFormat === "file" ? this.xScale(+r[this.x]) : this.xScale(+this.data[this.x][a])).attr("cy", (r, a) => this.dataFormat === "file" ? this.yScale(+r[this.y]) : this.yScale(+this.data[this.y][a]));
    this.dataFormat === "file" ? t.attr("r", (r) => typeof this.radius == "number" ? 0.7 * this.radius : 0.6 * this.radiusScale(+r[this.radius])).attr("fill", "transparent") : t.attr("r", (r, a) => {
      const h = this.data[this.radius][a];
      return typeof this.radius == "number" ? 0.7 * this.radius : this.radiusScale(h);
    }).attr("fill", "transparent");
    let e = x(this.el).append("div").style("opacity", 0).attr("class", "tooltip").style("position", "absolute").style("background-color", "white").style("border", "solid").style("border-width", "1px").style("border-radius", "5px").style("padding", "3px").style("font-family", this.fontFamily).style("font-size", this.tooltipFontSize).style("pointer-events", "none");
    const s = this;
    let n;
    z(this.interactionG).on("mouseover", function() {
      e.style("opacity", 1), n = x(this).selectAll("path").style("stroke"), s.highlight === void 0 ? x(this).selectAll("path:nth-child(1)").style("opacity", 0.4) : x(this).selectAll("path:nth-child(1)").style("stroke", s.highlight), x(this).selectAll("path:nth-child(2)").style("stroke-width", s.strokeWidth + 1.2);
    }), z(this.interactionG).on("mouseout", function() {
      e.style("opacity", 0), x(this).selectAll("path").style("opacity", 1), x(this).selectAll("path:nth-child(1)").style("stroke", n), x(this).selectAll("path:nth-child(2)").style("stroke", s.stroke), x(this).selectAll("path:nth-child(2)").style("stroke-width", s.strokeWidth);
    }), z(this.interactionG).on("mousemove", function(r) {
      const a = x(this).attr("attrX"), h = x(this).attr("attrY"), o = x(this).attr("attrHighlightLabel"), l = kt(this);
      e.html(s.highlightLabel === "xy" ? `<b>x</b>: ${a} <br><b>y</b>: ${h}` : `<b>${o}</b>`).attr("class", function(u) {
      }).style("transform", `translate(${l[0] + s.margin.left}px, 
          ${l[1] - (s.height + s.margin.top + s.margin.bottom / 2)}px)`);
    });
  }
  initRoughObjects() {
    this.roughSvg = document.getElementById(this.roughId), this.rcAxis = D(this.roughSvg, { options: { strokeWidth: this.axisStrokeWidth, roughness: this.axisRoughness } }), this.rc = D(this.roughSvg, { options: { stroke: this.stroke === "none" ? void 0 : this.stroke, strokeWidth: this.innerStrokeWidth, roughness: this.roughness, bowing: this.bowing, fillStyle: this.fillStyle } });
  }
  drawFromObject() {
    const t = this;
    let e;
    if (this.radiusScale = Z().domain([0, 20]).range([this.radiusExtent[0], this.radiusExtent[1]]), typeof this.radius == "number")
      e = Z().domain([0, 1]).range([this.radiusExtent[0], this.radiusExtent[1]]);
    else {
      const s = ut(this.data[this.radius]), n = Q(this.data[this.radius]);
      e = Z().domain([s, n]).range([this.radiusExtent[0], this.radiusExtent[1]]);
    }
    typeof this.colors == "string" && (this.colors = this.colors), this.colors === void 0 && (this.colors = Bi[0]), this.initRoughObjects(), this.addScales(), this.addAxes(), this.makeAxesRough(this.roughSvg, this.rcAxis), this.addLabels(), this.data.x.forEach((s, n) => {
      const r = typeof t.radius == "number" ? t.radius : e(+this.data[t.radius][n]), a = this.rc.circle(this.xScale(+s), this.yScale(+this.data[this.y][n]), r, { fill: typeof this.colors == "string" ? this.colors : this.colors.length === 1 ? this.colors[0] : this.colors[n], simplification: this.simplification, fillWeight: this.fillWeight }), h = this.roughSvg.appendChild(a);
      h.setAttribute("class", this.graphClass), h.setAttribute("attrX", s), h.setAttribute("attrY", this.data[this.y][n]), h.setAttribute("attrHighlightLabel", this.data[this.highlightLabel]);
    }), z(this.interactionG).selectAll("path:nth-child(2)").style("stroke-width", this.strokeWidth), this.interactive === !0 && this.addInteraction();
  }
  drawFromFile() {
    this.colors === void 0 && (this.colors = Bi), this.initRoughObjects(), this.addScales(), this.addAxes(), this.makeAxesRough(this.roughSvg, this.rcAxis), this.addLabels(), this.data.forEach((t, e) => {
      const s = this.rc.circle(this.xScale(+t[this.x]), this.yScale(+t[this.y]), typeof this.radius == "number" ? this.radius : this.radiusScale(+t[this.radius]), { fill: this.colorVar === void 0 ? this.colors[0] : this.colorScale(t[this.colorVar]), simplification: this.simplification, fillWeight: this.fillWeight }), n = this.roughSvg.appendChild(s);
      n.setAttribute("class", this.graphClass), n.setAttribute("attrX", t[this.x]), n.setAttribute("attrY", t[this.y]), n.setAttribute("attrHighlightLabel", t[this.highlightLabel]);
    }), z(this.interactionG).selectAll("path:nth-child(2)").style("stroke-width", this.strokeWidth), this.interactive === !0 && this.addInteraction();
  }
}
class Ah extends lt {
  constructor(t) {
    super(t), this.data = t.data, this.margin = t.margin || { top: 50, right: 20, bottom: 70, left: 100 }, this.color = t.color || "red", this.highlight = t.highlight || "coral", this.roughness = at({ roughness: t.roughness }), this.stroke = t.stroke || "black", this.strokeWidth = t.strokeWidth || 1, this.axisStrokeWidth = t.axisStrokeWidth || 0.5, this.axisRoughness = t.axisRoughness || 0.5, this.innerStrokeWidth = t.innerStrokeWidth || 1, this.fillWeight = t.fillWeight || 0.5, this.axisFontSize = t.axisFontSize, this.labels = t.labels, this.values = t.values, this.stackColorMapping = {}, this.padding = t.padding || 0.1, this.xLabel = t.xLabel || "", this.yLabel = t.yLabel || "", this.labelFontSize = t.labelFontSize || "1rem", this.responsive = !0, this.boundRedraw = this.redraw.bind(this, t), this.initChartValues(t) && (this.resolveFont(), this.drawChart = this.resolveData(t.data), this.drawChart(), t.title !== "undefined" && this.setTitle(t.title), this.boundResizeHandler = this.resizeHandler.bind(this), window.addEventListener("resize", this.boundResizeHandler));
  }
  resizeHandler() {
    x(this.el).node() ? this.responsive && this.boundRedraw() : this.boundResizeHandler && (window.removeEventListener("resize", this.boundResizeHandler), this.boundResizeHandler = null);
  }
  remove() {
    x(this.el).select("svg").remove();
  }
  redraw(t) {
    this.remove(), this.initChartValues(t) && (this.resolveFont(), this.drawChart = this.resolveData(t.data), this.drawChart(), t.title !== "undefined" && this.setTitle(t.title));
  }
  initChartValues(t) {
    this.roughness = t.roughness || this.roughness, this.stroke = t.stroke || this.stroke, this.strokeWidth = t.strokeWidth || this.strokeWidth, this.axisStrokeWidth = t.axisStrokeWidth || this.axisStrokeWidth, this.axisRoughness = t.axisRoughness || this.axisRoughness, this.innerStrokeWidth = t.innerStrokeWidth || this.innerStrokeWidth, this.fillWeight = t.fillWeight || this.fillWeight, this.fillStyle = t.fillStyle || this.fillStyle;
    const e = x(this.el).node();
    if (!e)
      return this.width = 0, this.height = 0, !1;
    const s = e.getBoundingClientRect(), n = s.width, r = s.height;
    return this.width = n - this.margin.left - this.margin.right, this.height = r - this.margin.top - this.margin.bottom, this.roughId = this.el + "_svg", this.graphClass = this.el.substring(1, this.el.length), this.interactionG = "g." + this.graphClass, this.setSvg(), !0;
  }
  getTotal(t) {
    for (let e = 0; e < t.length; e++) {
      let s = 0;
      for (let n = 0; n < t.columns.length; ++n)
        t.columns[n] !== this.labels && (s += t[e][t.columns[n]] = +t[e][t.columns[n]]);
      t[e].total = s;
    }
    return t;
  }
  updateColorMapping(t) {
    this.stackColorMapping[t] || (this.stackColorMapping[t] = xt[Object.keys(this.stackColorMapping).length]);
  }
  resolveData(t) {
    return typeof t != "string" ? () => {
      this.data = t, t = t.map((e) => (Object.keys(e).includes("total") && (e.total = 0), e));
      for (let e = 0; e < t.length; ++e) {
        let s = 0;
        Object.keys(t[e]).forEach((n) => {
          n !== this.labels && n !== "total" && (this.updateColorMapping(n), s += t[e][n]);
        }), t[e].total = s;
      }
      this.drawFromObject();
    } : t.includes(".csv") ? () => {
      vt(t).then((e) => {
        this.getTotal(e), this.data = e, this.drawFromFile();
      });
    } : t.includes(".tsv") ? () => {
      bt(t).then((e) => {
        this.getTotal(e), this.data = e, this.drawFromFile();
      });
    } : void 0;
  }
  addScales() {
    this.xScale = Tt().rangeRound([0, this.width]).padding(this.padding).domain(this.data.map((e) => e[this.labels])), this.yScale = Z().rangeRound([this.height, 0]).domain([0, Q(this.data, (e) => e.total)]).nice();
    const t = this.dataFormat === "object" ? this.data.map((e) => e[this.labels]) : this.data.columns;
    this.zScale = Ai().range(["#98abc5", "#8a89a6", "#7b6888", "#6b486b", "#a05d56", "#d0743c", "#ff8c00"]).domain(t);
  }
  addLabels() {
    this.xLabel !== "" && this.svg.append("text").attr("x", this.width / 2).attr("y", this.height + this.margin.bottom / 2).attr("dx", "1em").attr("class", "labelText").style("text-anchor", "middle").style("font-family", this.fontFamily).style("font-size", this.labelFontSize).text(this.xLabel), this.yLabel !== "" && this.svg.append("text").attr("transform", "rotate(-90)").attr("y", 0 - this.margin.left / 1.4).attr("x", 0 - this.height / 2).attr("dy", "1em").attr("class", "labelText").style("text-anchor", "middle").style("font-family", this.fontFamily).style("font-size", this.labelFontSize).text(this.yLabel);
  }
  addAxes() {
    const t = Ht(this.xScale).tickSize(0);
    this.svg.append("g").attr("transform", "translate(0," + this.height + ")").call(t).attr("class", `xAxis${this.graphClass}`).selectAll("text").attr("transform", "translate(-10,0)rotate(-45)").style("text-anchor", "end").style("font-family", this.fontFamily).style("font-size", this.axisFontSize === void 0 ? `${Math.min(0.8, Math.min(this.width, this.height) / 140)}rem` : this.axisFontSize).style("opacity", 0.9);
    const e = Dt(this.yScale).tickSize(0);
    this.svg.append("g").call(e).attr("class", `yAxis${this.graphClass}`).selectAll("text").style("font-family", this.fontFamily).style("font-size", this.axisFontSize === void 0 ? `${Math.min(0.95, Math.min(this.width, this.height) / 140)}rem` : this.axisFontSize).style("opacity", 0.9), z("path.domain").attr("stroke", "transparent");
  }
  makeAxesRough(t, e) {
    const s = `xAxis${this.graphClass}`, n = `yAxis${this.graphClass}`, r = `rough-${s}`, a = `rough-${n}`;
    x(`.${s}`).selectAll("path.domain").each(function(h, o) {
      const l = x(this).node().getAttribute("d"), u = e.path(l, { fillStyle: "hachure" });
      u.setAttribute("class", r), t.appendChild(u);
    }), z(`.${r}`).attr("transform", `translate(0, ${this.height})`), x(`.${n}`).selectAll("path.domain").each(function(h, o) {
      const l = x(this).node().getAttribute("d"), u = e.path(l, { fillStyle: "hachure" });
      u.setAttribute("class", a), t.appendChild(u);
    });
  }
  setTitle(t) {
    this.svg.append("text").attr("x", this.width / 2).attr("y", 0 - this.margin.top / 2).attr("class", "title").attr("text-anchor", "middle").style("font-size", this.titleFontSize === void 0 ? `${Math.min(40, Math.min(this.width, this.height) / 5)}px` : this.titleFontSize).style("font-family", this.fontFamily).style("opacity", 0.8).text(t);
  }
  addInteraction() {
    z(this.interactionG).each(function(n, r) {
      const a = this.attributes;
      x(this).append("rect").attr("x", a.x.value).attr("y", a.y.value).attr("width", a.width.value).attr("height", a.height.value).attr("fill", "transparent");
    });
    const t = x(this.el).append("div").style("opacity", 0).attr("class", "tooltip").style("position", "absolute").style("background-color", "white").style("border", "solid").style("border-width", "1px").style("border-radius", "5px").style("padding", "3px").style("font-family", this.fontFamily).style("font-size", this.tooltipFontSize).style("pointer-events", "none"), e = this;
    let s;
    z(this.interactionG).on("mouseover", function() {
      t.style("opacity", 1), s = x(this).selectAll("path").style("stroke"), x(this).select("path").style("stroke", e.highlight), x(this).selectAll("path:nth-child(2)").style("stroke-width", e.strokeWidth + 1.2);
    }), z(this.interactionG).on("mouseout", function() {
      t.style("opacity", 0), x(this).select("path").style("stroke", s), x(this).selectAll("path:nth-child(2)").style("stroke-width", e.strokeWidth);
    }), z(this.interactionG).on("mousemove", function(n) {
      const r = x(this).attr("attrX"), a = x(this).attr("attrY"), h = kt(this);
      t.html(`<b>${r}</b>: ${a}`).style("opacity", 0.95).attr("class", function(o) {
      }).style("transform", `translate(${h[0] + e.margin.left}px, 
          ${h[1] - (e.height + e.margin.top + e.margin.bottom / 2)}px)`);
    });
  }
  initRoughObjects() {
    this.roughSvg = document.getElementById(this.roughId), this.rcAxis = D(this.roughSvg, { options: { strokeWidth: this.axisStrokeWidth, roughness: this.axisRoughness } }), this.rc = D(this.roughSvg, { options: { stroke: this.stroke === "none" ? void 0 : this.stroke, strokeWidth: this.innerStrokeWidth, roughness: this.roughness, bowing: this.bowing, fillStyle: this.fillStyle } });
  }
  stacking() {
    this.data.forEach((t) => {
      const e = Object.keys(t);
      let s = 0;
      e.forEach((n, r) => {
        if (r > 0 && n !== "total") {
          s += parseInt(t[n], 10);
          const a = this.xScale(t[this.labels]), h = this.yScale(s), o = this.xScale.bandwidth(), l = this.height - this.yScale(+t[n]), u = this.rc.rectangle(a, h, o, l, { fill: this.stackColorMapping[n] || this.colors[r], stroke: this.stackColorMapping[n] || this.colors[r], simplification: this.simplification, fillWeight: this.fillWeight }), d = this.roughSvg.appendChild(u);
          d.setAttribute("class", this.graphClass), d.setAttribute("attrX", t[this.labels]), d.setAttribute("keyY", n), d.setAttribute("attrY", +t[n]), d.setAttribute("x", a), d.setAttribute("y", h), d.setAttribute("width", o), d.setAttribute("height", l);
        }
      });
    });
  }
  drawFromObject() {
    this.initRoughObjects(), this.addScales(), this.addAxes(), this.makeAxesRough(this.roughSvg, this.rcAxis), this.addLabels(), this.stacking(), z(this.interactionG).selectAll("path:nth-child(2)").style("stroke-width", this.strokeWidth), this.interactive === !0 && this.addInteraction();
  }
  drawFromFile() {
    this.initRoughObjects(), this.addScales(), this.addAxes(), this.makeAxesRough(this.roughSvg, this.rcAxis), this.addLabels(), this.stacking(), z(this.interactionG).selectAll("path:nth-child(2)").style("stroke-width", this.strokeWidth), this.interactive === !0 && this.addInteraction();
  }
}
export {
  lh as Bar,
  uh as BarH,
  ch as Donut,
  fh as Force,
  dh as Line,
  gh as Network,
  ph as Pie,
  mh as Scatter,
  Ah as StackedBar
};
